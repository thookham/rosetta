__kernel void CALC_RAY_SCORE(
	   __global float4 *rays,	
	      __global float4 *atomcoords_noh,
...
int const NUM_PARTICLES = (int)weights[3];
  ...
  float4 ray = rays[rayID];
    ...
    float4 ray_origin = RAYorigins[origin_index-1];//RAYorigins indexed from 0
...
float4 ligatom = atomcoords_noh[atomID];
...
__kernel void CALC_ELECTROSTATICS_SCORE(
        __global float4 *atomcoords_h,
	__global float4 *particles_translation_offset,
	__global float4 *particles_rotation_offset,
	__global float4 *ligCoM,
	__global float *elec_scores,
        __global float *gpu_espGrid,
	__global float *gpu_typGrid,
	__global float *grid_dim,
        __global float *grid_mid,
        const unsigned int NUM_ATOMS,
        const unsigned int TOTAL_ATOMS
    	       	          )         
  			         {

int espID = get_global_id(0);		
 if(espID >= TOTAL_ATOMS) return;
int particleID = (int)(espID/NUM_ATOMS);

float4 rot_ang = particles_rotation_offset[particleID];
float4 trans_offset = particles_translation_offset[particleID];

int atomposition = (int)(espID%NUM_ATOMS);
//Select the conformer and the atom position within the conformer
int ligconfID = (int)trans_offset.w;
int atomID = (ligconfID*NUM_ATOMS) + atomposition;
float4 ligatom = atomcoords_h[atomID];
float4 ligand_CoM = ligCoM[ligconfID];
...
ligatom.z = ligatom_z + ligand_CoM.z;
//Apply translation offset
ligatom.x -= trans_offset.x;
ligatom.y -= trans_offset.y;
ligatom.z -= trans_offset.z;
...
float spacing = grid_mid[3];
 ...
 float atm_esp_energy = 0;
      ...
      float grid_coord_x = (ligatom.x - ( mid_x - ( (float)((dim_x-1)/2) * spacing) ))/spacing;
      float grid_coord_y = (ligatom.y - ( mid_y - ( (float)((dim_y-1)/2) * spacing) ))/spacing;
      float grid_coord_z = (ligatom.z - ( mid_z - ( (float)((dim_z-1)/2) * spacing) ))/spacing;
      ...
      float C00 = (gpu_espGrid[inx_000] * (1-Xd)) + (gpu_espGrid[inx_100] * Xd);
      float C10 = (gpu_espGrid[inx_010] * (1-Xd)) + (gpu_espGrid[inx_110] * Xd);
      float C01 = (gpu_espGrid[inx_001] * (1-Xd)) + (gpu_espGrid[inx_101] * Xd);
      float C11 = (gpu_espGrid[inx_011] * (1-Xd)) + (gpu_espGrid[inx_111] * Xd);

      float C0 = C00*(1-Yd) + C10*Yd;
      float C1 = C01*(1-Yd) + C11*Yd;
      float C = C0*(1-Zd) + C1*Zd;

atm_esp_energy = C * ligatom.w;

	        if ( ( gpu_typGrid[inx_000] < 0.1 ) ||
   		     ( gpu_typGrid[inx_010] < 0.1 ) ||
		     ( gpu_typGrid[inx_001] < 0.1 ) ||
     		     ( gpu_typGrid[inx_011] < 0.1 ) ||
     		     ( gpu_typGrid[inx_100] < 0.1 ) ||
 		     ( gpu_typGrid[inx_110] < 0.1 ) ||
		     ( gpu_typGrid[inx_101] < 0.1 ) ||
   		     ( gpu_typGrid[inx_111] < 0.1 ) ) {
...
__kernel void GET_DARC_SHAPE_AND_ELECTROSTATICS_SCORE(
                         __global float *raytest_scores,
                         __global float *elec_scores,
...
__kernel void GET_DARC_SHAPE_SCORE(
                         __global float *raytest_scores,
                       	 __global float *particletest_scores,
                  	 const unsigned int NUM_RAYS,
         		 const unsigned int NUM_PARTICLES
