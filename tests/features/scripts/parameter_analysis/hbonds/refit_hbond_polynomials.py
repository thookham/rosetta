...
def refit_polynomial_from_file( fname, degree, hb_weight, verbose ) :
    ...
    return refit_polynomial_from_lines( lines, degree, hb_weight, verbose )

def refit_polynomial_from_lines( lines, degree, hb_weight, verbose ) :
    # these lines represent energies extracted from an database generated by
    # the sweep_hbond_energies executable
    last_x = -1234;
    xs = []; energies = [];
    x0s = []; y0s = [];
    for line in lines :
    ...
    for x, e in zip( xs, energies ) :
        ...
        if x > 1.1 and x < 2.4 and ( min_y == -1234 or diff < min_y ) :
            min_y = diff
            min_xval = x
    ...
    xs2, ys2 = tune_control_points( xs, ys, energies, min_xval, verbose )
    ...
    return output_name, poly

def print_polynomial_in_HBPoly_csv_format( output_name, poly, poly_index=0 ) :
    print write_polynomial_in_HBPoly_csv_format( output_name, poly, poly_index )

def write_polynomial_in_HBPoly_csv_format( output_name, poly, poly_index=0 ) :
...
def tune_control_points( xs, ys, energies, min_xval, verbose ) :
        ...
        if xmax <= 0 :
            ...
            while ( not done ) :
                if xmax + trim > min_xval :
                    ...
                    took_halfway_point = True
                # print "min_xval", min_xval, "xmax", xmax, "trim", trim
                xs3, ys3 = [], []
                for x,y in reversed(zip( xs,ys )) :
                    if x < min_xval and x >= xmax + trim :
                        xs3.append(x)
