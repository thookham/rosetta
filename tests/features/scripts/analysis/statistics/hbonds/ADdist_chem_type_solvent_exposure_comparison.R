...
id = "ADdist_chem_type_solvent_exposure_comparison",
...
d_ply(f, .(sasa_burial), function(sub_f){
	...
	comp_stats <- comparison_statistics(
		sample_sources, sub_f, c(), "ADdist", tests)
	table_id <- paste("ADdist_chem_type_solvent_exposure_comparison", sasa_burial, sep="_")
	table_title <- paste("H-Bond A-D Distance Distribution Comparison ", sasa_burial, ", B-Factor < 30", sep="")
	save_tables(self,
		comp_stats, table_id,
	...
	comp_stats <- comparison_statistics(
		sample_sources, sub_f, c("don_chem_type_name"), "ADdist", tests)
	table_id <- paste("ADdist_chem_type_solvent_exposure_comparison", sasa_burial, "by_don_chem_type", sep="_")
	table_title <- paste("H-Bond A-D Distance by Donor Chemical Type\nDistribution Comparison ", sasa_burial, ", B-Factor < 30", sep="")
	save_tables(self,
		comp_stats, table_id, sample_sources, output_dir, output_formats,
		caption=table_title, caption.placement="top")

	comp_stats <- comparison_statistics(
		sample_sources, sub_f, c("acc_hybrid"), "ADdist", tests)
	table_id <- paste("ADdist_chem_type_solvent_exposure_comparison", sasa_burial, "by_acc_hybrid", sep="_")
	table_title <- paste("H-Bond A-D Distance by Acceptor Hybrid\nDistribution Comparison ", sasa_burial, ", B-Factor < 30", sep="")
	save_tables(self,
		comp_stats, table_id, sample_sources, output_dir, output_formats,
		caption=table_title, caption.placement="top")

	comp_stats <- comparison_statistics(
		sample_sources, sub_f, c("acc_chem_type_name"), "ADdist", tests)
	table_id <- paste("ADdist_chem_type_solvent_exposure_comparison", sasa_burial, "by_acc_chem_type", sep="_")
	table_title <- paste("H-Bond A-D Distance by Acceptor Chemical Type\nDistribution Comparison ", sasa_burial, ", B-Factor < 30", sep="")
	save_tables(self,
		comp_stats, table_id, sample_sources, output_dir, output_formats,
		caption=table_title, caption.placement="top")

	comp_stats <- comparison_statistics(
		sample_sources, sub_f, c("don_chem_type_name", "acc_chem_type_name"), "ADdist", tests)
	table_id <- paste("ADdist_chem_type_solvent_exposure_comparison", "by_don_chem_type_acc_chem_type", sep="_")
	table_title <- paste("H-Bond A-D Distance by Donor and Acceptor Chemical Types\nDistribution Comparison ", sasa_burial, ", B-Factor < 30", sep="")
	save_tables(self,
		comp_stats, table_id, sample_sources, output_dir, output_formats,
		caption=table_title, caption.placement="top")
})
