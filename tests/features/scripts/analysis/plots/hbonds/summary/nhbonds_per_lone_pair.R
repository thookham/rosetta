...
CREATE TEMPORARY TABLE lone_pair_hbs AS SELECT
  acc.site_id AS site_id,
  don.site_id AS don_site_id,
  don.atmNum  AS don_atom_num,
  acc.struct_id AS struct_id,
  geom.chi as chi,
  CASE WHEN (
    geom.chi < 1.57079633 and geom.chi > -1.57079633
  ) THEN 1
  ELSE 2 END AS orbital_id,
  hb.accRank as acc_rank,
  hb.energy as energy,
  struct.input_tag as input_tag,
  acc.resNum as acc_res_num,
  don.resNum as don_res_num,
  acc.chain as chain_id,
  hb.hbond_id as hbond_id
FROM
  hbond_sites AS acc,
  hbond_sites AS don,
  hbonds as hb,
...
WHERE
  ...
  acc_env.struct_id == acc.struct_id AND acc_env.site_id == acc.site_id AND
  don_env.struct_id == hb.struct_id AND don_env.site_id == hb.don_id AND
  acc_env.num_hbonds == 2 AND
  ( acc.HBChemType == 'hbacc_CXL' OR acc.HBChemType == 'hbacc_CXA' ) AND
  don.HBChemType != 'hbdon_AMO' AND
  don_env.num_hbonds == 1 AND
  geom.struct_id = hb.struct_id AND geom.hbond_id = hb.hbond_id AND
  struct.struct_id == hb.struct_id;

SELECT
  hb.orbital_id,
  CASE WHEN hb2.orbital_id == hb.orbital_id
   THEN 1
    ELSE 0 END AS same_side,
  hb2.energy / hb.energy as energy_ratio
--  hb.input_tag,
--  hb.acc_res_num,
--  hb.don_res_num as don1,
--  hb.chi as chi1,
--  hb2.don_res_num as don2,
--  hb2.chi as chi2,
--  hb.chain_id
FROM
  lone_pair_hbs as hb,
  lone_pair_hbs as hb2
WHERE
  hb2.struct_id == hb.struct_id and hb2.site_id == hb.site_id AND
  hb.acc_rank == 1 AND hb2.acc_rank == 2;"

# AND
# hb.don_res_num != hb2.don_res_num;"


# SELECT
#   hb.orbital_id AS orbid1,
#   hb3.orbital_id AS orbid2,
#   hb.chi AS chi1,
#   hb3.chi AS chi2,
...
#     WHERE
#       hb2.struct_id == hb.struct_id and hb2.site_id == hb.site_id AND
#       hb2.orbital_id == hb.orbital_id AND hb2.acc_rank == 2
#     ) THEN 1
#     ELSE 0 END AS same_side
# FROM
#   lone_pair_hbs as hb,
#   lone_pair_hbs as hb3
# WHERE
#   hb3.struct_id == hb.struct_id and hb3.site_id == hb.site_id AND
#   hb.acc_rank == 1 AND hb3.acc_rank == 2
# LIMIT 20
# ";
...
subf1 <- f[ f$same_side == 1, ]
print("same side")
print(summary(subf1))

subf2 <- f[ f$same_side == 0, ]
...
# TEMP !
# TEMP !# tf2 <- cut(tf,tf.acc_chem_type,labels=T)
# TEMP !agg<-aggregate(tf$Freq, by=list(sample_source=tf$sample_source,acc_chem_type=tf$acc_chem_type), FUN=sum )
# TEMP !print(agg)
# TEMP !
# TEMP !count = 0
# TEMP !tf2 <- ddply( tf, c("sample_source","acc_chem_type","num_hbonds"), function(subf) {
# TEMP !    data.frame(perc=subf$Freq / agg[ agg$sample_source == subf$sample_source & agg$acc_chem_type == subf$acc_chem_type, "x" ], count=agg[ agg$sample_source == subf$sample_source & agg$acc_chem_type == subf$acc_chem_type, "x" ] )
# TEMP !})
# TEMP !cat(paste("count",count))
# TEMP !print(tf2)
# TEMP !
# TEMP !
# TEMP !
# TEMP !tf2$acc_chem_type_name <- acc_chem_type_name_linear(tf2$acc_chem_type)
# TEMP !
# TEMP !plot_id <- "nhbonds_per_site_by_acc_chem_type"
# TEMP !ggplot(tf2, aes(x=num_hbonds,group=sample_source,colour=sample_source,fill=sample_source)) + theme_bw() +
# TEMP !  #geom_indicator(aes(indicator=count)) +
# TEMP !  geom_bar( aes(y=perc), stat="identity", position="dodge" ) +
# TEMP !
# TEMP !  facet_grid( ~ acc_chem_type_name ) +
# TEMP !  ggtitle("Number of HBonds Per Site by Acceptor Type") +
# TEMP !  labs(x="Acceptor Type", y="counts") +
# TEMP !  #scale_y_continuous( labels = percent_format() )
# TEMP !
# TEMP !save_plots(self, plot_id, sample_sources, output_dir, output_formats)
# TEMP !
# TEMP !# ................ Now the donors
# TEMP !
# TEMP !sele <-"
...
# TEMP !
# TEMP !
# TEMP !f <- query_sample_sources(sample_sources, sele)
# TEMP !
# TEMP !tf <- as.data.frame(table(f))
# TEMP !print(tf)
# TEMP !
# TEMP !# tf2 <- cut(tf,tf.don_chem_type,labels=T)
# TEMP !agg<-aggregate(tf$Freq, by=list(sample_source=tf$sample_source,don_chem_type=tf$don_chem_type), FUN=sum )
# TEMP !print(agg)
# TEMP !
# TEMP !count = 0
# TEMP !tf2 <- ddply( tf, c("sample_source","don_chem_type","num_hbonds"), function(subf) {
# TEMP !    data.frame(perc=subf$Freq / agg[ agg$sample_source == subf$sample_source & agg$don_chem_type == subf$don_chem_type, "x" ], count=agg[ agg$sample_source == subf$sample_source & agg$don_chem_type == subf$don_chem_type, "x" ] )
# TEMP !})
# TEMP !cat(paste("count",count))
# TEMP !print(tf2)
# TEMP !
# TEMP !
# TEMP !
# TEMP !tf2$don_chem_type_name <- don_chem_type_name_linear(tf2$don_chem_type)
# TEMP !
...
# TEMP !  ggtitle("Number of HBonds Per Site by Donor Type") +
