...
WHERE
  ...
  acc_site_pdb.struct_id = hbond.struct_id AND acc_site_pdb.site_id = hbond.acc_id AND
	(acc_site.HBChemType = 'hbacc_AHX' OR acc_site.HBChemType = 'hbacc_HXL') AND
...
ggplot(data=dens) +
	...
	ggtitle("Hydrogen Bonds BAH Angle by for Hydroxyl Acceptors, SeqSep > 5; BFactors < 30\n(normalized for equal volume per unit distance)") +
