...
CREATE TEMPORARY TABLE antiparallel_Ca_O AS
SELECT dist.Ca_O_dist AS dist, 'antiparallel' AS strand_orientation
...
CREATE TEMPORARY TABLE antiparallel_O_Ca AS
SELECT dist.O_Ca_dist AS dist, 'antiparallel' AS strand_orientation
FROM   protein_backbone_atom_atom_pairs AS dist,
       antiparallel_close_contact_residue_pairs AS atpairs
WHERE  dist.struct_id =  atpairs.struct_id AND
...
CREATE TEMPORARY TABLE parallel_O_Ca AS
SELECT dist.O_Ca_dist AS dist, 'parallel' AS strand_orientation
FROM   protein_backbone_atom_atom_pairs AS dist,
...
CREATE TEMPORARY TABLE parallel_Ca_O AS
SELECT dist.Ca_O_dist AS dist, 'parallel' AS strand_orientation
FROM   protein_backbone_atom_atom_pairs AS dist,
       parallel_close_contact_residue_pairs AS apairs
WHERE  dist.struct_id =  apairs.struct_id AND
...
SELECT * FROM antiparallel_Ca_O UNION
SELECT * FROM antiparallel_O_Ca UNION
SELECT * FROM parallel_Ca_O     UNION
SELECT * FROM parallel_O_Ca;"
...
ylims <- c(10^-3,5)
plot_id <- "SSDists_Sheet_O_CA_parallel_anti_parallel"
p <- ggplot() + theme_bw() +
