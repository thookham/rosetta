...
CREATE TEMPORARY TABLE antiparallel_AHdists AS
SELECT geom.AHdist AS dist, 'antiparallel' AS strand_orientation
...
CREATE TEMPORARY TABLE parallel_AHdists AS
SELECT geom.AHdist AS dist, 'parallel' AS strand_orientation
FROM   hbond_geom_coords as geom,
			 parallel_close_contact_hbonds AS phbond
WHERE	 geom.struct_id =  phbond.struct_id AND geom.hbond_id = phbond.hbond_id;

SELECT * FROM antiparallel_AHdists UNION
SELECT * FROM     parallel_AHdists;"
