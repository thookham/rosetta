...
id = "EQ_by_dssp_lower_chi_tight_kernel",
...
d_ply(f, .(res_type, dssp), function(sub_f){
	...
	dssp_id <- sub_f$dssp[1]
	plot_id <- paste("rotamer_", res_type, "_", dssp_id, "_chi3_by_chi1_chi2_tight_kernel", sep="")
	p <- ggplot(data=dens) +
		theme_bw() +
		geom_line(aes(x=x, y=y, colour=sample_source)) +
		geom_indicator(aes(indicator=counts, colour=sample_source, group=sample_source)) +
		facet_grid(chi1_bin ~ chi2_bin) +
		ggtitle((res_type, " ", dssp_id, " chi3 by chi2 and chi1, BFact < 30", sep="")) +
