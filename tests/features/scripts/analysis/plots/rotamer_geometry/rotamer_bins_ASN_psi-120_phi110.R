...
WHERE
	bb.struct_id = res.struct_id AND
	bb.resNum = res.resNum AND
	-130 < bb.phi AND bb.phi < -110 AND
	100 < bb.psi AND bb.psi < 120 AND
	rot.struct_id = res.struct_id AND
	rot.residue_number = res.resNum AND
	res.name3 = 'ASN'
GROUP BY
	rotamer_bin
ORDER BY
...
plot_id <- "rotamer_bin_counts_ASN_psi-120_phi110"
