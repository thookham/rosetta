...
for i in range( 0, len( scorefiles ) ):
	...
	# read in score file
	x = subprocess.getoutput( "grep -v SEQUENCE " + scorefiles[i] + " | grep -v total_score | awk '{print $" + x_index + "}'" ).splitlines()
	y = subprocess.getoutput( "grep -v SEQUENCE " + scorefiles[i] + " | grep -v total_score | awk '{print $" + y_index + "}'" ).splitlines()
