...
## @file  docking_ensemble/1.submit.py
...
def partners(argument):
    """To get the partner chains for setting up docking fold-tree
    """
    switcher = {
        "1AY7": "A_B",
        "1MAH": "A_F",
...
def get_ensemble_path( argument, outfile ):
    """To update the path to the ensemble without affecting the prepacking
    probabilities
    *** Parameters **
        argument : *str*
            Name of the current list file with ensemble paths
        outfile : *str*
            Name of the updated list file
    """
    filepath = argument.rsplit('/', 1)[0]
    updated_file = open( outfile, 'w' )
    current_file = open( argument, 'r' ).readlines()

    for i in range( len(current_file) ):
        if 'ppk' in current_file[i]:
            updated_file.write( filepath + '/' + current_file[i] )
        else:
            updated_file.write( current_file[i] )
...
-in:file:s {rosetta_dir}/tests/scientific/data/{testname}/targets/{target}_u.pdb
-unboundrot {rosetta_dir}/tests/scientific/data/{testname}/targets/{target}_u.pdb
-in:file:native {rosetta_dir}/tests/scientific/data/{testname}/natives/{target}_b.pdb

-nstruct        {nstruct}

-ensemble1      {receptor_list}
-ensemble2      {ligand_list}
...
for target in targets:
    ...
    # Create the ensembles directory that stores the list files
    if not os.path.exists( f'{working_dir}/input/{target}' ): os.makedirs( f'{working_dir}/input/{target}' )
    receptor_list = get_ensemble_path( f'{rosetta_dir}/tests/scientific/data/{testname}/ensembles/{target}/rec_list' , f'{working_dir}/input/{target}/receptor_list' )
    ligand_list = get_ensemble_path( f'{rosetta_dir}/tests/scientific/data/{testname}/ensembles/{target}/lig_list' , f'{working_dir}/input/{target}/ligand_list' )
    prefix = f'{working_dir}/output/{target}'
    if not os.path.exists(prefix): os.makedirs(prefix)

    hpc_job_ids.append( hpc_driver.submit_serial_hpc_job(
        name = f'docking_ensemble-{target}',
