...
## @file  fragments_picking/2.analyze.py
...
scorefiles.extend( [ f'{working_dir}/output/{t}/{t}-abinitio.fsc' for t in targets ] )
#logfiles.extend( [ f'{working_dir}/hpc-logs/hpc.{testname}-{t}.*.log' for t in targets ] )

# get column numbers from labels, 1-indexed
print(scorefiles[0])
print(subprocess.getoutput( "grep " + x_label + " " + scorefiles[0] ).split())
...
for i in range( 0, len( scorefiles ) ):
	...
	f.write( targets[i] + "\t" )
	val_cutoff = qm.check_xpercent_values_below_cutoff( x, cutoffs_rmsd_dict[targets[i]], "rmsd", f , 10)
