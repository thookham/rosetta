...
x_label = "MotifScorer_1"
y_label = "InterModelMotifScorer_2"
outfile = "result.txt"
cutoffs = "cutoffs"
...
for i in range( 0, len( scorefiles ) ):
	...
	f.write( targets[i] + "\t" )
	val_cutoff = qm.check_xpercent_values_below_cutoff( x, cutoffs_rmsd_dict[targets[i]], "MotifScorer_1", f, 10 )
	target_results.update( val_cutoff )

    # add to failues
	if val_cutoff['All MotifScorer_1s < cutoff'] == False:
		failures.append( targets[i] )

	# check for scores below cutoff
	f.write( targets[i] + "\t" )
	val_cutoff = qm.check_xpercent_values_below_cutoff( y, cutoffs_score_dict[targets[i]], "InterModelMotifScorer_2", f, 10 )
	target_results.update( val_cutoff )

    # add to failures
	if val_cutoff['All InterModelMotifScorer_2s < cutoff'] == False:
		failures.append( targets[i] )
...
#		target_results.update( val_topscoring )
	...
	f.write( targets[i] + "\t" )
	val_rms = qm.check_range( x, "MotifScorer_1", f )
	target_results.update( val_rms )

	# check for score range
	f.write( targets[i] + "\t" )
	val_score = qm.check_range( y, "InterModelMotifScorer_2", f )
