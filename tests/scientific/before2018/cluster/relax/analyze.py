...
for t in targets:
    fname = 'output/%s/%s.out' % (t, t)
    if os.path.isfile(fname):
        (res, output) = commands.getstatusoutput('cd output/%s && cat %s.out | grep SCORE:' % (t, t) )
        lines = output.split('\n')
...
thresholds = yaml.load( file('ScientificCluster.Relax.thresholds').read() )
...
results['_isTestPassed'] = num_of_passed_targets > 58
