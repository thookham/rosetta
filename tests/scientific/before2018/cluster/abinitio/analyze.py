...
for t in targets:
    ...
    if os.path.isfile(fname):
        ...
        for l in lines[1:]:
            rms = float( l.split()[rms_key] )
            score = float( l.split()[score_key] )
...
thresholds = yaml.load( file('ScientificCluster.Abinitio.thresholds').read() )
num_of_passed_targets = 0
for k in thresholds:
    #print 'k=%s thresholds[k]=%s' % (k, thresholds[k])
    for sk in thresholds[k]:
        if k in results  and  sk in results[k]  and  k in thresholds \
           and  results[k][sk] < thresholds[k][sk]: num_of_passed_targets += 1

results['num_of_passed_targets'] = num_of_passed_targets
results['_isTestPassed'] = num_of_passed_targets > 51
