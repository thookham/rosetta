...
## @file  mpddg_cartesian/2.analyze.py
...
for t in targets:
	...
	for ref in exp_ref_list[t]:
		...
		file_ddg = f'{rosetta_dir}/tests/scientific/data/mp_ddg_cartesian/{t}/{ref[1]}'
		...
		for i in range(0, len(res_num)):
			...
			file_mut = f'{working_dir}/output/{t}/mutfile_{residue[i]}{res_num[i]}.ddg'
			...
			df = pd.read_csv(file_mut, header=None, sep='\s+')
			df = df.loc[(df[0]=='COMPLEX:')].reset_index(drop=True)
			score_wt = df.loc[(df[2].str.startswith('WT')), 3].to_list()
			for variant in df[2].unique():
				if not variant.startswith('WT'):
					resi = variant.split('_')[1][:-4]
					mut_AA = variant[-4:-1]
					var = f"{residue[i]}{resi}{d3t1[mut_AA]}"
					if var == f"{residue[i]}{res_num[i]}{mutant[i]}":
						score_mut = df.loc[(df[2]==variant), 3].to_list()
