...
def run(test, options):
    ...
    return dict(state='failed' if failed else 'passed', max_memory_allocated=max_memory_allocated, execution_time=execution_time, memory_usage=[ dict(time=m[0], memory=m[1]) for m in memory])
