...
def parseargs():
    ...
    parser.add_argument(
        "-d", "--density", default="density.mrc", help="The electron density map"
    )
    parser.add_argument(
        "-f", "--fasta", default="input.fasta", help="The sequence in fasta format"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Which loop are you operating on. Default will leave xml unchanged",
    )
    parser.add_argument(
        "-c",
        "--cores",
        type=int,
        default=1,
        help="How many cores you have available per node",
    )
    parser.add_argument(
        "-t",
        "--taboo",
        type=int,
        default=1,
        help="Do you want to use taboo filtering when available",
    )
    parser.add_argument(
        "-compare",
        "--compare",
        type=int,
        default=1,
        help="Do you want to run the comparator when all jobs are finished?",
    )
    parser.add_argument(
        "-docker",
        "--docker",
        type=int,
        default=1,
        help="Set to 1 if these arguments are being sent to the docker image or being run inside it",
    )
    parser.add_argument(
        "-rp", "--relaxpdbs", nargs="+", default=[], help="The pdbs to relax"
    )
    parser.add_argument(
        "-rlxs",
        "--relax_script",
        default="runrelax.sh",
        help="The script to call relax",
    )
    parser.add_argument(
        "-lps",
        "--lpsfiles",
        nargs="+",
        default=[],
        help="The loop partial solution files used by the Monte Carlo Assembly",
    )
    parser.add_argument(
        "-rs", "--runscript", default="runES.sh", help="The script to call RosettaES"
    )
    parser.add_argument(
        "-rd",
        "--rosettadir",
        default="/Rosetta/",
        help="The Rosetta directory containing a bin/ and database/ directory",
    )
    parser.add_argument(
        "-db", "--dumpbeam", help="The beam file you want to dump the pdbs of"
    )
    parser.add_argument(
        "-s",
        "--setup",
        type=int,
        default=1,
        help="Do you want to pick fragments before running",
    )
...
def update_xml(args, dumpbeam, read_from_file, finalbeam=1):
    for line in fileinput.input(args.xml, inplace=1):
        ...
        line = re.sub(
            "read_from_file=\S+", 'read_from_file="%s"' % read_from_file, line
        )
        ...
        if __name__ == "__main__":
            ...
            for i in range(1, args.cores + 1):
                p = multiprocessing.Process(
                    target=run, args=(args, "beam_%d_%d.txt" % (counter, i), i)
                )
                jobs.append(p)
                p.start()
                jobcount = jobcount + 1
