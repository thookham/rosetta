...
)
{
	...
	// There's cases where we can create a new ResidueType when switching.
	// We only want to replace the PoseRTS once -- we keep a mod version around,
	// only replacing it once.
	core::chemical::PoseResidueTypeSetOP mod_rts = nullptr;
	...
	for ( core::Size i=1; i<= pose.size(); ++i ) {
		...
		if ( ! new_rsd_type ) {
			if ( mod_rts != nullptr && mod_rts->has_name( rsd.name() ) ) {
				new_rsd_type = mod_rts->name_mapOP( rsd.name() );
			} else {
				core::chemical::MutableResidueTypeOP gen_restype( generate_replacement_restype( rsd.type(), type_set.mode() ) );
				if ( gen_restype ) {
					...
					if ( mod_rts == nullptr ) {
						mod_rts = pose.conformation().modifiable_residue_type_set_for_conf( type_set.mode() );
					}
					mod_rts->add_unpatchable_residue_type( gen_restype );
					new_rsd_type = mod_rts->name_mapOP( gen_restype->name() );
				}
			}
		}
