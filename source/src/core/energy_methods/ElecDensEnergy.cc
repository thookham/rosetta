...
// Unit headers
#include <core/energy_methods/ElecDensEnergy.hh>
...
/// c-tor
ElecDensEnergy::ElecDensEnergy() : parent( utility::pointer::make_shared< ElecDensEnergyCreator >() ) {}
		...
		if ( pose.residue(resid).aa() == core::chemical::aa_vrt )  {
			...
			if ( remapSymm ) {
				...
				for ( int i=0; i<=(int)myClones.size(); ++i ) {
					...
					// divide by the number of subunits since SymmetricScoreFunction will scale up later
					numeric::xyzVector< core::Real > dEdx = 0.5 * ( 1.0 / p_null ) *
						(-2.0/sqrt(M_PI)) *
						exp(-SQ( z_CC/sqrt(2.0) )) *
						1/sqrt(0.02) *
