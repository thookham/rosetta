...
check_full_model_info_OK( pose::Pose const & pose ){
	...
	for ( Size n = 1; n <= res_list.size(); ++n ) {
		...
		if ( sequence_char == 'r' && pose.residue_type( n ).name1() != 'c' && pose.residue_type( n ).name1() != 'u' && pose.residue_type( n ).name1() != 't' ) continue;
		if ( sequence_char == 'y' && pose.residue_type( n ).name1() != 'a' && pose.residue_type( n ).name1() != 'g' ) continue;
		if ( sequence_char == 's' && pose.residue_type( n ).name1() != 'a' && pose.residue_type( n ).name1() != 'u' && pose.residue_type( n ).name1() != 't' ) continue;
		if ( sequence_char == 'w' && pose.residue_type( n ).name1() != 'g' && pose.residue_type( n ).name1() != 'c' ) continue;
		if ( sequence_char == 'm' && pose.residue_type( n ).name1() != 'g' && pose.residue_type( n ).name1() != 'u' && pose.residue_type( n ).name1() != 't' ) continue;
		if ( sequence_char == 'k' && pose.residue_type( n ).name1() != 'c' && pose.residue_type( n ).name1() != 'a' ) continue;
		if ( sequence_char != pose.residue_type( n ).name1() ) {
			TR << res_list << std::endl;
			TR << "no match at " << n << " conventional numbering: " << res_num << "  sequence: " << sequence_char << " pose sequence: " <<  pose.residue_type( n ).name1() << std::endl;
			TR << "POSE SEQUENCE       " << pose.sequence() << std::endl;
