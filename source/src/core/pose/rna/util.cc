...
get_inf_base_pairs(pose::Pose & pose, pose::Pose const & native_pose) {
	// We need the number of interactions correctly retrieved from the
	// native, the number of additional interactions (false positives),
	// and the number of interactions missed.
	// inf_bp = sqrt(ppv * sty)
	// ppv = tp/(tp+fp)
	// sty = tp/(tp+fn)
	// tp+fn = bps in native
	// tp+fp = bps in model
	// tp = bps in native and model
	...
	utility::vector1< bool > is_bulged_native;
	core::pose::rna::classify_base_pairs( native_pose, base_pair_list_native, is_bulged_native );
	Size const n_bp_model = base_pair_list.size();
	Size const n_bp_native = base_pair_list_native.size();
	...
	for ( BasePair const & native_base_pair : base_pair_list_native ) {
		...
		if ( check_in_base_pair_list( decoy_base_pair /*from native*/, base_pair_list /*for decoy*/) ) n_retrieved++;
	}

	Real const ppv = Real( n_retrieved ) / Real( n_bp_model );
	Real const sty = Real( n_retrieved ) / Real( n_bp_native );
	return std::pow( sty * ppv, 0.5 );
}
