// -*- mode:c++;tab-width:2;indent-tabs-mode:t;show-trailing-whitespace:t;rm-trailing-spaces:t -*-
// vi: set ts=2 noet:
//
// (c) Copyright Rosetta Commons Member Institutions.
// (c) This file is part of the Rosetta software suite and is made available under license.
// (c) The Rosetta software is developed by the contributing members of the Rosetta Commons.
// (c) For more information, see http://www.rosettacommons.org. Questions about this can be
// (c) addressed to University of Washington CoMotion, email: license@uw.edu.
// (C) 199x-2008 University of Washington
// (C) 199x-2008 University of California Santa Cruz
// (C) 199x-2008 University of California San Francisco
// (C) 199x-2008 Johns Hopkins University
// (C) 199x-2008 University of North Carolina, Chapel Hill
// (C) 199x-2008 Vanderbilt University

/// @file   core/io/sequence_comparation/DesignContrast.fwd.hh
///
/// @brief
/// @author Yi Liu

#ifndef INCLUDED_core_io_sequence_comparation_DesignContrast_fwd_hh
#define INCLUDED_core_io_sequence_comparation_DesignContrast_fwd_hh


namespace core {
namespace io {
namespace sequence_comparation {

class DesignContrast;


} // namespace sequence_comparation
} // namespace io
} // namespace core


#endif // INCLUDED_core_io_sequence_comparation_DesignContrst_FWD_HH
