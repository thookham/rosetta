...
) const
{
	...
	if ( !atom4->is_jump() &&
		...
		if ( !quiet ) {
			TR.Debug << "No proper DoF can be found for these four atoms: ";
			TR.Debug << atom1_in_id.rsd() << "-" << atom1_in_id.atomno() << ", ";
			TR.Debug << atom2_in_id.rsd() << "-" << atom2_in_id.atomno() << ", ";
			TR.Debug << atom3_in_id.rsd() << "-" << atom3_in_id.atomno() << ", ";
			TR.Debug << atom4_in_id.rsd() << "-" << atom4_in_id.atomno() << "!" << std::endl;

			if ( TR.Trace.visible() ) {
				TR.Trace << "What condition failed?" << std::endl;
				TR.Trace << "!atom4->is_jump() = " << !atom4->is_jump() << std::endl;
				TR.Trace << "!atom4->keep_dof_fixed( id::PHI ) = " << !atom4->keep_dof_fixed( id::PHI ) << std::endl;
				if ( !atom4->is_jump() ) {
					TR.Trace << "(atom4->raw_input_stub_atom1() ~ " << atom4->raw_input_stub_atom1()->id().rsd() << "-" << atom4->raw_input_stub_atom1()->id().atomno()
						<< ") == ( atom3 ~ " << atom3_in_id.rsd() << "-" << atom3_in_id.atomno() << " ) = " << ( atom4->raw_input_stub_atom1() == atom3 ) << std::endl;
					TR.Trace << "(atom4->raw_input_stub_atom2() ~ " << atom4->raw_input_stub_atom2()->id().rsd() << "-" << atom4->raw_input_stub_atom2()->id().atomno()
						<< ") == ( atom2 ~ " << atom2_in_id.rsd() << "-" << atom2_in_id.atomno() << " ) = " << ( atom4->raw_input_stub_atom2() == atom2 ) << std::endl;
				}
				TR.Trace << " or... " << std::endl;

				TR.Trace << "!atom1->is_jump() = " << !atom1->is_jump() << std::endl;
				TR.Trace << "!atom1->keep_dof_fixed( id::PHI ) = " << !atom1->keep_dof_fixed( id::PHI ) << std::endl;
				if ( !atom1->is_jump() ) {
					TR.Trace << "atom1->raw_input_stub_atom1() ~ " << atom1->raw_input_stub_atom1()->id().rsd() << "-" << atom1->raw_input_stub_atom1()->id().atomno()
						<< ") == ( atom2 ~ " << atom2_in_id.rsd() << "-" << atom2_in_id.atomno() << " ) = " << ( atom1->raw_input_stub_atom1() == atom2 ) << std::endl;
					TR.Trace << "atom1->raw_input_stub_atom2() ~ " << atom1->raw_input_stub_atom2()->id().rsd() << "-" << atom1->raw_input_stub_atom2()->id().atomno()
