...
)
{
	...
	RotamerTrieBaseOP newtrie = utility::pointer::make_shared< RotamerTrie< AT, CPDAT > >( rotamer_descriptors, atomic_interaction_cutoff );
...
)
{
	...
	RotamerTrieBaseOP newtrie = utility::pointer::make_shared< RotamerTrie< AT, CPDAT > >( rotamer_descriptor, atomic_interaction_cutoff );
	for ( Size ii = 1; ii <= cpdata_map.n_entries(); ++ii ) {
		newtrie->set_resid_2_connection_entry( cpdata_map.resid_for_entry( ii ), ii );
	}
	return newtrie;
