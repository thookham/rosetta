...
EnergyMethodOptions::operator = (EnergyMethodOptions const & src) {
	if ( this != &src ) {
		...
		depc_ms_input_ = src.depc_ms_input_;
...
void EnergyMethodOptions::initialize_from_options( utility::options::OptionCollection const & options ) {
	...
	count_pair_full_ = options[ basic::options::OptionKeys::score::count_pair_full]();
...
EnergyMethodOptions::list_options_read( utility::options::OptionKeyList & read_options )
{
	...
	read_options
		...
		+ basic::options::OptionKeys::score::count_pair_full
		...
		+ basic::options::OptionKeys::score::depc_ms_input
...
std::string
EnergyMethodOptions::depc_ms_input() const {
	return depc_ms_input_;
}

void
EnergyMethodOptions::depc_ms_input( std::string const & setting ) {
	depc_ms_input_ = setting;
}
...
bool
EnergyMethodOptions::exclude_RNA_protein_fa_elec() const {
	return exclude_RNA_protein_fa_elec_;
}

void
EnergyMethodOptions::exclude_RNA_protein_fa_elec( bool const setting ) {
	exclude_RNA_protein_fa_elec_ = setting;
}
...
bool
EnergyMethodOptions::count_pair_full() const {
	return count_pair_full_;
}

void
EnergyMethodOptions::count_pair_full( bool const setting) {
	count_pair_full_ = setting;
}
...
// longxing
void
EnergyMethodOptions::target_clash_pdb( std::string const & setting ) {
	target_clash_pdb_ = setting;
}

std::string
EnergyMethodOptions::target_clash_pdb() const {
	return target_clash_pdb_;
}
...
operator==( EnergyMethodOptions const & a, EnergyMethodOptions const & b ) {
	...
	return (
		...
		( a.depc_ms_input_ == b.depc_ms_input_ ) &&
		...
		( a.count_pair_full_ == b.count_pair_full_ ) &&
...
EnergyMethodOptions::show( std::ostream & out ) const {
	...
	out << "EnergyMethodOptions::show: depc_ms_input: " << depc_ms_input_ << std::endl;
	...
	out << "EnergyMethodOptions::show: exclude_RNA_protein_fa_elec: "
		<< (exclude_RNA_protein_fa_elec_ ? "true" : "false") << std::endl;
	...
	out << "EnergyMethodOptions::show: count_pair_full: "
		<< (count_pair_full_ ? "true" : "false") << std::endl;
...
EnergyMethodOptions::insert_score_function_method_options_rows(
	...
	option_keys.push_back("depc_ms_input");
	option_values.push_back(depc_ms_input_);
	...
	option_keys.push_back("exclude_RNA_protein_fa_elec");
	option_values.push_back(exclude_RNA_protein_fa_elec_ ? "1" : "0");
	...
	option_keys.push_back("count_pair_full");
	option_values.push_back(count_pair_full_ ? "1" : "0");
...
core::scoring::methods::EnergyMethodOptions::save( Archive & arc ) const {
	...
	arc( CEREAL_NVP( depc_ms_input_ ) ); // std::string
	...
	arc( CEREAL_NVP( count_pair_full_ ) ); // _Bool
