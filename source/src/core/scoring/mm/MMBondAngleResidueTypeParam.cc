...
MMBondAngleResidueTypeParam::init(
	...
	for ( core::Size i = 1; i <= residue_type.num_bondangles(); ++i ) {
		...
		std::string const & type1(residue_type.mm_name(atom_set.key1()));
		std::string const & type2(residue_type.mm_name(atom_set.key2()));
		std::string const & type3(residue_type.mm_name(atom_set.key3()));
