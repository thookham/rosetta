...
void
NeighborhoodResidueSelector::provide_citation_info(basic::citation_manager::CitationCollectionList & citations ) const {
	...
	if ( focus_selector_ != nullptr ) {
		focus_selector_->provide_citation_info( citations );
	}
}
...
NeighborhoodResidueSelector::provide_xml_schema( utility::tag::XMLSchemaDefinition & xsd ) {
	...
	xsd_type_definition_w_attributes_and_optional_subselector( xsd, class_name(),
		...
		"A Residue Selector that selects neighboring residues to a set of residues (focus).  By default, includes this focus in the resulting selection.", attributes );
}
