...
ResidueTypeSet::get_patches( std::string const & patch_name,
	...
	if ( patch_name.find( "MP-" ) != std::string::npos ) {
		utility::vector1< std::string > split_name = utility::string_split( patch_name, '-' ) ;

		std::string metapatch_name = split_name[3];

		// This patch needs to be generated by this metapatch.
		// PHE-CD1-methylated
		// Atom name: second element if you split the patch name by '-'
		std::string atom_name = split_name[2];

		if ( metapatch_mapping.count( metapatch_name ) == 0 ) {
			return utility::vector1< PatchCOP >{}; // No patch availible.
		}
		...
		return utility::vector1< PatchCOP >{needed_patch};

	} else {
		auto iter = patch_mapping.find( patch_name );
		if ( iter == patch_mapping.end() ) {
			return utility::vector1< PatchCOP >{}; // No patch availible.
		}
...
ResidueTypeSet::apply_patch(
	...
	for ( PatchCOP const & p: get_patches( patch_name, patch_mapping, metapatch_mapping ) ) {
...
utility::vector1< PatchCOP >
ResidueTypeSet::get_patches( std::string const & name ) const {
	return get_patches( name, patch_map(), metapatch_map() ); // Use the functions to allow for subclass override.
}
