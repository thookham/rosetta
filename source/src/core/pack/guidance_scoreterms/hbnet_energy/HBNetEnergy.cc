...
/// @file   core/pack/guidance_scoreterms/hbnet_energy/HBNetEnergy.cc
...
HBNetEnergyCreator::score_types_for_method() const
{
	...
	sts.push_back( core::scoring::hbnet );
	return sts;
}
...
void HBNetEnergy::finalize_total_energy(
	...
	if ( disabled_ ) return; //Do nothing if disabled.
...
void
HBNetEnergy::setup_for_minimizing(
	core::pose::Pose & /*pose*/,
	core::scoring::ScoreFunction const & /*sfxn*/,
	core::kinematics::MinimizerMapBase const & /*minmap*/
) const {
	TR << "Disabling HBNetEnergy during minimization." << std::endl;
	disabled_ = true;
}

/// @brief Re-enable this scoreterm after a minimization trajectory.
void
HBNetEnergy::finalize_after_minimizing( core::pose::Pose & /*pose*/) const  {
	TR << "Re-enabling HBNetEnergy following minimization." << std::endl;
	disabled_ = false;
}
...
void
HBNetEnergy::provide_citation_info(basic::citation_manager::CitationCollectionList & citations ) const {
