	...
	residue_representatives_( atom_tree_representatives_.size() )
{
	...
	for ( auto
		...
		residue_representatives_[ ii ] = utility::pointer::make_shared< conformation::Residue >(
	...
	res_collections_( rsets.nmoltenres() )
{
	for ( Size ii = 1; ii <= rsets.nmoltenres(); ++ii ) {
		...
		res_collections_[ ii ] = utility::pointer::make_shared< ResidueAtomTreeCollection >(
	...
	res_collections_( task.num_to_be_packed() )
{
	...
	for ( Size ii = 1; ii <= pose.size(); ++ii ) {
		...
		res_collections_[ count_moltenres ] = utility::pointer::make_shared< ResidueAtomTreeCollection >( task.residue_task( ii ),  pose.conformation(), pose.residue( ii ) );
	}
}
...
)
{
	...
	res_collections_[ 1 ] = utility::pointer::make_shared< ResidueAtomTreeCollection >( rset, resid  );
}
...
{
	res_collections_.resize( 1 );
	res_collections_[ 1 ] = utility::pointer::make_shared< ResidueAtomTreeCollection >( rltask, pose.conformation(), pose.residue( resid ) );
}
