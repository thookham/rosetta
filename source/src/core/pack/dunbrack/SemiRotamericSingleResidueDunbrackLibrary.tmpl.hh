...
BBDepNRChiSample< P >::operator==( BBDepNRChiSample<P> const & other ) const {
	static basic::Tracer TR( "core.pack.dunbrack.BBDepNRChiSample" );
...
BBIndNRChiSample< P >::operator==( BBIndNRChiSample<P> const & other ) const {
	static basic::Tracer TR( "core.pack.dunbrack.BBIndNRChiSample" );
...
BBDepScoreInterpData< N >::operator==( BBDepScoreInterpData< N > const & other ) const {
	static basic::Tracer TR( "core.pack.dunbrack.BBDepScoreInterpData" );
...
{
	return rotamer_energy_bbdep( rsd, pose, tenergy );
}
...
) const
{
	...
	// there's probably to do the calculation just for that torsion.
	RotamerLibraryScratchSpace scratch;
	rotamer_deriv_bbdep( rsd, pose, scratch );
	scratch.extract_torsion_deriv( tor_id, rsd, pose, tderiv );
}

template < Size T, Size N >
Real
SemiRotamericSingleResidueDunbrackLibrary< T, N >::eval_nrchi_score(
	conformation::Residue const & rsd,
	pose::Pose const & pose,
	...
	Real nrchi_score = bbdep_nrchi_score( rsd, pose, packed_rotno, dnrchiscore_dchi, dnrchiscore_dbb ); //Handles D-amino acids; derivatives WRT phi, psi, and chi are inverted iff D-amino acid.
...
template < Size T, Size N >
Real
SemiRotamericSingleResidueDunbrackLibrary< T, N >::rotamer_energy_bbdep(
	conformation::Residue const & rsd,
	pose::Pose const & pose,
	rotamers::TorsionEnergy & tenergy
) const {
	...
	rotamers::TorsionEnergy parent_energy; // Just the parent energy.
	core::Size packed_rotno;
	parent::eval_rotameric_energy( rsd, pose, scratch, chidev, chidevpen, packed_rotno, parent_energy );

	Real dnrchiscore_dchi;
	utility::fixedsizearray1< Real, N > dnrchiscore_dbb;
	Real const nrchi_score = eval_nrchi_score( rsd, pose, scratch, packed_rotno, dnrchiscore_dchi, dnrchiscore_dbb );

	tenergy.tot += parent_energy.dev + nrchi_score;
	// We have zero rot energy
	tenergy.dev += parent_energy.dev;
	tenergy.semi += nrchi_score;

	return parent_energy.dev + nrchi_score;
}

template < Size T, Size N >
void
SemiRotamericSingleResidueDunbrackLibrary< T, N >::rotamer_deriv_bbdep(
	conformation::Residue const & rsd,
	pose::Pose const & pose,
	RotamerLibraryScratchSpace & scratch
) const
{
	...
	parent::eval_rotameric_deriv( rsd, pose, scratch ); //This has been updated to allow scoring of D-amino acids.

	Real dnrchiscore_dchi;
	utility::fixedsizearray1< Real, N > dnrchiscore_dbb;
	Size const packed_rotno = this->get_packed_rotno(rsd, pose);

	Real const nrchi_score = eval_nrchi_score( rsd, pose, scratch, packed_rotno, dnrchiscore_dchi, dnrchiscore_dbb );

	scratch.fa_dun_tot() = scratch.fa_dun_dev() + nrchi_score;
...
) const
{
	...
	utility::fixedsizearray1< Real, N > bbs = parent::get_IVs_from_rsd( rsd, pose );
	...
	for ( Size bbi = 1; bbi <= N; ++bbi ) {
		debug_assert( bb_bin[ bbi ] >= 1 && bb_bin[ bbi ] <= parent::N_BB_BINS[bbi] );
	}
	...
	polycubic_interpolation( n_derivs, dbbp, binwbb, interpolated_energy, dvaldbb );
...
) const
{
	...
	if ( curr_rotamer_only ) {
		...
		Size const packed_rotno = this->get_packed_rotno( rsd, pose );
		nrchi_score = bbdep_nrchi_score( rsd, pose, packed_rotno, dnrchiscore_dchi, dnrchiscore_dbb );
		...
		for ( Size jj = 0; jj <= bbdep_nrchi_nbins_; ++jj ) {
			...
			Size const packed_rotno_copy = this->get_packed_rotno( rsd_copy, pose );
			Real tmp_nrchi_score = bbdep_nrchi_score( rsd_copy, pose, packed_rotno_copy, dnrchiscore_dchi, dnrchiscore_dbb );
			if ( tmp_nrchi_score < nrchi_score ) nrchi_score = tmp_nrchi_score;
		}
	} else {

		// AMW
		utility::fixedsizearray1< Real, N > const bbs2( parent::get_IVs_from_rsd( rsd, pose ) );
		...
		for ( Size jj = 1; jj <= rotamer_samples.size(); ++jj ) {
			...
			for ( Size kk = 0; kk <= bbdep_nrchi_nbins_; ++kk ) {
				...
				rsd_copy.chi( rsd_chi );
				Size const packed_rotno_copy = this->get_packed_rotno( rsd_copy, pose );
				tmp_nrchi_score = bbdep_nrchi_score( rsd_copy, pose, packed_rotno_copy, dnrchiscore_dchi, dnrchiscore_dbb );
				if ( tmp_nrchi_score < nrchi_score ) nrchi_score = tmp_nrchi_score;
			}
		}
	}
...
{
	assign_random_rotamer_with_bias_bbdep( rsd, pose, RG, new_chi_angles, perturb_from_rotamer_center );
}
...
) const
{
	...
	while ( random_prob > 0 ) {
		...
		Size index00 = make_index< N >( parent::N_BB_BINS, bb_bin );
...
) const
{
	...
	core::Real const d_multiplier( existing_residue.type().is_mirrored_type() ? -1.0 : 1.0 );

	/// Save backbone interpolation data for reuse
	utility::fixedsizearray1< Real, N > bbs = parent::get_IVs_from_rsd( existing_residue, pose );
	for ( Size bbi = 1; bbi <= N; ++bbi ) bbs[ bbi ] *= d_multiplier;
	utility::fixedsizearray1< Size, N > bb_bin, bb_bin_next;
	utility::fixedsizearray1< Real, N > bb_alpha;
	parent::get_bb_bins( bbs, bb_bin, bb_bin_next, bb_alpha );
	...
	while ( accumulated_probability < requisit_probability ) {
		...
		for ( Size indi = 2; indi <= ( 1 << N ); ++indi ) {
			Size index = make_conditional_index< N >( parent::N_BB_BINS, indi, bb_bin_next, bb_bin );
...
) const
{
	...
	utility::fixedsizearray1< Real, N > binw( parent::BB_BINRANGE );
...
SemiRotamericSingleResidueDunbrackLibrary< T, N >::write_to_binary( utility::io::ozstream & out ) const
{
	...
	Size const n_bbdep_scores = grandparent::n_packed_rots() * product( parent::N_BB_BINS ) * bbdep_nrchi_nbins_;
	...
	for ( Size ii = 1; ii <= grandparent::n_packed_rots(); ++ii ) {
		for ( Size jj = 1; jj <= bbdep_nrchi_nbins_; ++jj ) {
			...
			for ( Size i = 1; i <= N; ++i ) bb_bin_maxes = parent::N_BB_BINS[i];
			Size p = 1;
			while ( bb_bin[ N + 1 ] == 1 ) {

				bbdep_nrc_interpdata[ count ] = bbdep_nrc_interpdata_[ ii ]( make_index< N >( parent::N_BB_BINS, bb_bin ), jj );
				++count;
	...
	Size const n_bbdep_nrchi_samples =
		grandparent::n_packed_rots() * product( parent::N_BB_BINS ) * n_nrchi_sample_bins_;
	...
	for ( Size ii = 1; ii <= n_nrchi_sample_bins_; ++ii ) {
		for ( Size jj = 1; jj <= grandparent::n_packed_rots(); ++jj ) {
			...
			while ( bb_bin[ N + 1 ] == 1 ) {
				...
				Size llkk = make_index< N >( parent::N_BB_BINS, bb_bin );
...
SemiRotamericSingleResidueDunbrackLibrary< T, N >::read_from_binary( utility::io::izstream & in )
{
	...
	Size const num_rot_bin = product( parent::N_BB_BINS );
	...
	for ( Size ii = 1, count = 0, count_iijj = 1; ii <= n_nrchi_sample_bins_; ++ii ) {
		for ( Size jj = 1; jj <= grandparent::n_packed_rots(); ++jj ) {
			...
			while ( bb_bin[ N + 1 ] == 1 ) {
				...
				Size rot_index = make_index< N >( parent::N_BB_BINS, bb_bin );
...
SemiRotamericSingleResidueDunbrackLibrary< T, N >::operator ==( rotamers::SingleResidueRotamerLibrary const & rhs) const {
	static basic::Tracer TR( "core.pack.dunbrack.SemiRotamericSingleResidueDunbrackLibrary" );
	...
	for ( Size ii = 1; ii <= grandparent::n_packed_rots(); ++ii ) {
		for ( Size jj = 1; jj <= bbdep_nrchi_nbins_; ++jj ) {
			...
			Size p = 1;
			while ( bb_bin[ N + 1 ] == 1 ) {
				Size bb_rot_index = make_index< N >( parent::N_BB_BINS, bb_bin );
...
)
{
	...
	Size const num_bb_bins = product( parent::N_BB_BINS );
	...
	while ( in_rotameric ) {
		...
		// calc index
		Size ind = make_index< N >( parent::N_BB_BINS, bb_bin );
...
{
	Size const n_bin_pow = product( parent::N_BB_BINS );
	...
	for ( Size ii = 1; ii <= grandparent::n_packed_rots(); ++ii ) {
		...
		for ( Size i = 1; i <= N; ++i ) dimensions[ i ] = parent::N_BB_BINS[i];
		...
		while ( bb_bin[ N + 1 ] == 1 ) {
			...
			Size bbdepindex = make_index< N >( parent::N_BB_BINS, bb_bin );
...
)
{
	...
	Size num_bins = product( parent::N_BB_BINS );
