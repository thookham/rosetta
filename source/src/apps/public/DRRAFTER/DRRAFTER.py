...
#def DRRAFTER_util.pdbslice( pdbfile, subset_or_excise, segment_residues, prefix ):
#
...
def parse_fasta( fasta ):
	...
	for line in open( fasta ):
		if line.startswith('>'):
			...
			resnums, chains = DRRAFTER_util.parse_tag( tag )
			for c in chains:
...
def get_near_residues( struct, resnums, chains, dist_cutoff ):
	( coords, pdb_lines, sequence, chains2, residues2 ) = DRRAFTER_util.read_pdb( struct )
...
def setup_job( args ):
	...
	if len( full_secstruct ) != len( full_seq ):
		...
		print( "Sequence:            ", "".join(full_seq) )
		print( "Secondary structure: ", "".join(full_secstruct) )
		exit( 1 )
	...
	#		find_pairs_dict( full_secstruct_str, ['{','}'] ).items() )
	pairs1 = find_pairs_dict( full_secstruct_str )
	pairs2 = find_pairs_dict( full_secstruct_str, ['[',']'] )
	pairs3 = find_pairs_dict( full_secstruct_str, ['{','}'] )
	pairs_dict = dict( pairs1 )
	pairs_dict.update( pairs2 )
	pairs_dict.update( pairs3 )
	...
	##########################
	remodel_resnums, remodel_chains = DRRAFTER_util.parse_tag( args.residues_to_model )
	...
	for pdb in args.include_as_rigid_body_structures:
		...
		( sequences, chains_sep, resnums_sep) = DRRAFTER_util.get_sequences( pdb, removechain=False )
	...
	all_include_residues_around_resnums, all_include_residues_around_chains = DRRAFTER_util.parse_tag( args.include_residues_around )
	# and then add all the remodel residues that are in the start_struct
	start_struct_seq, start_struct_chains_sep, start_struct_resnums_sep = DRRAFTER_util.get_sequences( args.start_struct, removechain=False )
	start_struct_chains = []
	...
	if len( near_resnums_for_subset_pdb ) > 0: 
		tag_for_fixed_struct = DRRAFTER_util.make_tag_with_dashes( near_resnums_for_subset_pdb, near_chains_for_subset_pdb )
		DRRAFTER_util.pdbslice( args.start_struct, "subset", tag_for_fixed_struct, 'fixed_struct_%s_' %(args.job_name) )
	...
	subset_to_include_tag = DRRAFTER_util.make_tag_with_dashes( residues_to_include_resnums, residues_to_include_chains )
	...
	if not args.no_initial_structures:
		DRRAFTER_util.pdbslice( args.start_struct, "subset", subset_to_include_tag, 'init_struct_%s_' %(args.job_name))
	...
	if not args.no_csts:
		...
		if args.ref_pdb_for_coord_csts:
			...
			DRRAFTER_util.pdbslice( args.ref_pdb_for_coord_csts, "subset", subset_to_include_tag, 'ref_pdb_custom_%s_' %(args.job_name) )
			...
			ref_pdb_tag = DRRAFTER_util.make_tag_with_dashes( ref_pdb_resnums, ref_pdb_chains )
			DRRAFTER_util.pdbslice( args.start_struct, "subset", ref_pdb_tag, 'ref_pdb_%s_' %(args.job_name) )
...
def get_n_cycles( remodel_residues ):
	resnums, chains = DRRAFTER_util.parse_tag( remodel_residues ) 
...
def estimate_error( args ):
	...
	for s in args.final_structures:
		struct_str += s
		struct_str += ' '
	( sequences, chains, resnums) = DRRAFTER_util.get_sequences( args.final_structures[0], removechain=False )
