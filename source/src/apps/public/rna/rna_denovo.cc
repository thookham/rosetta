...
rna_denovo()
{
	...
	if ( option[ OptionKeys::in::file::s ].user() ) the_mover->set_input_pdbs( option[ OptionKeys::in::file::s ].value() );
	if ( option[ OptionKeys::in::file::silent ].user() ) the_mover->set_input_silent_files( option[ OptionKeys::in::file::silent ].value() );
	if ( option[ OptionKeys::rna::denovo::sequence ].user() ) the_mover->set_sequence_strings( option[ OptionKeys::rna::denovo::sequence ].value() );
	if ( option[ OptionKeys::rna::denovo::minimize_rna ].user() ) {
		the_mover->set_minimize_rna( option[ OptionKeys::rna::denovo::minimize_rna ].value() );
	}
	if ( option[ OptionKeys::rna::denovo::helical_substructs ].user() ) the_mover->set_helical_substructs( option[ OptionKeys::rna::denovo::helical_substructs ].value() );
	if ( option[ OptionKeys::rna::denovo::dock_chunks ].user() ) the_mover->set_dock_chunks( option[ OptionKeys::rna::denovo::dock_chunks ].value() );
	if ( option[ OptionKeys::rna::denovo::initial_structures ].user() ) the_mover->set_input_initialization_pdbs( option[ OptionKeys::rna::denovo::initial_structures ].value() );

	protocol_options->initialize_from_command_line();

	the_mover->set_residue_type_set( core::chemical::ChemicalManager::get_instance()->residue_type_set( core::chemical::FA_STANDARD ) );
	the_mover->de_novo_setup_from_command_line();
	auto ji = utility::pointer::make_shared< protocols::jd2::EmptyPoseJobInputter >();
	auto jo = utility::pointer::make_shared< protocols::jd2::RNA_DeNovoJobOutputter >();
	protocols::jd2::JobDistributor::get_instance()->go( the_mover, ji, jo );
...
main( int argc, char * argv [] )
{
	try {
		...
		protocols::rna::denovo::RNA_DeNovoProtocol::register_options();
