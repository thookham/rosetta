...
create_rna_vall_torsions_test( ){
	...
	std::string outfile  = option[ basic::options::OptionKeys::rna::denovo::vall_torsions ]();
	...
	for ( Size n = 1; n <= infiles.size(); n++ ) {
		...
		protocols::rna::denovo::create_rna_vall_torsions( pose, torsions_out, exclude_res_list );
	...
	std::string const & intag )
{
	...
	protocols::rna::denovo::figure_out_base_pair_partner( pose, partner, false );
	...
	std::string const & intag )
{
	...
	if ( basic::options::option[ use_lores_base_pair_classification ]() ) {
		base_pair_list= protocols::rna::denovo::classify_base_pairs_lores( pose );
	} else {
...
{
	using namespace basic::options;
	using namespace basic::options::OptionKeys::rna::denovo;
	using namespace basic::options::OptionKeys::rna::denovo::db;
...
main( int argc, char * argv [] )
{
	try {
		...
		option.add_relevant( basic::options::OptionKeys::rna::denovo::db::jump_database );
		option.add_relevant( basic::options::OptionKeys::rna::denovo::db::bps_database );
