...
main( int argc, char * argv [] ) {
	...
	try {
		...
		if ( !option[legacy_sewing::mode].user() ) {
		...
		if ( !option[legacy_sewing::model_file_name].user() ) {
			std::stringstream err;
		...
		if ( option[legacy_sewing::mode].value() == "generate" ) {
			...
			bool hash_tag_only_terminal_Es = option[legacy_sewing::hash_tag_only_terminal_Es].def(false);
			...
			if ( option[ in::file::l ].user() ) {
				...
				if ( !option[ legacy_sewing::assembly_type ].user() ) {
					std::stringstream err;
			...
			std::exit(0);
		} else if ( option[legacy_sewing::mode].value() == "generate_five_ss_model" ) {
		...
		if ( option[legacy_sewing::mode].value() == "test" ) {
			std::string binary_filename = option[legacy_sewing::score_file_name].value();
		...
		if ( ( option[legacy_sewing::mode].value() == "convert" ) ) {
			...
			std::string binary_filename = utility::to_string(option[legacy_sewing::score_file_name].value())+utility::to_string(".bin");
		...
		if ( ( option[legacy_sewing::mode].value() == "hash" ) ) {
			...
			core::Size min_hash_score = option[legacy_sewing::min_hash_score].def(10);
			core::Size max_clash_score = option[legacy_sewing::max_clash_score].def(0);
			core::Size num_segments_to_match = option[legacy_sewing::num_segments_to_match].def(1);
			core::Size box_length = option[legacy_sewing::box_length].def(3);
			...
			*/
			...
			std::string hash_target = option[legacy_sewing::mode].value();
	    	...
	    	core::Size n_models;
	    	if ( option[legacy_sewing::max_models].user() ) {
	    		n_models = std::min( (core::Size)option[legacy_sewing::max_models], models.size() );
	    	} else {
	    		n_models = models.size();
	    	}

	    	int starting_model = 0;
	    	if ( option[legacy_sewing::starting_model].user() ) {
	    		starting_model = option[legacy_sewing::starting_model].value();
	    	}
