		...
		do {
			...
			if (defined $line && $line !~/^No/  && $line !~/^Done/) {
				...
				if ($targetline =~ /^Q.* (\d+) (\S+) /) {
					...
					die "Error parsing target line at $targetline\n";
				}
				<HHR>;
				<HHR>;
				<HHR>;
				...
				if ($templline =~ /^T.* (\d+) (\S+) /) {
					...
					die "Error parsing template line at $templline\n";
				}
				<HHR>;
				<HHR>;
			}
...
foreach my $i (0..$nseqs-1) {
	...
	### strip PDB
	################
	open (PDBOUT, ">$pdbid"."_mr.pdb") || die "Cannot open $hhrfile.";
	my $pdbptr = $pdbstart;
	...
	open (ALIOUT, ">$pdbid".".ali") || die "Cannot open $hhrfile.";
	print ALIOUT "## 1XXX_ $pdbid"."_mr.pdb\n";
