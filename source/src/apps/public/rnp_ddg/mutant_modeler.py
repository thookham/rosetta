...
from util import get_surrounding_res
...
class MutantModeler:
	...
	def __init__( self, method, wt_struct ):
		...
		self.wt_RNA_secstruct = ''
		for x in range(len(self.wt_RNA_seq)):
			self.wt_RNA_secstruct += '.'
		self.mutant_index = 0
		self.cmd_opts = ''
		self.sfxn = 'rnp_ddg'
	...
	def get_low_res_command_lines( self, index ):
		...
		# Now write the rnp_ddg command line
		cmd = ''
		cmd += '%srnp_ddg -ignore_zero_occupancy false ' %(self.rosetta_prefix)
	...
	def get_med_res_command_lines( self, index ):
		...
		if len(other_mut_pos) > 0 : # only if there are other mutations
			...
			# First rnp_ddg run to make the initial mutations
			setup_cmd += '%srnp_ddg -ignore_zero_occupancy false ' %(self.rosetta_prefix)
			...
			# Don't need to run rnp_ddg first
		...
		with open('README_FARFAR','w') as readme_farfar:
			readme_farfar.write('%srna_denovo -nstruct 200 -fasta rna_seq.fasta -secstruct "%s" ' %(self.rosetta_prefix, new_ss_str))
			readme_farfar.write('-fixed_stems -chemical:enlarge_H_lj -s scaff_RNA_bound_conf.pdb ')
			readme_farfar.write('-score:weights %s -minimize_rna true ' %(self.sfxn))
			readme_farfar.write('-out:file:silent RNA_seq.out -include_neighbor_base_stacks ')
			readme_farfar.write('-restore_talaris_behavior\n')
		...
		# Write a file that lists the excised residues to be read later by rnp_ddg
		...
		final_cmd += '%scombine_silent -s complex.out.*.pdb -out:file:silent_struct_type binary ' %(self.rosetta_prefix)
		final_cmd += '-out:file:silent top20_complexes.out\n'

		# Now do the final rnp_ddg run to get the complex scores
		final_cmd += '%srnp_ddg -ignore_zero_occupancy false ' %(self.rosetta_prefix)
		...
		if wt_index == -1: # hasn't already been built
			...
			# just write the README_FARFAR command
			with open('README_FARFAR','w') as readme_farfar:
				readme_farfar.write('%srna_denovo -nstruct 200 -fasta rna_seq.fasta -secstruct "%s" '%(self.rosetta_prefix, new_ss_str))
				readme_farfar.write('-fixed_stems -chemical:enlarge_H_lj -s scaff_RNA_bound_conf.pdb ')
				readme_farfar.write('-score:weights %s -minimize_rna true ' %(self.sfxn))
				...
				readme_farfar.write('-restore_talaris_behavior\n')
			if self.run_local:
				setup_cmd += 'cd %s/wt_%d\n' %(base_dir,len(self.med_res_WT_rebuild))
				setup_cmd += 'source README_FARFAR\n'
	...
	def make_high_res_subsets_and_cmd_lines( self, index, mut_subsets, mutant_seq_full, native_pdb, mut_depends ):
		...
		for subset_perm in permutations(range(1,21),len(mut_subsets)+len(mut_depends)):
			...
			final_cmd += 'python combine_pdbs.py -f complex_start_struct.pdb -s '
