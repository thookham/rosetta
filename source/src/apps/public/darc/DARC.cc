...
int main( int argc, char * argv [] ) {
	...
	try {
		...
		NEW_OPT( missing_point_weight, "missing point weight", 5.48 );
		NEW_OPT( extra_point_weight, "extra point weight", 5.47 );
		NEW_OPT( steric_weight, "steric weight for PSO", 0.61 );
		...
		//register calculator outside docking loop
		std::string sasa_calc_name = "darcsasa";
		...
		for ( core::Size lig_num = 1; lig_num <= ligand_poses.size(); ++lig_num ) {
			...
			if ( option[ darc_score_only ]() ) {
				...
				for ( Size ii = 1; ii <= small_mol_pose.size(); ++ii ) {
					...
					conf_pf.move_ligand_and_update_rhos_( npf, tmp_offset, 0.0, 0.0, 0.0, confnum );
					if ( option[ OptionKeys::fingerprint::darc_shape_only ].user() ) {
						unaligned_darc_score = conf_pf.fp_compare( npf, missing_pt_wt, steric_wt, extra_pt_wt );
		...
		npf.gpu_setup( missing_pt_wt, steric_wt, extra_pt_wt, particle_size, esp_wt);
#endif
...
#ifdef USEOPENCL
			npf.gpu_setup_atomcoords(pf, particle_size);
#endif
				...
				//End measuring time
				duration = ( std::clock() - start ) / (core::Real) CLOCKS_PER_SEC;
				std::cout<<"Time for DARC runs: "<< duration <<std::endl;
