...
run_tests()
{
	...
	Pose pose;
	core::import_pose::pose_from_file( "input/1aay.pdb" , core::import_pose::PDB_file);
...
)
{
	...
	HBondSet const & hbond_set( static_cast< HBondSet const & >( pose.energies().data().get( basic::HBOND_SET )));
...
water_test()
{
	...
	// try packing waters
	{
		...
		pose.data().set( basic::WATER_PACKING_INFO, water_info );
...
not1_test()
{
	...
	{ // here's an example of packing with a resfile
...

		{
			...
			HBondDatabase hbond_database( hbonds::HBondDatabase::get_database( hbond_set.hbond_options()->params_database_tag()));
...
zf_test()
{
	...
	Size const ref_motif_begin_protein( ref_pose_map.find( 'A', 146 ) ),
		...
		/*ref_motif_begin_dna( ref_pose_map.find( 'B', 5 ) ),*/
		motif_size( 3 );
...
dna_chain_check()
{
	...
	methods::EnergyMethodOptions options( scorefxn->energy_method_options() );
	options.exclude_DNA_DNA( false );
	options.set_etable( FA_STANDARD_SOFT );
	scorefxn->set_energy_method_options( options );
		...
		for ( Size i=1; i<= nres; ++i ) {
			...
			if ( j ) {
				std::cout << "PARTNER " << file <<
					...
					" i,p[i](pdb): " <<
					I(6,pose.pdb_resid( i ).first) << ' ' << pose.pdb_resid( i ).second <<
					I(6,pose.pdb_resid( j ).first) << ' ' << pose.pdb_resid( j ).second <<
			...
			for ( Size j=i+1; j<= nres; ++j ) {
				...
				if ( emap[ fa_rep ] > clash_threshold ) {
					std::cout << "DNA_CLASH: " << F(12,3,emap[fa_rep]) << ' ' << rsd1.name1() << ' ' << rsd2.name1() <<
						...
						" i,j(pdb): " <<
						I(6,pose.pdb_resid( i ).first) << ' ' << pose.pdb_resid( i ).second <<
						I(6,pose.pdb_resid( j ).first) << ' ' << pose.pdb_resid( j ).second << std::endl;
				}
			}
		}
	}
...
not1_loop_test()
{
	...
	for ( int i=1; i<= option[ OK::out::nstruct ]; ++i ) {
		...
		pose = start_pose;
		refine_loops_with_ccd( pose, loops, &scorefxn, 0, design_loop );
