...
public:
	...
	find_zinc(core::pose::Pose & pose) {
		...
		core::pose::Pose match_pose( pose );
		match_pose_ = match_pose;
	...
	constrain_zinc(core::pose::Pose & pose) {
		...
		//Identify the His that has a backbone backside hbond.  We want to constrain this hbond.

		Size backsideO_His = match_pose_.pdb_info()->number( 1 /*first His has backside O*/ );
		Size backsideO_res = match_pose_.pdb_info()->number( 2 /*second res is backside hbond partner O*/ );
		backsideO_His_ = backsideO_His;

		point zinc_xyz = msr_[1]->get_ligand_atom_xyz();
		point NE2_xyz = match_pose_.residue(1).atom("NE2").xyz();
		point ND1_xyz = match_pose_.residue(1).atom("ND1").xyz();

		Real NE2_dist = NE2_xyz.distance(zinc_xyz);
		Real ND1_dist = ND1_xyz.distance(zinc_xyz);

		id::AtomID backsideO_His_nitrogen_id;
		if ( NE2_dist < ND1_dist ) {
			backsideO_His_nitrogen_id = id::AtomID( match_pose_.residue(1).atom_index("NE2"), backsideO_His);
		} else {
			backsideO_His_nitrogen_id = id::AtomID( match_pose_.residue(1).atom_index("ND1"), backsideO_His);
		}

		id::AtomID backsideO_His_CE1_id( pose.residue(backsideO_His).atom_index("CE1"), backsideO_His );
		id::AtomID backsideO_His_hydrogen_id ( pose.residue(backsideO_His).atom_index("H"), backsideO_His );
		id::AtomID backsideO_oxygen_id( 4, backsideO_res );
		id::AtomID backsideO_carbon_id( 3, backsideO_res );

		TR << "Backside Hbond atom IDs" << std::endl;
		TR << "N  : " << backsideO_His_nitrogen_id << std::endl;
		TR << "CE1: " << backsideO_His_CE1_id << std::endl;
		TR << "H  : " << backsideO_His_hydrogen_id << std::endl;
		TR << "O  : " << backsideO_oxygen_id << std::endl;
		TR << "C  : " << backsideO_carbon_id << std::endl;

		TR << "N  : " << pose.residue(backsideO_His_nitrogen_id.rsd()).atom(backsideO_His_nitrogen_id.atomno()).xyz() << std::endl;
		TR << "CE1: " << pose.residue(backsideO_His_CE1_id.rsd()).atom(backsideO_His_CE1_id.atomno()).xyz() << std::endl;
		TR << "H  : " << pose.residue(backsideO_His_hydrogen_id.rsd()).atom(backsideO_His_hydrogen_id.atomno()).xyz() << std::endl;
		TR << "O  : " << pose.residue(backsideO_oxygen_id.rsd()).atom(backsideO_oxygen_id.atomno()).xyz() << std::endl;
		TR << "C  : " << pose.residue(backsideO_carbon_id.rsd()).atom(backsideO_carbon_id.atomno()).xyz() << std::endl;
		...
		FuncOP const NO_dist_func( new HarmonicFunc( 2.6, 0.2 ) );
		FuncOP const NHO_angle_func( new CircularHarmonicFunc( radians(180), radians(20) ) );
		FuncOP const HOC_angle_func( new HarmonicFunc( radians(120), radians(20) ) );
		FuncOP const H_dihedral_func( new CircularHarmonicFunc( radians(180), radians(20) ) );

		core::scoring::constraints::AtomPairConstraintCOP NO_dist_cst = new AtomPairConstraint( backsideO_His_nitrogen_id, backsideO_oxygen_id, NO_dist_func );
		core::scoring::constraints::AngleConstraintCOP NHO_angle_cst = new AngleConstraint( backsideO_His_nitrogen_id, backsideO_His_hydrogen_id, backsideO_oxygen_id, NHO_angle_func );
		core::scoring::constraints::AngleConstraintCOP HOC_angle_cst = new AngleConstraint( backsideO_His_hydrogen_id, backsideO_oxygen_id, backsideO_carbon_id, HOC_angle_func );
		core::scoring::constraints::DihedralConstraintCOP H_dihedral_cst = new DihedralConstraint( backsideO_His_CE1_id, backsideO_His_nitrogen_id, backsideO_oxygen_id, backsideO_carbon_id, H_dihedral_func );


		TR << "Adding hbond constraints" << std::endl;
		pose.add_constraint( NO_dist_cst );
		//pose.add_constraint( NHO_angle_cst );
		//pose.add_constraint( HOC_angle_cst );
		//pose.add_constraint( H_dihedral_cst );
		...
		mm->set_bb  ( false );
		mm->set_chi ( true ); //will only minimize one sidechain
		...
		TR << "Preminimizing sc" << std::endl;
		min_mover->apply(pose);

		//mm->set_bb(true);
		//min_mover->apply(pose);

		TR << "min completed" << std::endl;
		pose.dump_pdb("minimized_pose.pdb");
	...
	build_rotamers(core::pose::Pose & pose) {
		...
		for ( Size i(1); i <= primary_shell_neighbors_.size(); ++i ) {
			...
			TR << "Shell " << i << std::endl;
			if ( msr_[i+1]->get_seqpos() == backsideO_His_ ) { //don't need to find a backside hbond for the His that already has a backside backbone hbond
				TR << "SKIPPING " << backsideO_His_ << std::endl;
				continue;
			}
			...
			for ( Size j(1); j <= primary_shell_neighbors_[i].size()-1; ++j ) {
				...
				if ( this_res - 2 < 1 || this_res + 2 > pose.size() ) {
					continue;
				}
				...
				for ( Size polar_aa_index(1); polar_aa_index <= polar_aas.size(); ++polar_aa_index ) {
					...
					utility::graph::GraphOP dummy_png = core::pack::create_packer_graph( pose, dummy_sfxn, task );
	...
	hbond_constraint(core::pose::Pose & pose, Size hbond_res, Size aa_index, Size primary_shell_index, Size krot) {
		...
		FuncOP const hbond_angle_func( new CircularHarmonicFunc( radians(180), radians(20) ) );
		...
		if ( current_hbond_dist < 3.0 && current_hbond_angle > 120 ) {
			std::stringstream output_name;
			output_name << "out_" << hbond_res << "_" << aa_index << "_" << krot << "_" << pose.pdb_info()->name();
			pose.dump_pdb(output_name.str());
		}
