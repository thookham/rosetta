...
main( int argc, char * argv [] )
{
	...
	// add local options
 	option.add( peptoid_seq, "peptoid sequence" ).def("11");
...
// create score function
	...
	utility::vector1<std::string> aa_names( option[ peptoid_seq ].value() );
	for( Size i(1); i <= aa_names.size(); ++i ) {
		std::cout << aa_names[ i ] << std::endl;
	}
	...
	pose.dump_pdb( "peptoid_sequence.pdb" );
