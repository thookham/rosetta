...
main( int argc, char* argv[] )
{
	...
	**********************************************************************************************************************/
	...
	option.add( dddm::ia_ener, "Upper energy limit for final design/interface analysis checkpoint" ).def( 10000.0 ); //REALLY HIGH FOR PEPTOID AT FIRST
	...
	// create a cyclization mover
	CyclizationMoverOP cm( new CyclizationMover( 2 /* chain id */, true /* constraints */, false /* minimize */, option[num_min_rebuild].value() ) );
	...
	// add movers to sequence mover
	main_sequence->add_mover( cm );
	main_sequence->add_mover( D3DM );

	// create job distributor
	protocols::jd2::JobDistributor::get_instance()->go( main_sequence );
...
)
{
	...
	score_fxn->set_weight( core::scoring::angle_constraint, 1 );
	score_fxn->set_weight( core::scoring::dihedral_constraint, 1 );
	...
	*******************************************************************************/
	...
	protocols::minimization_packing::RotamerTrialsMoverOP pert_rt(new protocols::minimization_packing::EnergyCutRotamerTrialsMover( score_fxn, pert_tf, pert_mc, 0.1 /*energycut*/ ) );
	...
	**********************************************************/
	...
	//pert_random->add_mover( pert_pep_repeat, 1 ); // TURN OFF BB MOVES FOR PEPTOIDS
	...
	pert_sequence->add_mover( pmm ); // DEBUG DEBUG PEPTOID PYMOL MOVER
	...
	**********************************************************/
	...
	protocols::minimization_packing::MinMoverOP desn_min( new protocols::minimization_packing::MinMover( desn_mm, score_fxn, option[ OptionKeys::run::min_type ].value(), 0.01, true ) );
	...
	protocols::minimization_packing::TaskAwareMinMoverOP desn_ta_min = new protocols::minimization_packing::TaskAwareMinMover( desn_min, desn_tf );
	...
	**********************************************************************************************************************/
	...
	for ( Size k = 1; k <= Size( option[ dddm::inner_num ].value() ); ++k ) {
		...
		if ( k % 10 == 0 ) {
			...
			protocols::minimization_packing::PackRotamersMoverOP final_desn_pr( new protocols::minimization_packing::PackRotamersMover(score_fxn, final_desn_pt, 10 ) );
...
)
{
	...
	**********************************************************/
	...
	// DEBUG
	protocols::viewer::dump_pose_kinemage( "pose.kin", pose );
