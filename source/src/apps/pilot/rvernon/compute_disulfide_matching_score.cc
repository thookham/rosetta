...
main( int argc, char* argv [] )
{
	...
	try {
		...
		utility::io::ozstream outfile(pdb_file_list+".angle.data");
		...
		while ( infile.good() ) {
			...
			float mt_dist, mr_dist, mrt_dist;

			mt_dist = std::sqrt( distance_squared( this_rt.get_translation(), disulfides[1].get_translation() ));
			mr_dist = std::sqrt( distance_squared( this_rt.get_rotation().col(1), disulfides[1].get_rotation().col(1) ) +
			...
			for ( Size d = 2; d <= disulfides.size(); ++d ) {
				float t_dist, r_dist, rt_dist;

				t_dist = std::sqrt( distance_squared( this_rt.get_translation(), disulfides[d].get_translation() ));
				r_dist = std::sqrt( distance_squared( this_rt.get_rotation().col(1), disulfides[d].get_rotation().col(1) ) +
					distance_squared( this_rt.get_rotation().col(2), disulfides[d].get_rotation().col(2) ) +
					distance_squared( this_rt.get_rotation().col(3), disulfides[d].get_rotation().col(3) ) );
				rt_dist = core::kinematics::distance( this_rt, disulfides[d] );

				if ( t_dist <= mt_dist ) mt_dist = t_dist;
				if ( ( r_dist <= mr_dist ) && ( t_dist <= 2.5 ) ) mr_dist = r_dist;
				if ( rt_dist <= mrt_dist ) mrt_dist = rt_dist;

				//if ( t_dist <= 2.5) rt_dist = 5.5;

				std::cout << "HEYO " << disulfides[d] << " RT " << rt_dist << std::endl;
			}

			// COLUMNS:
			// DISULF PDB RES1 RES1 AA1 AA2 SS1 SS2 SEQ_SEP CB_DIS CEN_DIS CA1CB1CB2_angle1 CA2CB2CB1_angle2 CACBCBCA_dihedral NCACAC_dihedral
			...
			outfile << pdb.residue(res1).xyz("CEN").distance(pdb.residue(res2).xyz("CEN")) << " ";
