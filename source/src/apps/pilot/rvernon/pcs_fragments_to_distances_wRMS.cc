...
largest_cluster( utility::vector1< pose::Pose > lowscore_poses, utility::vector1< pose::Pose > pose_list, utility::vector1< Real > score_list, utility::vector1< Real > rms_list )//, pose::Pose native )
{
	...
	for ( Size jj = 1; jj <= pose_list.size(); ++jj ) {
		...
		if ( maxsub_coverage >= maxsub_filter ) {
			...
			std::cout << "CLUSTER_SCORE_RMS: " << maxsub_coverage << " " << score_list[jj] << " " << rms_list[jj] << std::endl;
		} else {
			std::cout << "EXCLUDED_SCORE_RMS: " << maxsub_coverage << " " << score_list[jj] << " " << rms_list[jj] << std::endl;
		}
	}
...
main( int argc, char* argv [] )
{
	...
	try {
		...
		while ( infile.good() ) {
			...
			utility::vector1< Real > allscore_list;
			utility::vector1< Real > allrms_list;
			...
			for ( core::io::silent::SilentFileData::iterator iter = sfd.begin(), end = sfd.end();
				...
				allscore_list.push_back( iter->get_energy("pcs") );
				allrms_list.push_back( iter->get_energy("rms") );
			}

			utility::vector1< Real > pcs_scores( allscore_list );
			std::sort( pcs_scores.begin(), pcs_scores.end() );
			...
			if ( basic::options::option[ robert::pcs_cluster_lowscoring ]() ) { //Default true
				...
				for ( Size pp=1; pp <= allpose_list.size(); ++pp ) {
					if ( allscore_list[pp] <= pcs_scores[index_20] ) {
						if ( lowscore_list.size() <= index_20 ) {
							lowscore_list.push_back( allpose_list[pp] );
						}
					}

					if ( allscore_list[pp] <= pcs_scores[index_median] ) {
						pose_list.push_back( allpose_list[pp] );
						score_list.push_back( allscore_list[pp] );
						rms_list.push_back( allrms_list[pp] );
					}
				}
			} else {
			...
			utility::vector1< pose::Pose > cluster_list( largest_cluster( lowscore_list, pose_list, score_list, rms_list) );//, fragnative ) );
