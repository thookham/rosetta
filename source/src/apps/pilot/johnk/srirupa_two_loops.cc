...
OPT_KEY( Integer, relax1_start_resnum )
OPT_KEY( Integer, relax1_final_resnum )
OPT_KEY( Integer, relax2_start_resnum )
OPT_KEY( Integer, relax2_final_resnum )

static basic::Tracer TR( "apps.pilot.srirupa_two_loops.main" );
...
main( int argc, char * argv [] )
{
	try {
		...
		// Setup first region to backrub
		core::Size start_relax1_res = 0;
		...
		for ( int j = 1, resnum = input_pose.size(); j <= resnum; ++j ) {
			...
			if ( input_pose.pdb_info()->number(j) == relax1_final_pdb_number ) {
				end_relax1_res = j;
			}
		}
		if ( start_relax1_res == 0 ) {
		...
		// Setup second region to backrub
		core::Size start_relax2_res = 0;
		...
		for ( int j = 1, resnum = input_pose.size(); j <= resnum; ++j ) {
			...
			if ( input_pose.pdb_info()->number(j) == relax2_final_pdb_number ) {
				end_relax2_res = j;
			}
		}
		if ( start_relax2_res == 0 ) {
			std::cerr << "ERROR!! Could not find residue/chain" << std::endl;
			exit(1);
		}
		if ( end_relax2_res == 0 ) {
		...
		if ( ( start_relax1_res < start_relax2_res ) && ( end_relax1_res > start_relax2_res ) ) {
			std::cerr << "ERROR!! Flexible regions should not be overlapping" << std::endl;
			exit(1);
		}
		if ( ( start_relax2_res < start_relax1_res ) && ( end_relax2_res > start_relax1_res ) ) {
			std::cerr << "ERROR!! Flexible regions should not be overlapping" << std::endl;
			exit(1);
		}
		...
		for ( int traj = 1; traj <= ntraj; ++traj ) {
			...
			*relax_poseOP = input_pose;
			...
			for ( core::Size seg_start = start_relax1_res; seg_start < end_relax1_res; ++seg_start ) {
				for ( core::Size seg_end = seg_start + 2; seg_end <= end_relax1_res; ++seg_end ) {
			...
			for ( core::Size seg_start = start_relax2_res; seg_start < end_relax2_res; ++seg_start ) {
				for ( core::Size seg_end = seg_start + 2; seg_end <= end_relax2_res; ++seg_end ) {
					id::AtomID start_atom_id = id::AtomID( input_pose.residue( seg_start ).atom_index("CA") , seg_start );
					id::AtomID end_atom_id = id::AtomID( input_pose.residue( seg_end ).atom_index("CA"), seg_end );
					// add segment to the mover
					backrubmover.add_segment( start_atom_id, end_atom_id, 0 );
