...
main( int argc, char * argv [] )
{
	try {
		...
		for ( Size r = 1; r <= pArg_rotset->num_rotamers(); ++r ) {
			...
			if ( distance(pArg_P.xyz(),pTyr_P.xyz()) > match_dist_cut ) continue;
			...
			if ( option[ do_minimization ] ) {
				...
				core::scoring::constraints::FuncOP func( new scoring::constraints::HarmonicFunc( 0., force_constant ) );
				min_pose.add_constraint( new core::scoring::constraints::CoordinateConstraint( P_atom_id, fixed_atom_id, pTyr_P.xyz(), func ) );
				...
				*/
				...
				if ( distance(min_pose.residue(pR_resnum).atom( "P" ).xyz(),pTyr_P.xyz()) > 0.75 ) continue;
				if ( N1_bonded_to_P ) {
				...
				min_pose.dump_scored_pdb( basic::options::start_file()+".min"+utility::to_string(model)+".pdb", *scorefxn );
