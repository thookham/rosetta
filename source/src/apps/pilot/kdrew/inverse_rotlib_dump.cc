...
InverseRotlibDumpMover::apply(core::pose::Pose& pose)
{
	...
	core::pose::add_variant_type_to_pose_residue( primary_hs_pose, "SHOVE_BB", primary_hs_pose.size());
	//kdrew: create starting pose by combining the target pose and the scaffold pose

	// the hotspot residue is the final one in the file, i.e. primary_hs_pose.residue(primary_hs_pose.size())

	core::conformation::ResidueCOP primary_hs_residue  =  primary_hs_pose.residue(primary_hs_pose.size()).get_self_ptr();
	...
	for ( core::Size iii = 1; iii <= ancillary_locations.size(); iii++ ) {
		...
		core::pose::add_variant_type_to_pose_residue( secondary_hs_pose, "SHOVE_BB", secondary_hs_pose.size());

		core::conformation::ResidueCOP secondary_hs_residue  =  &primary_hs_pose.residue(secondary_hs_pose.size());
