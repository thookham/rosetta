...
get_pair_keys_from_list(
	...
	while ( getline( stream, line ) ) {
		...
		core::Size pair_id = utility::string2int(tokens[2]);
		keys.push_back(std::make_pair(struct_id, pair_id));
	}
	return keys;
}
...
dump_pair_pose(
	...
	std::stringstream pdb_name;
	pdb_name << struct_id << "_" << pair_id << ".pdb";
	pose.dump_pdb(pdb_name.str());
}
...
main( int argc, char * argv [] )
{
	...
	option.add( BundleDumper::pair_list, "File with list of pairs to dump");
