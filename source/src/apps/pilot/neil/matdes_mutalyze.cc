...
minimize(Pose & pose, ScoreFunctionOP sf, utility::vector1<Size> design_pos, bool move_bb, bool move_sc, bool move_rb) {
	...
	protocols::minimization_packing::symmetry::SymMinMover m1( movemap, sf, "lbfgs_armijo_nonmonotone", 1e-5, true, false, false );
	m1.apply(pose);

	if ( move_rb ) {
		movemap->set_jump(true);
		core::pose::symmetry::make_symmetric_movemap( pose, *movemap );
		protocols::minimization_packing::symmetry::SymMinMover m2( movemap, sf, "lbfgs_armijo_nonmonotone", 1e-5, true, false, false );
		m2.apply(pose);
	}
}
...
average_degree (Pose const &pose, vector1<Size> mutalyze_pos, Size intra_subs, Real distance_threshold=10.0)
{
	...
	for ( Size i = 1; i <= mutalyze_pos.size(); ++i ) {
		...
		for ( Size jres = 1; jres <= (nres_monomer*intra_subs); ++jres ) {
			// for Nik's stuff    for(Size jres = (nres_monomer*intra_subs)+1; jres <= (nres_monomer*(intra_subs+1)); ++jres) {
