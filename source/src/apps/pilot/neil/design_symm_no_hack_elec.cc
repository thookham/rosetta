...
void
minimize(core::pose::Pose & pose, ScoreFunctionOP sf, utility::vector1<Size> design_pos, bool move_bb, bool move_sc, bool move_rb) {
...
sidechain_sasa(core::pose::Pose const & pose, Real probe_radius) {
	using core::id::AtomID;
...
void
get_sc(core::pose::Pose &pose, Real& int_area, Real& sc, std::string sctag) {
	...
	for ( Size ifile = 1; ifile <= files.size(); ++ifile ) {
		...
		for ( Size iangle = 1; iangle <= grid_nsamp_angle; iangle++ ) {
			...
			for ( Size iradius = 1; iradius <= grid_nsamp_radius; iradius++ ) {
				...
				TR << "contact_dist_sq = " << contact_dist_sq << std::endl;
				...
				// Minimize
				//minimize(pose, sf, design_pos, false, true, true);
