...
get_movemap( bool const nonideal )
{
  ...
  mm->set_jump( true );
  if( nonideal ){
    mm->set( core::id::D, false );
    mm->set( core::id::THETA, false );
  }
  return mm;
}
		    ...
		    utility::vector1< pose::Pose > const &poses )
{
  ...
  for( Size i_pose = 1; i_pose <= poses.size(); ++i_pose ){
    ...
    Real gdtha = scoring::gdtha   ( native_pose, pose_work, resmap );
    Real gdtmm = scoring::CA_gdtmm( native_pose, pose_work, resmap );
    Real rmsd  = scoring::CA_rmsd ( native_pose, pose_work, resmap );

    Real irmsd = scoring::CA_rmsd( pose0, pose_work );
    
    printf( "%4d %8.3f %8.3f %8.4f %8.4f\n", irmsd, rmsd, gdtmm, gdtha );
