...
get_options(
	...
	runtime_assert_string_msg( option[rama_file].user(), "Error in symmetrize_3d_ramamap application: an input file must be specified with the \"-rama_file <filename>\" commandline option." );
...
symmetrize_map(
	...
	runtime_assert_string_msg( instream.good(), "Error in symmetrize_3d_ramamap application: could not open " + input_filename + " for read." );
	...
	do {
		...
		std::string aa_name;
		signed long int phi_bin, theta_bin, psi_bin;
		core::Real pval, eval;

		linestream >> aa_name >> phi_bin >> theta_bin >> psi_bin >> pval >> eval;
		runtime_assert_string_msg( !linestream.fail(), "Error in symmetrize_3d_ramamap application: could not parse line \"" + std::string(line) + "\"." );

		//Ensure that we're in the range [0, 360]:
		phi_bin = numeric::nonnegative_principal_angle_degrees(phi_bin);
		theta_bin = numeric::nonnegative_principal_angle_degrees(theta_bin);
		psi_bin = numeric::nonnegative_principal_angle_degrees(psi_bin);

		if ( phi_bin < 180 ) continue; //Positive phi region should be ignored.

		char outtemp[2048];
		snprintf( outtemp, sizeof( outtemp), "%s\t%li\t%li\t%li\t%0.8f\t%0.8f", aa_name.c_str(), phi_bin, theta_bin, psi_bin, pval, eval );
		...
		core::Size const thetaprime( static_cast< core::Size >( numeric::nonnegative_principal_angle_degrees( 360 - static_cast<signed long>(theta_bin) - 10 ) ) );
		...
		snprintf( outtemp, sizeof( outtemp), "%s\t%lu\t%lu\t%lu\t%0.8f\t%0.8f", aa_name.c_str(), phiprime, thetaprime, psiprime, pval, eval );
		outstream << outtemp << std::endl;
...
main( int argc, char * argv [] )
{
	...
	try {
		...
		if ( TR.visible() ) {
			TR << "Starting symmetrize_3d_ramamap.cc" << std::endl;
			TR << "Pilot app created 14 January 2019 by Vikram K. Mulligan (vmullig@uw.edu), Baker laboratory." << std::endl;
		}
