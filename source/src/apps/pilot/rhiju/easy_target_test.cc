																...
																)
{
	...
	for (Size i = 1; i <= desired_sequence.size(); i++ ){
		...
		ResidueTypeCOP new_rsd_type( ResidueTypeSelector().set_name1( new_seq ).exclude_variants().select( rsd_set )[1] );
	...
	for (Size n = 1; n <= loops.size(); n++ ) {
		...
		for (Size i = loops[n].first; i <= loops[n].second; i++ ) {
			ResidueTypeCOP new_rsd_type( ResidueTypeSelector().set_name1( desired_sequence[i-1] ).exclude_variants().select( rsd_set )[1] );
