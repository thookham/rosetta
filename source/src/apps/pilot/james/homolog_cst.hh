...
void create_starting_template (
	...
	while( getline(data,line) ) { // data line
		...
		// std::cout << "resi = " << resi << " resj = " << resj << " dist = " << dist << std::endl;
	...
	std::cout << "added " << cst_list.size() << " constraints." << std::endl;
	...
	for ( Size i = 1; i <= pose.size(); ++i ) {
		...
		if ( template_pos != 0 ) {
			// std::cout << "looking at (" << i << "," << template_pos << ")" << std::endl;
			...
			// 				// 					<< template_pose->residue(template_pos).name1() << ")" << std::endl;
