...
main( int argc, char * argv [] ) {
	try {
		...
		scorefxn->show(pose);
		core::optimization::MinimizerOptionsOP min_options( new core::optimization::MinimizerOptions( "lbfgs_armijo_nonmonotone", 1e-2, true ) );
		...
		if ( !option[ james::debug ]() ) {
			...
			for ( Size ii = 1; ii <= 100; ++ii ) {
				protocols::minimization_packing::SaneMinMover min_mover( mm, scorefxn, min_options );
