...
## @brief  Create an alpha-3-beta HBS using pyrosetta
...
def delete_extra_residues(pose, hbs_chain, hbs_length):
    final_res = Options.hbs_final_res
    
    for ii in xrange(pose.size()):
        i = ii + 1
        if pose.pdb_info().chain(i) != hbs_chain: continue
        ...
        elif pdb_res_num > final_res + hbs_length:
	    #TR << "deleting residue " << pdb_res_num << std::endl;
            while chn == hbs_chain and i <= pose.size():
...
if __name__ == '__main__':
    ...
    #
    # apply function to input pose with args.
