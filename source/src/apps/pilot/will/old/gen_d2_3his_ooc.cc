...
Real ik_his_clamp(Pose & pose,
	...
	Vec com(0.0,0.0,0.0); for ( Size i = 1; i <= tmp1.size(); ++i ) com += tmp1.xyz(AtomID(2,i));
	com /= Real(tmp1.size());
...
void run(std::string fname) {
	...
	vector1<vector1<bool> > rotc1(natp.size()),rotc2(natp.size());
	for ( Size ir = 1; ir <= natp.size(); ++ir ) {
