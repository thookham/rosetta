...
struct ConstraintConfig {
	...
	void show_cst_grids(std::ostream & out = TR) const {
		...
		for ( Size i = 1; i <= templates_fname.size(); ++i ) {
			...
			out << endl << "          ";
			for ( Size it = 1; it <= (Size)X; ++it ) out << " " << templates_fa[i]->residue(it).name1();
			out << endl;
			for ( Size id = 1; id <= (Size)Y; ++id ) {
				...
				out << lss(id,3) << " " << ss[id] << " " ;
				// char c = '.';
				for ( Size it = 1; it <= (Size)X; ++it ) out << " " << grid(it,id);
				out << endl;
			}
		}
	}
...
struct HubDenovo {
	...
	void min_as_poly_ala(Pose & p, ScoreFunctionOP sf) {
		...
		movemap->set_bb(1,false);
		movemap->set_chi(true);
		protocols::minimization_packing::symmetry::SymMinMover( movemap, sf, "lbfgs_armijo_nonmonotone", 1e-5, true, false, false ).apply(p);
		sf->set_weight(core::scoring::atom_pair_constraint,1.0);
		sf->set_weight(core::scoring::    angle_constraint,1.0);
	}
