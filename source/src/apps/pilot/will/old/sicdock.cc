...
dock(
	...
	protocols::sic_dock::RigidScoreCOP cbscore_( utility::pointer::make_shared< protocols::sic_dock::CBScore >(init_pose,init_pose,option[sicdock::clash_dis](),option[sicdock::contact_dis]()) );
	// rigidsfxn->add_score(cbscore_,1.0);

	if ( option[basic::options::OptionKeys::sicdock::trisbpy].user() ) {
		bpy3_sc_ = utility::pointer::make_shared< protocols::sic_dock::scores::TrisBpyScore >(init_pose,option[sicdock::trisbpy](),option[sicdock::trisbpy_min_contacts]());
		rigidsfxn->add_score(bpy3_sc_,1.0);
	}

	if ( option[basic::options::OptionKeys::mh::xform_score_data].user() ) {
		mhscore_ = utility::pointer::make_shared< protocols::sic_dock::scores::MotifHashRigidScore >(init_pose,init_pose);
	...
	Size nout=0,i=0;
	cout << "  file                       nf    iori    dang   order   rscore"; if ( NAT ) cout << "     rms";
	cout << "   rd1   rd2";
	...
	while ( nout < (Size)option[sicdock::nscore]() && ++i <= hits.size() ) {
		...
		if ( nout < nstruct ) {
			if ( mhscore_ ) {
				...
				clash_checker = utility::pointer::make_shared< xyzStripeHashPose >(olig,NCO,2.5);
