...
Real ik_his_clamp(Pose & pose, Size rsd1, Size rsd2, ImplicitFastClashCheck & ifc, vector1<HitOP> & , vector1<core::pack::rotamer_set::RotamerSetOP> const & rotsets) {
	...
	Vec com(0.0,0.0,0.0); for ( Size i = 1; i <= pose.size(); ++i ) com += pose.xyz(AtomID(2,i));
	com /= Real(pose.size());
		...
		for ( Size blen2 = 20; blen2 < 24; ++blen2 ) {
			...
			for ( Size bang = 97; bang < 122; bang+=3 ) {
				...
				for ( int isol = 1; isol <= nsol; isol++ ) {
					...
					//#endif
					for ( Size rsd3 = 1; rsd3 <= pose.size(); ++rsd3 ) {
						...
						for ( Size imer = 1; imer < 2; ++imer ) {
							for ( Size hrot = 1; hrot <= rotsets[rsd3]->num_rotamers(); ++hrot ) {
								...
								for ( Size rsd4 = 1; rsd4 <= tmp1.size(); ++rsd4 ) {
									...
									for ( Size h4rot = 1; h4rot <= rotsets[rsd4]->num_rotamers(); ++h4rot ) {
										tmp3.set_chi(1,rsd4,rotsets[rsd4]->rotamer(h4rot)->chi(1));
										tmp3.set_chi(2,rsd4,rotsets[rsd4]->rotamer(h4rot)->chi(2));
										//his4.replace_residue(1,tmp1.residue(rsd4),false);
										Vec h4m = tmp3.residue(rsd4).xyz("HE2");
										if ( fabs(((h4m-symcen1) - Psym1*(h4m-symcen1)).length() - Dzn) > 1.0 ) continue;
										Real h4sa1 = dihedral_degrees(ZN-symcen1,symaxs1,Vec(0,0,0),h4m-symcen1);
										Vec h4n = rotation_matrix_degrees(symaxs1,h4sa1)*(tmp3.residue(rsd4).xyz("NE2")-symcen1)+symcen1;
										Vec h4axs = (h4n-ZN).normalized();
										if ( his4axs.dot(h4axs) < 0.80 ) continue;
										...
										tmp1.set_chi(1,rsd4,rotsets[rsd4]->rotamer(h4rot)->chi(1));
										tmp1.set_chi(2,rsd4,rotsets[rsd4]->rotamer(h4rot)->chi(2));
										tmp2.set_chi(1,rsd4,rotsets[rsd4]->rotamer(h4rot)->chi(1));
										tmp2.set_chi(2,rsd4,rotsets[rsd4]->rotamer(h4rot)->chi(2));
										tmp3.set_chi(1,rsd4,rotsets[rsd4]->rotamer(h4rot)->chi(1));
										tmp3.set_chi(2,rsd4,rotsets[rsd4]->rotamer(h4rot)->chi(2));
...
void run(std::string fname) {
	...
	vector1<core::pack::rotamer_set::RotamerSetOP> rotsets(hisp.size(),NULL);
	//#ifdef USE_OPENMP
	//#pragma omp parallel for schedule(dynamic,1)
	//#endif
	for ( Size i = 1; i <= hisp.size(); ++i ) {
		//TR << i << std::endl;
