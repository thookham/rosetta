...
Real ddg(Pose const & p_in, ScoreFunctionOP sf, Size Ntri, Real & rholes, Real & /*hsp*/, Real & /*ssp*/, Real & /*sht*/) {
	...
	for ( Size i = 1; i <= p.size(); ++i ) {
		bool iface = false;
		if ( i <= Ntri ) {
			for ( Size j = Ntri+1; j <= 2*Ntri; ++j ) if ( p.xyz(AtomID(2,i)).distance_squared(p.xyz(AtomID(2,j))) < 144.0 ) iface = true;
		} else if ( i <= 2*Ntri ) {
			for ( Size j =      1; j <=   Ntri; ++j ) if ( p.xyz(AtomID(2,i)).distance_squared(p.xyz(AtomID(2,j))) < 144.0 ) iface = true;
		}
		if ( !iface ) {
			task->nonconst_residue_task(i).prevent_repacking();
		} else {
		...
		for ( Size isg1 = 1; isg1 <= 1; ++isg1 ) {
			for ( Size isg2 = 0; isg2 <= 1; ++isg2 ) {
				...
				for ( Size iori = 0; iori <= 360; ++iori ) {
					...
					TR << "BEGIN XTAL CHECK " << fname << " " << iaxs << " " << iori <<  std::endl;
					vector1<Pose> toadd;
					...
					if ( xcbc == cbc ) continue;
					trans_pose(todes,-C2);
					alignaxis(todes,Vec(0,0,1),axs,Vec(0,0,0));
					TR << cbc << " " << xcbc << std::endl;
					todes.dump_pdb("todes0.pdb");
					rot_pose(todes,Vec(0,0,1),180.0);
					todes.dump_pdb("todes1.pdb");
					utility_exit_with_message("aoirstn");


					if ( xcbc < CONTACT_TH1 ) continue;

					trans_pose(p3b,-t*ori);
					vector1<DsfHit> dhits = find_dsf(p3a,p3b,ifc3,cyschi1,R2,C2,0,0);
					trans_pose(p3b,t*ori);

					if ( xcbc < CONTACT_TH2 && dhits.size() == 0 ) continue;

					TR << "CBC " << iaxs << " " << isg1 << " " << isg2 << " " << iori << " " << cbc << std::endl;

					string tag = utility::file_basename(fname)+"_"+lzs(iaxs,3)+"_"+lzs(isg1,1)+"_"+lzs(isg2,1)+"_"+lzs(iori,3)+"_"+F(4,2,ofst)+"_"+lzs(xcbc,3);
					...
					for ( Size id = 1; id <= dhits.size(); id++ ) {
						...
						ss->add_energy( "rholes", core::scoring::packing::compute_dec15_score(ssym) - rholes0 );
					...
					if ( xcbc >= CONTACT_TH2 ) {
						...
						design_1comp(sym,sf,todes.size());
						{
							...
							ss->add_energy( "cbc", cbc );
							ss->add_energy( "nres", init.size() );
							ss->add_energy( "rholes", core::scoring::packing::compute_dec15_score(sym) - rholes0 );
