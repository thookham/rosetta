	...
	for ( int i = 1; i <= nres; i++ ) {
		...
		for ( int j = 1; j <=nres; j++ ) {
			...
			if ( CA_dist < CA_cutoff && i != j ) {
				...
				// flat in the region that we want the structure to stay in, and sharply penalizes outside that region.
...
run_mc(pose::Pose & p, ScoreFunctionOP s,
	...
	protocols::minimization_packing::RotamerTrialsMoverOP rottrial_mover(new protocols::minimization_packing::RotamerTrialsMover(s,(*pt)));
	...
	//add packrotamermover to alter sidechains with a certain probability (0.1?)
...
core::Real
set_temp_based_on_ens_diversity(pose::Pose & p,ScoreFunctionOP s, core::Real avg_ca_rmsd_target){
