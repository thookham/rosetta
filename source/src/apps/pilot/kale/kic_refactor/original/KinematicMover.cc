// I made the following modifications to this file:
// 1. Add folds
// 2. Remove all commented code.

KinematicMover::KinematicMover() : // {{{1
	Mover(),
	start_res_(2),
	middle_res_(3),
	end_res_(4),
...
void KinematicMover::set_pivots( // {{{1
		Size start_res, Size middle_res, Size end_res ) {
...
void KinematicMover::set_loop_begin_and_end( // {{{1
		Size loop_begin, Size loop_end ) {
	loop_begin_ = loop_begin;
	loop_end_ = loop_end;
}

void KinematicMover::set_vary_bondangles( bool vary ) // {{{1
{
	KinematicMover::vary_bond_angles_=vary;
}

bool KinematicMover::get_vary_bondangles() // {{{1
{
	return KinematicMover::vary_bond_angles_;
}

void
KinematicMover::set_perturber( KinematicPerturberOP perturber_in ) // {{{1
{
	perturber_ = perturber_in;
	perturber_->set_kinmover( this );
}

void KinematicMover::set_sample_nonpivot_torsions( bool sample ) // {{{1
{
	KinematicMover::sample_nonpivot_torsions_=sample;
}
	
bool KinematicMover::get_sample_nonpivot_torsions() // {{{1
{
	return KinematicMover::sample_nonpivot_torsions_;
}

void KinematicMover::set_idealize_loop_first( bool idealize ) // {{{1
{
	KinematicMover::idealize_loop_first_ = idealize;
}

bool KinematicMover::get_idealize_loop_first() // {{{1
{
	return KinematicMover::idealize_loop_first_;
}

void KinematicMover::set_rama_check( bool do_rama ) // {{{1
{
	KinematicMover::do_rama_check_=do_rama;
}

bool KinematicMover::get_rama_check() // {{{1
{
	return KinematicMover::do_rama_check_;
}

void KinematicMover::set_hardsphere_bump_check( bool do_bump_check ) // {{{1
{
	do_hardsphere_bump_check_ = do_bump_check;
}

bool KinematicMover::get_hardsphere_bump_check() // {{{1
{
	return do_hardsphere_bump_check_;
}

void KinematicMover::set_do_sfxn_eval_every_iteration( bool do_sfxn_eval ) // {{{1
{
	do_sfxn_eval_every_iteration_ = do_sfxn_eval;
}

void KinematicMover::set_sfxn(core::scoring::ScoreFunctionCOP sfxn_in) // {{{1
{
	sfxn_ = sfxn_in;
}

void KinematicMover::set_sweep_nonpivot_torsions( bool sweep ) { // {{{1
...
bool KinematicMover::sweep_incomplete() const // {{{1
{
	return !sweep_iterator_.at_end();
}

void KinematicMover::set_temperature( Real temp_in ) // {{{1
{
	temperature_=temp_in;
}

bool KinematicMover::check_rama(Real old_rama_score, Real new_rama_score) { // {{{1
...
bool KinematicMover::last_move_succeeded() // {{{1
{
...
std::string KinematicMover::get_bb_atoms_for_residue ( // {{{1
	const core::conformation::Residue &res,
...
void KinematicMover::apply( core::pose::Pose & pose ) // {{{1
{
...
bool KinematicMover::perform_bump_check(
	core::pose::Pose const & pose,
	Size const start_res,
	Size const end_res
)
...
// {{{1
/// set default options
/// AS -- after vicinity refactoring, only the perturber knows about vicinity 
/// sampling -- for now this function doesn't do anything any more... 
void
...
// {{{1
void KinematicMover::update_sequence( utility::vector1< core::chemical::AA > const & sequence ) {
	
	// note: it would be much better to implement this as a vector/map of 
	// strings, that then redirect to the corresponding taboo map -- otherwise we 
	// lose all information the moment we change loops, which will happen a lot 
	// when remodeling multiple loops -- TODO in other words, this implementation 
	// is only intended for remodeling single loops
...
// {{{1
/// the TabooSamplingKinematicPerturber must access this to derive the 
/// appropriate torsion bins
utility::vector1< core::chemical::AA > KinematicMover::get_loop_sequence() const 
{
	return sequence_;		
}

// {{{1
void KinematicMover::insert_sampled_torsion_string_into_taboo_map( std::string const & ts ) {
...
// {{{1
bool KinematicMover::is_in_taboo_map( std::string const & ts ) const {
...
core::Real KinematicMover::frequency_in_taboo_map( core::Size const & pos, char const & torsion_bin ) const {
...
// {{{1
/// @brief bin torsion angles as described in 
/// http://www.ncbi.nlm.nih.gov/pubmed/19646450
/// @detail generates a string with the torsion angle bins, using uppercase 
/// letters as in the publication above for omega ~ 180, and lowercase letters 
/// for omega ~ 0; to be used in loop sampling analysis -- same as in the 
/// LoopMover implementation... 
