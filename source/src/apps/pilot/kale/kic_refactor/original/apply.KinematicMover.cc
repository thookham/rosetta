void KinematicMover::apply( core::pose::Pose & pose ) {
	...
	// if we want kinematic closure to be generally applicable (i.e. useful for 
	// non-protein backbones, too).
	chainTORS(atoms.size(), atoms, dt_ang, db_ang, db_len, R0, Q0);
	// }}}1

	if (! idealize_loop_first_) {
		// Copy the "angle/length" matrices {{{1
		// save the previous torsions, lengths and angles to restore if no solution 
		// is found
	...
	if (idealize_loop_first_) {
		// Copy all the original residues {{{1
		// idealize the loop if requested
		// NOTE: for now, this only works for alpha-amino acids.  If there are 
		// beta-amino acids in the mix, this will return an error.

		// save a backup of the non-ideal residues in case closure fails -- 
		// AS_DEBUG: now trying to simply store the entire pose and write it back
