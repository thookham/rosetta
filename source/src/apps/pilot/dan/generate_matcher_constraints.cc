...
static basic::Tracer TR( "generate_matcher_constraints" );
...
int main( int argc, char * argv [] )
{
	try {
		...
		if ( ! ( option[ U1 ].user() && option[ U2 ].user() && option[ U3 ].user()) ) { // auto-select the upstream atoms
			TR << "All upstream atoms not specifified. Auto-selecting from residue " << upres << std::endl;
			...
			// SPECIAL CASES
			if ( ( pose.residue( upres ).name3() == "TYR" ) ||
					( pose.residue( upres ).name3() == "THR" ) ) {
				// Go back one atom to get an aroC for Tyr and the OH for Thr -- also trying this for LEU
				--u1ind;
			}
		...
		if ( ! ( option[ D1 ].user() && option[ D2 ].user() && option[ D3 ].user()) ) { // auto-seslect the downstream
			TR << "All downstream atoms not specifified. Auto-selecting from residue " << downres << std::endl;
