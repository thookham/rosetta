...
static basic::Tracer TT( "pilot_apps.jcorn.two_chain_interface", basic::t_trace );
static basic::Tracer TD( "pilot_apps.jcorn.two_chain_interface", basic::t_debug );
static basic::Tracer TI( "pilot_apps.jcorn.two_chain_interface", basic::t_info );

class TwoChainTenANeighbors {

public:
	TwoChainTenANeighbors(); //  empty constructor
	TwoChainTenANeighbors( pose::Pose &, Size const, Size const ); // constructor with chain numbers
	TwoChainTenANeighbors( pose::Pose &, char const, char const ); // constructor with chain letters
...
TwoChainTenANeighbors::TwoChainTenANeighbors( pose::Pose & pose, Size const chain1_id, Size const chain2_id ) { // constructor with arguments
	chain1_number_ = chain1_id;
	chain2_number_ = chain2_id;
	find_interface( pose, chain1_number_, chain2_number_ );
}

TwoChainTenANeighbors::TwoChainTenANeighbors( pose::Pose & pose, char const chain1_id, char const chain2_id ) { // constructor with arguments
	chain1_letter_ = chain1_id;
	chain2_letter_ = chain2_id;
	find_interface( pose, chain1_letter_, chain2_letter_ );
}

TwoChainTenANeighbors::TwoChainTenANeighbors() { } // empty constructor
	...
	try {
		...
		for ( utility::vector1< char >::iterator chain1 = search_chain_letters.begin(); chain1 != search_chain_letters.end(); ++chain1 ) {
			for ( utility::vector1< char >::iterator chain2 = chain1+1; chain2 != search_chain_letters.end(); ++chain2 ) {
				two_chain_interface = TwoChainTenANeighbors( pose, *chain1, *chain2 );
