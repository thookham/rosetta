...
)
{
	...
	core::io::silent::SilentFileData pose_silent_data( opts );
	...
	core::io::silent::SilentStructOP pose_ss( new core::io::silent::ScoreFileSilentStruct(opts) );
	...
	for ( Size iaa = 0; iaa <= restypes.length() - 1; ++iaa ) {
		...
		for ( Size seqpos = 1; seqpos <= nres; ++seqpos ) {
			...
			core::io::silent::SilentStructOP res_ss( new core::io::silent::ScoreFileSilentStruct(opts) );
			res_ss->decoy_tag( pdbname );

			//print byatom data
			core::io::silent::SilentStructOP atom_ss( new core::io::silent::ScoreFileSilentStruct(opts) );
