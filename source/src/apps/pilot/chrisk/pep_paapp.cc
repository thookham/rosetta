...
id::initialize( atom_map, pose, id::AtomID::BOGUS_ATOM_ID() );
...
RunPepSpec()
{
	...
	pose.conformation().safely_append_polymer_residue_after_seqpos( *gly, 1, true );
	pose.conformation().safely_prepend_polymer_residue_before_seqpos( *gly, 1, true );
	for ( Size i_omega = 1; i_omega <= pose.size() - 1; ++i_omega ) {
		pose.set_omega( i_omega, 180.0 );
	}

	for ( Size peptide_loop = 1; peptide_loop <= 20; ++peptide_loop ) { //LOOP how many final peps

		chemical::make_sequence_change( 2, chemical::AA( peptide_loop ), pose );
		Size pep_begin( 1 );
		Size pep_end( pose.size() );

		//create output file
		//  std::string out_file_name_str( pose.residue( 2 ).name3() + ".paapp" );
		std::string out_file_name_str( pose.residue( 2 ).name3() + ".rama" );
		...
		for ( Real i = -180; i < 180; ++i ) {
			if ( i == 0 ) i = 0.1;
			pose.set_psi( 2, i );
			for ( Real ii = -180; ii < 180; ++ii ) {
				if ( ii == 0 ) ii = 0.1;
				pose.set_phi( 2, ii );
				( *full_scorefxn )( pose );
				//    out_file << pose.residue( 2 ).name1() << "\t" << pose.phi( 2 ) << "\t" << pose.psi( 2 ) << "\t" << pose.energies().residue_total_energies( 2 )[ p_aa_pp ] << "\n";
				//    out_file << pose.energies().residue_total_energies( 2 )[ p_aa_pp ] << "\t";
				out_file << pose.energies().residue_total_energies( 2 )[ rama ] << "\t";
			}
			out_file << "\n";
		}
