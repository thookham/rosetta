...
static basic::Tracer TR( "apps.pilot.chrisk/pepspec" );
...
protocols::minimization_packing::MinMoverOP min_mover = new protocols::minimization_packing::MinMover( mm, cen_scorefxn, "lbfgs_armijo_nonmonotone", 0.001, true );
		...
		*/
		...
		for ( Size cgrelax_loop = 1; cgrelax_loop <= n_cgrelax_loop; cgrelax_loop++ ) {
			...
			//full_wts
			{
				...
				protocols::minimization_packing::RotamerTrialsMoverOP dz_rottrial ( new protocols::minimization_packing::RotamerTrialsMover( full_scorefxn, dz_task_factory ) );
			...
			if ( option[ pepspec::soft_wts ].user() ) {
				...
				protocols::minimization_packing::RotamerTrialsMoverOP dz_rottrial ( new protocols::minimization_packing::RotamerTrialsMover( soft_scorefxn, dz_task_factory ) );
				SequenceMoverOP design_seq = new SequenceMover;
				design_seq->add_mover( dz_pack );
				design_seq->add_mover( dz_rottrial );
				design_seq->add_mover( min_mover );
