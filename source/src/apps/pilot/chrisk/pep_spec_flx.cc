...
)
{
	...
	using namespace chemical;
	using namespace ObjexxFCL::format; // I and F
	vector1< bool > is_clash( pose.size(), false );
...
)
{
	...
	if ( option[ pep_spec::input_seq ].user() ) {
		...
		for ( Size mut_site = pep_begin; mut_site <= pep_end; mut_site++ ) {
			if ( mut_site==pep_anchor ) continue;
			chemical::make_sequence_change( mut_site, chemical::AA( aa_from_oneletter_code( res->name1() ) ) , pose );
		}
	}
...
)
{
	...
	while ( n_prepended < n_prepend || n_appended < n_append ) {
		Size this_prepend( static_cast< int >( numeric::random::rg().uniform() * ( n_prepend - n_prepended + 1 ) ) );
		Size this_append( static_cast< int >( numeric::random::rg().uniform() * ( n_append - n_appended + 1 ) ) );
		for ( Size ii = 1; ii <= this_prepend; ++ii ) {
			...
			pose.conformation().update_polymeric_connection( pep_begin );
			pose.conformation().update_polymeric_connection( pep_begin + 1 );
			if ( option[ pep_spec::no_cen ] ) {
				if ( numeric::random::rg().uniform() < 0.5 ) chemical::make_sequence_change( pep_begin, chemical::AA( aa_from_oneletter_code( 'A' ) ), pose );
				else chemical::make_sequence_change( pep_begin, chemical::AA( aa_from_oneletter_code( 'G' ) ), pose );
			}
		...
		for ( Size ii = 1; ii <= this_append; ++ii ) {
			...
			pose.conformation().update_polymeric_connection( pep_end - 1 );
			if ( option[ pep_spec::no_cen ] ) {
				if ( numeric::random::rg().uniform() < 0.5 ) chemical::make_sequence_change( pep_end, chemical::AA( aa_from_oneletter_code( 'A' ) ), pose );
				else chemical::make_sequence_change( pep_end, chemical::AA( aa_from_oneletter_code( 'G' ) ), pose );
			}
		...
		for ( Size build_loop_inner = 1; build_loop_inner <= this_build_loop; ++build_loop_inner ) {
			...
			/*
			//if makes clash w/ prot sidechain, try fix w/ rot trials, if still clash w/ it or new clash, reject
			if( option[ pep_spec::no_cen ] ){
			#find pep seqpos w/ prot clash
			vector1< bool > is_clash( is_clash( pose, ... ) );
			#is clash w/ prot sidechain?
			...
			for ( Size i=1; i<= pose.size(); ++i ) {
			if ( is_clash[ i ] ) restrict_to_repack_taskop->include_residue( i );
			...
			protocols::minimization_packing::RotamerTrialsMoverOP dz_rottrial ( new protocols::minimization_packing::RotamerTrialsMover( soft_scorefxn, rp_task_factory ) );
			}
			*/
		...
		//  if( !option[ pep_spec::no_cen ] ) mc_frag->recover_low( pose );
		mc_frag->recover_low( pose );

		( *cen_scorefxn )( pose );
		//if gen clash, step back a step
		Real clash_score( pose.energies().total_energies()[ vdw ] );
		bool this_has_clash( clash_score > min_score + 0.03 );
		if ( option[ pep_spec::no_cen ] ) {
			vector1< bool > is_pep( pose.size(), false );
			for ( Size i = 1; i <= pose.size(); ++i ) is_pep[ i ] = ( i >= pep_begin && i <= pep_end );
			this_has_clash = has_clash( pose, is_pep, cen_scorefxn, option[ pep_spec::clash_fa_rep_tol ] );
		}
		if ( break_loop < 5 && this_has_clash && !option[ pep_spec::random_seq ] ) {
			++break_loop;
			for ( Size ii = 1; ii <= this_prepend; ++ii ) {
				pose.conformation().delete_residue_slow( pep_begin );
				pep_anchor = pep_anchor - 1;
...
)
{
	...
	for ( Size mut_site = 1; mut_site <= pose.size(); mut_site++ ) {
		...
		} else if ( is_pep_nbr[ mut_site ] && pose.residue( mut_site ).name1() != 'G' && pose.residue( mut_site ).name1() != 'P' ) {
...
)
{
	...
	protocols::minimization_packing::PackRotamersMoverOP pack( new protocols::minimization_packing::PackRotamersMover( full_scorefxn, task, 1 ) );
	pack->apply( pose );
...
RunPepSpec()
{
	...
	*/
	...
	if ( prot_chain < pep_chain ) {
		...
		if ( start_pep_end != pose.size() ) f.new_jump( prot_anchor, start_pep_end + 1, start_pep_end );
	...
	*/
	...
	// set up the PoseMetricCalculators
	...
	for ( Size peptide_loop = 1; peptide_loop <= n_peptides; ++peptide_loop ) {
		...
		std::string ss_seq( "L" );
		if ( option[ pep_spec::ss_type ].user() ) ss_seq = option[ pep_spec::ss_type ];
		for ( Size i = 1; i <= 20; ++i ) {
		...
		if ( option[ pep_spec::fastrelax_input ] ) {
			protocols::relax::FastMultiRelax relaxer( full_scorefxn );
			relaxer.apply( pose );
			//   dump_pdb( pose, pdb_dir + "/" + out_nametag + "_" + string_of( peptide_loop ) + ".pdb"  );
			//   out_file << pdb_dir + "/" + out_nametag + "_" + string_of( peptide_loop ) + ".pdb" + "\t"<<pose.energies().total_energies().weighted_string_of( full_scorefxn->weights() );
		...
		if ( !option[ pep_spec::no_cen ] ) core::util::switch_to_residue_type_set( pose, core::chemical::CENTROID );
		( *cen_scorefxn )( pose );
		...
		*/
		...
		vall.get_frags( 10000, frag_seq, ss_seq, seq_wt, 0.0, false, false, true, lib_old[i] );
		}
		...
		*/
		...
		( *cen_scorefxn )( pose );
		if ( peptide_loop > 1 && pose.energies().total_energies()[ vdw ] > min_vdw + 0.1 && !option[ pep_spec::random_seq ] ) {
			--peptide_loop;
			continue;
		}
		...
		for ( Size cgrelax_loop = 1; cgrelax_loop <= n_cgrelax_loop; cgrelax_loop++ ) {
			...
			if ( option[ pep_spec::homol_bb ] ) n_cgperturb_iter = option[ pep_spec::n_build_loop ];
			if ( !( option[ pep_spec::test_no_cgrelax ] || cgrelax_loop == 1 ) || option[ pep_spec::homol_bb ] ) {
			...
			/*
			basic::MetricValue< std::set< Size > > interface_res;
			pose.metric( "interface", "interface_residues", interface_res );
			for ( std::set< Size >::const_iterator it = interface_res.value().begin(); it != interface_res.value().end(); ++it ){
			std::cout << string_of( *it ) + "\t";
			}
			std::cout << "\n";
			*/
			...
			if ( !( option[ pep_spec::test_no_design ] || option[ pep_spec::input_seq ].user() ) ) {
				if ( refine_fa_pep_bb( pose, pep_anchor, pep_jump, is_pep, is_pep_nbr, full_scorefxn ) && peptide_loop > 1 ) {
					--peptide_loop;
					continue;
				}
			}
			...
			}
			{
				...
				//upweight interface?
				if ( option[ pep_spec::upweight_interface ] ) {
					Real upweight( 2.0 );
				...
				print_pep_analysis( pdb_name, out_file, pose, full_scorefxn, prot_begin, prot_end, pep_begin, pep_anchor, pep_end );
			}
			std::string wts_name( option[ pep_spec::wts ] ), soft_wts_name( option[ pep_spec::soft_wts ] );
			if ( wts_name.compare( soft_wts_name ) != 0 ) {
				pack::task::PackerTaskOP dz_task( dz_task_factory->create_task_and_apply_taskoperations( pose ));
			...
			if ( option[ pep_spec::mcmin_design ] ) {
				for ( Size ii = 1; ii <= 10 * ( pep_end - pep_begin + 1 ); ++ii ) {
					mutate_random_residue( pose, is_mutable, soft_scorefxn, full_scorefxn );
					if ( mc_relax->boltzmann( pose ) ) {
			...
			if ( n_farelax_loop > 0 ) {
				...
				mc_relax->recover_low( pose );
				pdb_name = pdb_dir + "/" + out_nametag + "_" + string_of( peptide_loop ) + "_relax.pdb";
				print_pep_analysis( pdb_name, out_file, pose, full_scorefxn, prot_begin, prot_end, pep_begin, pep_anchor, pep_end );
			}
			//remove buffer res
			if ( option[ pep_spec::add_buffer_res ] ) remove_pep_res( pose, prot_anchor, pep_begin, pep_anchor, pep_end, add_nterm, add_cterm );
