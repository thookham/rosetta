#ifndef INCLUDED_ObjexxFCL_DimensionExpressionSub_hh
#define INCLUDED_ObjexxFCL_DimensionExpressionSub_hh
...
/// @brief DimensionExpressionSub: DimensionExpression Binary Subtraction
...
public: // Creation
	...
	inline
	DimensionExpressionSub( DimensionExpressionSub const & exp ) :
		Super(),
	...
	inline
	DimensionExpressionSub( DimensionExpression const & exp1, DimensionExpression const & exp2 ) :
		exp1_p_( exp1.clone() ),
		exp2_p_( exp2.clone() )
	{
		assert( exp1_p_ );
	...
	inline
	DimensionExpressionSub( DimensionExpression * exp1_p_a, DimensionExpression * exp2_p_a ) :
		exp1_p_( exp1_p_a ),
		exp2_p_( exp2_p_a )
	{
		assert( exp1_p_ );
	...
	clone() const override
	{
		...
		if ( constant() ) {
			...
			return new DimensionExpressionSub( exp1_p_->clone(), exp2_p_->clone() );
		}
	}
	...
	clone( Dimension const & dim ) const override
	{
		...
		if ( constant() ) {
			...
			return new DimensionExpressionSub( exp1_p_->clone( dim ), exp2_p_->clone( dim ) );
		}
	}
	...
	/// @brief Destructor
	inline
	~DimensionExpressionSub() override
	{
		assert( exp1_p_ );
		assert( exp2_p_ );
		delete exp1_p_;
...
#endif // INCLUDED_ObjexxFCL_DimensionExpressionSub_HH
