...
/// @file   utility/graph/UpperEdgeGraph.srlz.hh
/// @brief  templated graph for fast edge additions
/// @author Andrew Leaver-Fay (aleaverfay@gmail.com)

#ifdef    SERIALIZATION

#ifndef INCLUDED_utility_graph_UpperEdgeGraph_SRLZ_HH
#define INCLUDED_utility_graph_UpperEdgeGraph_SRLZ_HH
...
void save_to_archive( Archive & arc, typename utility::graph::UpperEdgeGraph< V, E > const & graph ) {
	arc( graph.num_vertices() );
	for ( platform::Size ii = 1; ii <= graph.num_vertices(); ++ii ) {
...
template < class Archive, class V, class E >
void load_from_archive( Archive & arc, typename utility::graph::UpperEdgeGraph< V, E > & graph ) {
	...
	for ( platform::Size ii = 1; ii <= graph.num_vertices(); ++ii ) {
		...
		for ( platform::Size jj = 1; jj <= ii_n_upper_neighbors; ++jj ) {
