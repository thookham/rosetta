...
//fpd error function with complex argmuments
// code from http://ab-initio.mit.edu/Faddeeva
...
cmplx errf(cmplx z, double relerr)
{
	...
	if ( y == 0 ) {
		return C(errf(x), y); // preserve sign of 0
	}
	if ( x == 0 ) { // handle separately for speed & handling of y = Inf or NaN
		...
		return C(x, y*y > 720 ? (y > 0 ? Inf : -Inf): exp(y*y) * w_im(y));
	}
...
cmplx errfi(cmplx z, double relerr) {
	cmplx e = errf(C(-cimag(z),creal(z)), relerr);
...
double errfc(double x) {
	...
	return x >= 0 ? exp(-x*x) * errfcx(x) : 2. - exp(-x*x) * errfcx(-x);
}
...
cmplx Dawson(cmplx z, double relerr) {
	...
	if ( x == 0 ) {
		...
		return C(x, // preserve sign of 0
			spi2 * (y >= 0
			? exp(y2) - errfcx(y) : errfcx(-y) - exp(y2)));
	}
...
// for double-precision a2 = 0.26865... in w, below.
...
cmplx w(cmplx z, double relerr) {
	if ( creal(z) == 0.0 ) {
		return C(errfcx(cimag(z)), creal(z)); // give correct sign of 0 in cimag(w)
	} else if ( cimag(z) == 0 ) {
		return C(exp(-sqr(creal(z))), w_im(creal(z)));
	}
