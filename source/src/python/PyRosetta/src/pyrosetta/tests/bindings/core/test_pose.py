...
class TestPoseCacheAccessor(unittest.TestCase):
    ...
    def test_pose_cache(self):
        ...
        self.assertDictEqual(dict(ref_scores["metrics"]["string"]), dict(self.pose.cache.metrics.string))
        self.assertDictEqual(dict(ref_scores["metrics"]["composite_string"]), dict(self.pose.cache.metrics.composite_string))
        self.assertDictEqual(dict(ref_scores["metrics"]["composite_real"]), dict(self.pose.cache.metrics.composite_real))
        self.assertDictEqual(dict(ref_scores["metrics"]["per_residue_string"]), dict(self.pose.cache.metrics.per_residue_string))
        self.assertDictEqual(dict(ref_scores["metrics"]["per_residue_real"]), dict(self.pose.cache.metrics.per_residue_real))
        self.assertDictEqual(dict(ref_scores["metrics"]["per_residue_probabilities"]), dict(self.pose.cache.metrics.per_residue_probabilities))
        self.assertDictEqual(dict(ref_scores["extra"]["real"]), dict(self.pose.cache.extra.real))
        self.assertDictEqual(dict(ref_scores["extra"]["string"]), dict(self.pose.cache.extra.string))
