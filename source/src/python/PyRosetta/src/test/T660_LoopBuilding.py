...
loop = protocols.loops.Loop(70,80,75)
loops = protocols.loops.Loops()
loops.add_loop(loop)

protocols.loops.set_single_loop_fold_tree(loop_p, loop)
...
#scorefxn = create_score_function('cen_std', 'score4L')
#scorefxn(loop_p)
#loop_perturb = protocols.loops.LoopMover_Perturb_CCD(loops, scorefxn, fragset3mer)
#loop_perturb.apply(p)
#loop_perturb.model_loop(loop_p, loop)

#recover_sidechain.apply(loop_p)

scorefxn = get_fa_scorefxn() #  create_score_function('standard', 'score12')
scorefxn(loop_p)
