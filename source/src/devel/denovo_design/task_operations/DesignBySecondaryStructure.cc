...
DesignBySecondaryStructureOperation::get_residues_to_design( core::pose::Pose const & pose ) const
{
	...
	if ( ! psipred_interface_ ) {
		...
		core::io::external::PsiPredResult const & psipred_result( psipred_interface_->run_psipred( pose, wanted_ss ) );
	...
	if ( regions_to_design() == 0 ) {
		...
		residues_to_design = core::io::external::nonmatching_residues( bp_ss, pred_ss );
	} else {
