...
static basic::Tracer tr( "devel.replica_docking.WrapFilterAsEvaluatorCreator" );
...
void WrapFilterAsEvaluatorCreator::add_evaluators( protocols::evaluation::MetaPoseEvaluator & eval ) const {
	...
	if ( option[ OptionKeys::evaluation::I_sc ].user() || option[ OptionKeys::evaluation::DockMetrics ].user() ) {
		...
		eval.add_evaluation( utility::pointer::make_shared< devel::replica_docking::WrapFilterAsEvaluator >( f , "I_sc" ) );
	}
	if ( option[ OptionKeys::evaluation::Irms ].user() || option[ OptionKeys::evaluation::DockMetrics ].user() ) {
		tr << "Add Irms evaluator " << std::endl;
		eval.add_evaluation( utility::pointer::make_shared< WrapFilterAsEvaluator >( utility::pointer::make_shared< devel::replica_docking::IrmsdFilter >(), "Irms" ) );
	}

	if ( option[ OptionKeys::evaluation::Ca_Irms ].user() || option[ OptionKeys::evaluation::DockMetrics ].user() ) {
		tr << "Add Irms evaluator " << std::endl;
		eval.add_evaluation( utility::pointer::make_shared< WrapFilterAsEvaluator >( utility::pointer::make_shared< devel::replica_docking::CaIrmsdFilter >(), "Ca_Irms" ) );
	}
	if ( option[ OptionKeys::evaluation::Fnat ].user() || option[ OptionKeys::evaluation::DockMetrics ].user() ) {
		tr << "Add evaluator of Fnat " << std::endl;
		eval.add_evaluation( utility::pointer::make_shared< WrapFilterAsEvaluator >( utility::pointer::make_shared< FnatFilter >(), "Fnat_n" ) );
	}
	if ( option[ OptionKeys::evaluation::Lrmsd ].user() || option[ OptionKeys::evaluation::DockMetrics ].user() ) {
		tr << "Add Lrmsd evaluator " << std::endl;
		eval.add_evaluation( utility::pointer::make_shared< WrapFilterAsEvaluator >( utility::pointer::make_shared< devel::replica_docking::LrmsdFilter >(), "Lrmsd" ) );
	}
	if ( option[ OptionKeys::evaluation::Fnonnat ].user() || option[ OptionKeys::evaluation::DockMetrics ].user() ) {
		...
		eval.add_evaluation( utility::pointer::make_shared< WrapFilterAsEvaluator >( utility::pointer::make_shared< FnonnatFilter >(), "Fnonnat" ) );
	}
}
