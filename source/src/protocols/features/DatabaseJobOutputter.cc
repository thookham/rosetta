...
bool DatabaseJobOutputter::job_has_completed(
	...
	if ( option[out::resume_batch].user() ) {
		...
		for ( core::Size j = 1; j < placeholder_count; ++j ) {
			placeholder_block += ",?";
		}
		placeholder_block += ")";
		...
		for ( core::Size i = 1; i <= batch_ids.size(); ++i ) {
...
DatabaseJobOutputterCreator::create_JobOutputter() const {
	return utility::pointer::make_shared< DatabaseJobOutputter >();
}
