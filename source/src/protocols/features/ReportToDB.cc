...
ReportToDB::parse_protocol_id_tag_item(
	...
	if ( tag->hasOption("protocol_id") ) {
		...
		//  protocol_id_ = tag->getOption<core::Size>("protocol_id") + mpi_rank;
		//#else
		protocol_id_ = tag->getOption<core::Size>("protocol_id");
		//#endif
	}
#ifdef USEMPI
	else {
...
ReportToDB::parse_batch_id_tag_item(
	...
	if ( tag->hasOption("batch_id") ) {
		...
		//  int mpi_rank(0);
		//  MPI_Comm_rank(MPI_COMM_WORLD, &mpi_rank);
		//  batch_id_ = tag->getOption<core::Size>("batch_id") + mpi_rank;
		//#else
		batch_id_ = tag->getOption<core::Size>("batch_id");
		//#endif
	}
#ifdef USEMPI
	else {
...
ReportToDB::apply( Pose& pose ){
	...
	try{
		...
		if ( protocol_id_==0 && batch_id_==0 ) {
			std::pair<core::Size, core::Size> ids = get_protocol_and_batch_id(batch_name_, batch_description_, features_reporters_, db_session_);
...
ReportToDB::report_features(
	...
	for ( core::Size i=1; i <= features_reporters_.size(); ++i ) {
...
void
ReportToDB::provide_citation_info(basic::citation_manager::CitationCollectionList & citations ) const {
