...
SidechainMinMover::SidechainMinMover() : DockingHighRes()
{
	...
	core::scoring::ScoreFunctionOP temp_sf = core::scoring::get_score_function();
...
void SidechainMinMover::set_default()
{
	...
	minmover_ = utility::pointer::make_shared< protocols::minimization_packing::MinMover >(movemap_, scorefxn(), "lbfgs_armijo_nonmonotone", 0.01, true/*nblist*/, false/*deriv_check*/  );
}
...
{
	interface_ = utility::pointer::make_shared< protocols::scoring::Interface >( movable_jumps()[1] );
}
...
InterfaceSidechainMinMover::apply( core::pose::Pose & pose )
{
	...
	if ( !( tf2() )->get_norepack2() ) {
		for ( core::Size i = cutpoint+1; i <= pose.size(); i++ ) {
			if ( interface_->is_interface(i) ) movemap_->set_chi(i, true);
		}
	}
