...
static basic::Tracer TR( "protocols.toolbox.rigid_body.FloatingBaseUtil" );
...
initialize_stub( conformation::Residue const & rsd ){
	...
	runtime_assert( rsd.is_RNA() || rsd.type().is_protein() || !rsd.type().is_polymer() ); // for now. can easily update in the future!
