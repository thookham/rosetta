...
loop_mover::LoopResult LoopMover_Perturb_KIC::model_loop(
	...
	*/
	...
	if ( basic::options::option[ basic::options::OptionKeys::loops::restrict_kic_sampling_to_torsion_string ].user() ||
			basic::options::option[ basic::options::OptionKeys::loops::derive_torsion_string_from_native_pose ]() ) {
		...
		// NO NO NO NO NO.
		// Use if statements for validation of user inputs
		// Use assertion statements for program validation.
		// this is where torsion-restricted sampling with multiple loops currently fails -- however, proper access
	...
	core::conformation::torsion_bin_string torsion_features = torsion_features_string( pose );
