...
static basic::Tracer TR_pcs_d_p_l_Ts2( "protocols.scoring.methods.pcsTs2.PCS_data_per_lanthanides_Ts2" );
static basic::Tracer TR_pcs_d_Ts2( "protocols.scoring.methods.pcsTs2.PCS_data_Ts2" );

PCS_data_per_lanthanides_Ts2::~PCS_data_per_lanthanides_Ts2()= default;

PCS_data_per_lanthanides_Ts2::PCS_data_per_lanthanides_Ts2(PCS_data_per_lanthanides_Ts2 const &other):
...
core::Real
PCS_data_per_lanthanides_Ts2::get_weight() const{
	return weight_;
}
...
PCS_data_per_lanthanides_Ts2::operator=( PCS_data_per_lanthanides_Ts2 const & other )
{
	if ( this != &other ) {
		n_pcs_ = other.n_pcs_;
		A_index_ = other.A_index_;
...
PCS_data_Ts2::clone() const {
	return utility::pointer::make_shared< PCS_data_Ts2 >( *this );
}
...
core::Real
PCS_data_per_lanthanides_Ts2::calculate_tensor_and_cost_with_svd(PCS_tensor_Ts2 &PCS_t){
...
utility::vector1<PCS_data_per_lanthanides_Ts2> &
PCS_data_Ts2::get_pcs_data_per_lanthanides_all() {
	return (PCS_data_per_lanthanides_all_);
}

const utility::vector1<PCS_line_data_Ts2> &
PCS_data_Ts2::get_PCS_data_line_all_spin() const{
	return (PCS_data_line_all_spin_);
}

const utility::vector1<PCS_data_per_lanthanides_Ts2>&
PCS_data_Ts2::get_pcs_data_per_lanthanides_all() const {
	return (PCS_data_per_lanthanides_all_);
}
...
void
PCS_data_Ts2::update_X_Y_Z_all(core::pose::Pose const & pose){
	core::Size i, res;
	std::string at;
...
core::Size
PCS_data_per_lanthanides_Ts2::get_n_pcs() const{
	return (n_pcs_);
}


const utility::vector1<core::Size> &
PCS_data_per_lanthanides_Ts2::get_A_index() const{
	return(A_index_);
}

const FArray1D< core::Real > &
PCS_data_per_lanthanides_Ts2::get_fstyle_b() const{
	return(fstyle_b_);
}


core::Size
PCS_data_Ts2::where_is_line(PCS_line_data_Ts2 & P_l_d){
...
PCS_data_per_lanthanides_Ts2::PCS_data_per_lanthanides_Ts2(std::string const & filename,
	...
	filename_(filename), svd_s_(basic::svd::SVD_Solver(PCS_d_l_a.size(), 5)), weight_(weight)
{
	...
	TR_pcs_d_p_l_Ts2 << "n_pcs: " << n_pcs_ << std::endl;
	TR_pcs_d_p_l_Ts2 << "Normalization 1: " << normalization_1_ << std::endl;
...
std::string
PCS_data_per_lanthanides_Ts2::get_filename() const {
	return (filename_);
}
...
PCS_data_Ts2::PCS_data_Ts2(PCS_data_input_Ts2 & P_d_i){
	std::map< std::string, PCS_file_data_Ts2 >::iterator it;

	std::string filename;
	PCS_line_data_Ts2 * P_l_d_temp;
	core::Size index, i, j;

	n_pcs_spin_ = 0;
	n_lanthanides_ = 0;

	std::map< std::string, PCS_file_data_Ts2 > & P_f_a_d = P_d_i.get_PCS_data_input_reference();

	for ( it = P_f_a_d.begin(); it != P_f_a_d.end(); ++it ) {
		filename = it->first;

		PCS_file_data_Ts2 & P_f_d = (it->second);
		n_lanthanides_ = n_lanthanides_ + 1;
		TR_pcs_d_Ts2 << "Filename " << filename  <<std::endl;
		core::Real weight(P_f_d.get_weight());

		utility::vector1<PCS_line_data_Ts2>::iterator it2;
		utility::vector1<PCS_line_data_Ts2> & PCS_d_l_a = P_f_d.get_PCS_data_line_all_reference();
...
PCS_data_Ts2::PCS_data_Ts2(PCS_data_input_Ts2 & P_d_i, utility::vector1< bool > const exclude_residues ){
	std::map< std::string, PCS_file_data_Ts2 >::iterator it;
...
core::Size
PCS_data_Ts2::get_n_lanthanides() const{
	return (n_lanthanides_);
}

const utility::vector1<core::Real> &
PCS_data_Ts2::get_X_all() const{
	return(X_all_);
}

const utility::vector1<core::Real> &
PCS_data_Ts2::get_Y_all() const{
	return(Y_all_);
}

const utility::vector1<core::Real> &
PCS_data_Ts2::get_Z_all() const{
	return(Z_all_);
}

std::ostream &
operator<<(std::ostream& out, const PCS_data_Ts2 & P_d){
	core::Size i;
...
std::ostream &
operator<<(std::ostream& out, const PCS_data_per_lanthanides_Ts2 &PCS_d_p_l){
	core::Size i, j;
