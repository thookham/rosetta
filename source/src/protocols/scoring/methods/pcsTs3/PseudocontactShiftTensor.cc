...
PCS_tensor_Ts3::PCS_tensor_Ts3(){
	utility_exit_with_message( "You shouldn't call the empty constructor for PCS_tensor_Ts3" );
}

PCS_tensor_Ts3::~PCS_tensor_Ts3()= default;

PCS_tensor_Ts3::PCS_tensor_Ts3(PCS_tensor_Ts3 const & other){
	a_ = other.a_;
	b_ = other.b_;
	g_ = other.g_;
	ax_ = other.ax_;
...
PCS_tensor_Ts3 &
PCS_tensor_Ts3::operator=(PCS_tensor_Ts3 const & other){
...
///////////////////////////////////////////////
PCS_tensor_Ts3::PCS_tensor_Ts3(core::Real const chi_xx,
...
void
PCS_tensor_Ts3::reset_tensor(core::Real const chi_xx,
	core::Real const chi_xy,
	core::Real const chi_xz,
	core::Real const chi_yy,
	core::Real const chi_yz){
...
std::ostream &
operator<<(std::ostream& out, const PCS_tensor_Ts3 &PCS_t){
...
void
PCS_tensor_Ts3::copy_from_ref(PCS_tensor_Ts3 & other){
	a_ = other.a_;
	b_ = other.b_;
	g_ = other.g_;
	ax_ = other.ax_;
...
std::string const &
PCS_tensor_Ts3::get_label() const{
	return label_;
}

core::Real
PCS_tensor_Ts3::delta_X_xx() const{
	return(rh_ / 2.0 - ax_ / 3.0);
}

core::Real
PCS_tensor_Ts3::delta_X_yy() const{
	return(-rh_ / 2.0 - ax_ / 2.0);
}

core::Real
PCS_tensor_Ts3::delta_X_zz() const{
	return(2.0/3.0 * ax_);
}

core::Real
PCS_tensor_Ts3::delta_chi_zz() const{
	return(-chi_xx_ -chi_yy_);
}


core::Real
PCS_tensor_Ts3::chi_xx() const{
	return(chi_xx_);
}

core::Real
PCS_tensor_Ts3::chi_yy() const{
	return(chi_yy_);
}

core::Real
PCS_tensor_Ts3::chi_xy() const{
	return(chi_xy_);
}

core::Real
PCS_tensor_Ts3::chi_xz() const{
	return(chi_xz_);
}

core::Real
PCS_tensor_Ts3::chi_yz() const{
	return(chi_yz_);
}
