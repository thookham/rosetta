...
static basic::Tracer TR_tsr_opt_Ts1( "protocols.scoring.methods.pcsTs1.TensorsOptimizer_Ts1" );


TensorsOptimizer_Ts1::TensorsOptimizer_Ts1(PCS_data_Ts1 const & pcs_d):
	pcs_d_(pcs_d)
{
}

TensorsOptimizer_Ts1::~TensorsOptimizer_Ts1()= default;

core::Real
TensorsOptimizer_Ts1::operator ()( core::optimization::Multivec const & vars ) const{
	core::Size i, j, idx;
	...
	if ( n_la != (vars.size() - 3) / 5 ) {
		TR_tsr_opt_Ts1 << "n_la: " << n_la << " (vars.size() - 3) / 5):" << (vars.size() - 3) / 5 << " vars.size(): " << vars.size() <<std::endl;
	...
	const utility::vector1<PCS_data_per_lanthanides_Ts1> & pcs_d_p_l_a(pcs_d_.get_pcs_data_per_lanthanides_all());
	...
	for ( i = 1; i <= n_la; i++ ) {
		...
		PCS_data_per_lanthanides_Ts1 const & pcs_d_p_l(pcs_d_p_l_a[i]);
...
void
TensorsOptimizer_Ts1::dfunc(core::optimization::Multivec const & vars,
