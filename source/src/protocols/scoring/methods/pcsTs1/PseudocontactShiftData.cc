...
core::Real
PCS_data_per_lanthanides_Ts1::get_weight() const{
	return weight_;
}

core::Real
PCS_data_per_lanthanides_Ts1::get_normalization_factor() const{
	return normalization_factor_;
}


PCS_data_per_lanthanides_Ts1 &
PCS_data_per_lanthanides_Ts1::operator=( PCS_data_per_lanthanides_Ts1 const & other )
{
	if ( this != &other ) {
		n_pcs_ = other.n_pcs_;
		A_index_ = other.A_index_;
...
PCS_data_Ts1::clone() const {
	return utility::pointer::make_shared< PCS_data_Ts1 >( *this );
}
...
void
PCS_data_per_lanthanides_Ts1::update_my_A_matrix(utility::vector1< utility::vector1<core::Real> > & A_all){
	core::Size i, j;
...
core::Real
PCS_data_per_lanthanides_Ts1::calculate_tensor_and_cost_with_svd(PCS_tensor_Ts1 &PCS_t){
...
utility::vector1<PCS_data_per_lanthanides_Ts1> &
PCS_data_Ts1::get_pcs_data_per_lanthanides_all() {
	return (PCS_data_per_lanthanides_all_);
}

const utility::vector1<PCS_line_data_Ts1> &
PCS_data_Ts1::get_PCS_data_line_all_spin() const{
	return (PCS_data_line_all_spin_);
}

const utility::vector1<PCS_data_per_lanthanides_Ts1>&
PCS_data_Ts1::get_pcs_data_per_lanthanides_all() const {
	return (PCS_data_per_lanthanides_all_);
}

void
PCS_data_Ts1::update_matrix_A(){
	core::Size i;
...
void
PCS_data_Ts1::update_matrix_A_all(core::Real const X,
	core::Real const Y,
	core::Real const Z){
	core::Size i;
	core::Real x, y, z;
...
void
PCS_data_Ts1::update_X_Y_Z_all(core::pose::Pose const & pose){
	core::Size i, res;
	std::string at;
...
core::Size
PCS_data_per_lanthanides_Ts1::get_n_pcs() const{
	return (n_pcs_);
}


const utility::vector1<core::Size> &
PCS_data_per_lanthanides_Ts1::get_A_index() const{
	return(A_index_);
}

const FArray1D< core::Real > &
PCS_data_per_lanthanides_Ts1::get_fstyle_b() const{
	return(fstyle_b_);
}


core::Size
PCS_data_Ts1::where_is_line(PCS_line_data_Ts1 & P_l_d){
	...
	filename_(filename), svd_s_(basic::svd::SVD_Solver(PCS_d_l_a.size(), 5)), weight_(weight)
{
	...
	for ( i = 1; i <= PCS_d_l_a.size(); ++i ) {
		PCS_line_data_Ts1 PCS_l_d = PCS_d_l_a[i];
	...
	TR_pcs_d_p_l_Ts1 << "n_pcs: " << n_pcs_ << std::endl;
	TR_pcs_d_p_l_Ts1 << "Normalization 1: " << normalization_1_ << std::endl;
	TR_pcs_d_p_l_Ts1 << "Normalization 2: " << normalization_2_ << std::endl;
	TR_pcs_d_p_l_Ts1 << "Normalization 3: " << normalization_3_ << std::endl;

	if ( option[ basic::options::OptionKeys::PCSTS1::normalization_id ].user() ) {
		core::Size norma_id (option[ basic::options::OptionKeys::PCSTS1::normalization_id ]());

		switch (norma_id){
		case 1 : {
			TR_pcs_d_p_l_Ts1 << "Using Normalization '1': " << normalization_1_ << ". Normalize each data set by SQRT( SUM( PCScalc(i)^2 ) ) " << std::endl;
			normalization_factor_ = normalization_1_;
			break;
		}
		case 2 : {
			TR_pcs_d_p_l_Ts1 << "Using Normalization '2': " << normalization_2_ << ". Normalize each data set by Standard Deviation" << std::endl;
			normalization_factor_ = normalization_2_;
			break;
		}
		case 3 : {
			TR_pcs_d_p_l_Ts1 << "Using Normalization '3': " << normalization_3_ << ". Normalize each data set by SQRT( SUM( PCScalc(i)^2 ) / N) " << std::endl;
			normalization_factor_ = normalization_3_;
			break;
		}
		default : {
			TR_pcs_d_p_l_Ts1 << "Normalization '"<< norma_id << "' id not recognized " << std::endl;
			utility_exit_with_message("You should use a valid normalization id ('1' or '2' or '3')");
			break;
		}
		}
	} else {
		TR_pcs_d_p_l_Ts1 << "Normalization NOT used in calculations " << std::endl;
		normalization_factor_ = 1.0;
	}
}

std::string
PCS_data_per_lanthanides_Ts1::get_filename() const {
	return (filename_);
}

void
PCS_data_per_lanthanides_Ts1::set_A_index(core::Size j,
	core::Size index){
	A_index_[j] = index;
}

PCS_data_Ts1::PCS_data_Ts1(PCS_data_input_Ts1 & P_d_i){
	std::map< std::string, PCS_file_data_Ts1 >::iterator it;

	std::string filename;
	PCS_line_data_Ts1 * P_l_d_temp;
	core::Size index, i, j;

	n_pcs_spin_ = 0;
	n_lanthanides_ = 0;

	std::map< std::string, PCS_file_data_Ts1 > & P_f_a_d = P_d_i.get_PCS_data_input_reference();

	for ( it = P_f_a_d.begin(); it != P_f_a_d.end(); ++it ) {
		filename = it->first;

		PCS_file_data_Ts1 & P_f_d = (it->second);
		n_lanthanides_ = n_lanthanides_ + 1;
		TR_pcs_d_Ts1 << "Filename " << filename  <<std::endl;
		core::Real weight(P_f_d.get_weight());

		utility::vector1<PCS_line_data_Ts1>::iterator it2;
		utility::vector1<PCS_line_data_Ts1> & PCS_d_l_a = P_f_d.get_PCS_data_line_all_reference();

		PCS_data_per_lanthanides_Ts1 P_d_p_l = PCS_data_per_lanthanides_Ts1(filename, weight, PCS_d_l_a );
	...
	TR_pcs_d_Ts1 << "Total spin independent: " << n_pcs_spin_ << std::endl;
...
PCS_data_Ts1::PCS_data_Ts1(PCS_data_input_Ts1 & P_d_i, utility::vector1< bool > const exclude_residues ){
	std::map< std::string, PCS_file_data_Ts1 >::iterator it;
	...
	for ( it = P_f_a_d.begin(); it != P_f_a_d.end(); ++it ) {
		...
		utility::vector1< PCS_line_data_Ts1 >::iterator it2;
		utility::vector1< PCS_line_data_Ts1 > & file_PCS_d_l_a = P_f_d.get_PCS_data_line_all_reference();
		utility::vector1< PCS_line_data_Ts1 > PCS_d_l_a;
...
core::Size
PCS_data_Ts1::get_n_lanthanides() const{
	return (n_lanthanides_);
}

const utility::vector1<core::Real> &
PCS_data_Ts1::get_X_all() const{
	return(X_all_);
}

const utility::vector1<core::Real> &
PCS_data_Ts1::get_Y_all() const{
	return(Y_all_);
}

const utility::vector1<core::Real> &
PCS_data_Ts1::get_Z_all() const{
	return(Z_all_);
}

std::ostream &
operator<<(std::ostream& out, const PCS_data_Ts1 & P_d){
	core::Size i;
...
std::ostream &
operator<<(std::ostream& out, const PCS_data_per_lanthanides_Ts1 &PCS_d_p_l){
	core::Size i, j;
