...
GridSearchIterator_Ts1::GridSearchIterator_Ts1():
	x_center_(0),
	y_center_(0),
	z_center_(0),
	step_(0),
	...
	cone_angle_cos_(0)
{
	utility_exit_with_message( "You shouldn't call the empty constructor for GridSearchIterator_Ts1 class" );
}

GridSearchIterator_Ts1::~GridSearchIterator_Ts1()= default;

GridSearchIterator_Ts1::GridSearchIterator_Ts1(GridSearchIterator_Ts1 const & other):
	VirtualBase(),
...
GridSearchIterator_Ts1::operator=(GridSearchIterator_Ts1 const & other){
	if ( this != &other ) {
		if ( (x_center_ != other.x_center_)||
				...
				(edge_ != other.edge_)||
				(delta_ != other.delta_) ) {
			utility_exit_with_message( "You can't call the = operator on GridSearchIterator_Ts1 object of different size" );
		}
		x_current_ = other.x_current_;
		y_current_ = other.y_current_;
		z_current_ = other.z_current_;
...
GridSearchIterator_Ts1::GridSearchIterator_Ts1(numeric::xyzVector< core::Real > const coo1,
...
bool
GridSearchIterator_Ts1::next_center(core::Real &x,
	core::Real &y,
	core::Real &z){

	//core::Real r2;
	while ( next(x, y, z) == true ) {
		core::Real r2 = (x-x_center_)*(x-x_center_) + (y-y_center_)* (y-y_center_) + (z-z_center_)*(z-z_center_);
...
bool
GridSearchIterator_Ts1::next(core::Real &x,
	core::Real &y,
	core::Real &z){
	x = x_current_;
	y = y_current_;
