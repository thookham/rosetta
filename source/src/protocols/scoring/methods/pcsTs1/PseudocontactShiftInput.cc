...
core::Size
PCS_line_data_Ts1::residue_num() const{
	return residue_num_;
}

std::string
PCS_line_data_Ts1::atom_name() const{
	return atom_name_;
}

core::Real
PCS_line_data_Ts1::PCS_experimental() const{
	return PCS_experimental_;
}


core::Real
PCS_line_data_Ts1::PCS_tolerance() const{
...
PCS_data_input_Ts1::get_PCS_data_input_reference(){
	return  PCS_filename_and_data_;
}

std::string
PCS_file_data_Ts1::get_filename() const{
	return(filename_);
}

core::Real
PCS_file_data_Ts1::get_weight() const{
	return(weight_);
}

PCS_file_data_Ts1::PCS_file_data_Ts1(std::string const & filename, core::Real const my_weight ):
	filename_(std::string(filename)), weight_(my_weight)
{
	read_PCS_file();
}

PCS_data_input_Ts1::PCS_data_input_Ts1(utility::vector1<std::string> const & filenames, utility::vector1<core::Real> const & weight){
	...
	for ( i = 1; i <= filenames.size(); i++ ) {
		...
		PCS_file_data_Ts1 pcs_f_d_temp(filenames[i], my_weight);
		PCS_filename_and_data_.insert ( std::pair< std::string, PCS_file_data_Ts1 >(filenames[i], pcs_f_d_temp) );
	}
}

void
PCS_file_data_Ts1::read_PCS_file(){
	...
	TR_PCS_d_i_Ts1 << "Opening file '" << get_filename().c_str() << "'" << std::endl;
	...
	while ( getline( myfile, line ) ) {
		...
		if ( (line_stream >> residue_num >> atom_name >> PCS_experimental >> PCS_tolerance).fail() ) {
			TR_PCS_d_i_Ts1 << "Ignoring line " <<line_number << ": `" << line << "` from file " << get_filename() <<std::endl;
			continue;
		}

		PCS_data_line_all_.push_back( PCS_line_data_Ts1( residue_num, atom_name, PCS_experimental, PCS_tolerance ) );
	}
...
PCS_file_data_Ts1::get_PCS_data_line_all_reference(){
...
operator<<(std::ostream & out,  const PCS_data_input_Ts1 &PCS_d_i ){
	...
	std::map< std::string, PCS_file_data_Ts1 > mymap;
...
PCS_data_input_Ts1
PCS_data_input_manager_Ts1::get_input_data(utility::vector1<std::string> const & filenames, utility::vector1<core::Real> const & weight){
	std::string id;
	core::Size i;
	...
	std::map< std::string, PCS_data_input_Ts1 >::iterator it;
	...
	PCS_data_input_Ts1 pcs_d_i(filenames, weight);
	it = file_2_data_map_.begin();
	file_2_data_map_.insert(it, std::pair< std::string , PCS_data_input_Ts1 >( id ,pcs_d_i));

	TR_PCS_d_i_Ts1 << pcs_d_i << std::endl;
