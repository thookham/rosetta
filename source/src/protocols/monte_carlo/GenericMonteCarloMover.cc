...
///////////////////////////////////////////////////
static basic::Tracer TR( "protocols.monte_carlo.GenericMonteCarloMover" );
static basic::Tracer TR_energies( "protocols.monte_carlo.GenericMonteCarloMover.individual_energies" );
	...
	if ( reset_baselines() ) {
		...
		for ( FilterOP filter : filters_ ) {
			if ( filter->get_type() == "Operator" ) {
				...
				calc_taskop_filters::OperatorOP operator_filter( utility::pointer::dynamic_pointer_cast< protocols::calc_taskop_filters::Operator > ( filter ) );
				...
				for ( auto & cs_it : *comp_filt_op ) {
					...
					if ( filt->get_type() == "Operator" ) {
						TR<<"Resetting Operator filter's baseline"<<std::endl;
						calc_taskop_filters::OperatorOP operator_filter( utility::pointer::dynamic_pointer_cast< protocols::calc_taskop_filters::Operator > ( filt ) );
						operator_filter->reset_baseline( pose, trial != 1/*if trial>1, attempt to read the baselines from checkpointing files. Otherwise, don't use the checkpointing files*/ );
						call_reset = true;
					}// fi Operator
				}// for cs_it
...
void
GenericMonteCarloMover::provide_citation_info(basic::citation_manager::CitationCollectionList & citations ) const {
