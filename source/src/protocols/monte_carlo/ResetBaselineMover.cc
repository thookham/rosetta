...
// Unit headers
#include <protocols/monte_carlo/ResetBaselineMover.hh>
	...
	if ( filter_type == "Operator" ) {
		...
		for ( auto & cs_it : *comp_filt_op ) {
			...
			if ( filt->get_type() == "Operator" ) {
				TR<<"Resetting Operator filter's baseline"<<std::endl;
				OperatorOP operator_filter( utility::pointer::dynamic_pointer_cast< protocols::calc_taskop_filters::Operator > ( filt ) );
				runtime_assert( operator_filter != nullptr );
