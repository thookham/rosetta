...
#ifndef INCLUDED_protocols_electron_density_DockFragmentsIntoDensityMover_hh
#define INCLUDED_protocols_electron_density_DockFragmentsIntoDensityMover_hh
...
///
///  The workhorse
class DockFragmentsIntoDensityMover : public protocols::moves::Mover {
public:
	...
	// getters as wrappers for multi natives
	utility::vector1< core::pose::PoseCOP >
	get_multi_native() const & {
		if ( native_ ) return {native_};
		else return utility::vector1< core::pose::PoseCOP >();
	}

	utility::vector1< numeric::xyzVector< core::Real > >
	get_multi_native_com() const {
		if ( native_ ) return {native_com_};
		else return utility::vector1< numeric::xyzVector< core::Real > >();
	}

	utility::vector1< numeric::xyzVector< core::Real > >
	get_multi_native_mca() const {
		if ( native_ ) return {native_middle_ca_};
		else return utility::vector1< numeric::xyzVector< core::Real > >();
	}
