	...
	for ( int i = 1; i <= nres; i++ ) {
		...
		for ( int j = 1; j <=nres; j++ ) {
			...
			if ( CA_dist < CA_cutoff ) {
				ConstraintCOP cst( utility::pointer::make_shared< AtomPairConstraint >( AtomID(pose.residue(i).atom_index(" CA "),i),AtomID(pose.residue(j).atom_index(" CA "),j), utility::pointer::make_shared< core::scoring::func::HarmonicFunc >(CA_dist, cst_tol) ) );
