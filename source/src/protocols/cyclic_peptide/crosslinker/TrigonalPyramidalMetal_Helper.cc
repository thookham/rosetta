...
/// @file protocols/cyclic_peptide/crosslinker/TrigonalPyramidalMetal_Helper.cc
/// @brief A helper class for setting up trigonal pyramidally-coordinated metals
/// like Zn or Cu.
...
TrigonalPyramidalMetal_Helper::TrigonalPyramidalMetal_Helper(
...
////////////////////////////////////////////////////////////////////////////////
TrigonalPyramidalMetal_Helper::TrigonalPyramidalMetal_Helper( TrigonalPyramidalMetal_Helper const &src ) :
...
void
TrigonalPyramidalMetal_Helper::check_residue_indices_valid(
	utility::vector1< core::Size > const &indices,
	core::pose::Pose const &pose
) const {
	static const std::string errmsg( "Error in protocols::cyclic_peptide::crosslinker::TrigonalPyramidalMetal_Helper::check_residue_indices_valid(): " );
	runtime_assert_string_msg( indices.size() == 3, errmsg + "The number of side-chains needed for trigonal pyramidal metal coordination is THREE." );
...
/// @brief Given a residue type, check whether it's an allowed residue type for trigonal pyramidally coordinating metals.
/// @details Returns "true" for pass (allowed type) and "false" for failure (prohibited type).  Currently, allowed types are
/// L- and D-histidine, L- or D-aspartate, L- or D-glutamate, and the beta-3-amino acid equivalents.
bool
TrigonalPyramidalMetal_Helper::is_allowed_type(
...
void
TrigonalPyramidalMetal_Helper::add_angle_constraints(
	core::pose::Pose &pose,
	...
	static std::string const circularharmonic_string(" CIRCULARHARMONIC 1.9106332359 0.017453292522222223\n"); //Static const data are threadsafe under the C++11 standard.
...
core::Real const &
TrigonalPyramidalMetal_Helper::ideal_bond_length(
	...
	runtime_assert_string_msg( data != ideal_bond_lengths.end(), "Error in protocols::cyclic_peptide::crosslinker::TrigonalPyramidalMetal_Helper::ideal_bond_length(): Trigonal pyramidal coordination information is unavailable for the given ligand type when it binds to " + metal_type_string_from_enum(metal_type) + "." );
...
void
TrigonalPyramidalMetal_Helper::check_compatible_symmetry_type() const {
	bool passed(false);
	if ( symm_type() == 'C' ) {
		if ( symm_count() == 3 ) {
			passed = true;
	...
	runtime_assert_string_msg(passed, "Error in protocols::cyclic_peptide::crosslinker::TrigonalPyramidalMetal_Helper::check_compatible_symmetry_type(): Trigonal pyramidal metal crosslinks are only compatible with C3 symmetry.");
}
