...
/// @file protocols/cyclic_peptide/crosslinker/SquarePlanarMetal_Helper.cc
...
#include <utility/fixedsizearray1.hh> // AUTO IWYU For fixedsizearray1, fixedsizearray1<>::valu...

static basic::Tracer TR( "protocols.cyclic_peptide.crosslinker.SquarePlanarMetal_Helper" );
...
SquarePlanarMetal_Helper::SquarePlanarMetal_Helper(
...
////////////////////////////////////////////////////////////////////////////////
SquarePlanarMetal_Helper::SquarePlanarMetal_Helper( SquarePlanarMetal_Helper const &src ) :
	Metal_HelperBase( src ),
	is_pyramidal_(src.is_pyramidal_)
	//TODO copy data here
{
...
void
SquarePlanarMetal_Helper::provide_citation_info(
	basic::citation_manager::CitationCollectionList & citations
) const {
	basic::citation_manager::CitationCollectionOP cc(
		utility::pointer::make_shared< basic::citation_manager::CitationCollection >(
		"SquarePlanarMetal_Helper", basic::citation_manager::CitedModuleType::CrosslinkerMoverHelper
		)
	);
	cc->add_citation(
...
void
SquarePlanarMetal_Helper::check_residue_indices_valid(
	utility::vector1< core::Size > const &indices,
	core::pose::Pose const &pose
) const {
	static const std::string errmsg( "Error in protocols::cyclic_peptide::crosslinker::SquarePlanarMetal_Helper::check_residue_indices_valid(): " );
	runtime_assert_string_msg( indices.size() == 4, errmsg + "The number of side-chains needed for square planar metal coordination is FOUR." );
...
/// @brief Given a residue type, check whether it's an allowed residue type for square planarly coordinating metals.
...
bool
SquarePlanarMetal_Helper::is_allowed_type(
...
void
SquarePlanarMetal_Helper::add_angle_constraints(
	core::pose::Pose &pose,
	...
	if ( is_pyramidal_ ) {
		runtime_assert_string_msg( res_indices.size() == 5, "Error in SquarePlanarMetal_Helper::add_angle_constraints(): The number of selected residues must be FIVE for the square pyramidal case.");
	} else {
		runtime_assert_string_msg( res_indices.size() == 4, "Error in SquarePlanarMetal_Helper::add_angle_constraints(): The number of selected residues must be FOUR for the square planar case.");
	}
	...
	static std::string const circularharmonic_string1(" CIRCULARHARMONIC 1.570796327 0.017453292522\n"); //Static const data are threadsafe under the C++11 standard.
	static std::string const circularharmonic_string2(" CIRCULARHARMONIC 3.141592654 0.017453292522\n"); //Static const data are threadsafe under the C++11 standard.

	cststring << "AmbiguousConstraint\n";
	for ( core::Size ii(1); ii<=2; ++ii ) {
		std::string const &circularharmonic_string( ii == 1 ? circularharmonic_string1 : circularharmonic_string2 );
...
void
SquarePlanarMetal_Helper::add_dihedral_constraints(
	core::pose::Pose &pose,
	utility::vector1< core::Size > const &res_indices
) const {
	std::stringstream cststring;
...
/// @details Can be a single constraint or a block of ambiguous constraints for histidines.  Useful for constructing a larger
/// ambiguous constraint (e.g. by derived class).
void
SquarePlanarMetal_Helper::add_dihedral_constraint_to_stream(
	std::stringstream &cststream,
	core::pose::Pose const &pose,
	utility::vector1< core::Size > const &res_indices
) const {
	static std::string const errmsg( "Error in SquarePlanarMetal_Helper::add_dihedral_constraint_to_stream(): " );
	runtime_assert_string_msg( res_indices.size() == 4, errmsg + "The number of selected residues must be FOUR.");
	static std::string const circularharmonic_string(" CIRCULARHARMONIC 0.0 0.052359877566666664\n"); //Static const data are threadsafe under the C++11 standard.

	utility::fixedsizearray1< core::chemical::ResidueTypeCOP, 4 > const rtypes {  pose.residue_type_ptr(res_indices[1]),  pose.residue_type_ptr(res_indices[2]),  pose.residue_type_ptr(res_indices[3]),  pose.residue_type_ptr(res_indices[4]) };
	utility::fixedsizearray1< bool, 4 > is_his;
	for ( core::Size i(1); i<=4; ++i ) {
		is_his[i] = ( rtypes[i]->aa() == core::chemical::aa_his || rtypes[i]->aa() == core::chemical::aa_dhi );
	}
	bool const any_his( is_his[1] || is_his[2] || is_his[3] || is_his[4] );
	static utility::fixedsizearray1< utility::fixedsizearray1 < core::Size, 3 >, 12 > const res_permutations {
		...
		utility::fixedsizearray1< core::Size, 3 > { 4, 2, 3 },
		utility::fixedsizearray1< core::Size, 3 > { 3, 4, 2 },
		utility::fixedsizearray1< core::Size, 3 > { 2, 3, 4 },
		utility::fixedsizearray1< core::Size, 3 > { 1, 4, 3 },
		utility::fixedsizearray1< core::Size, 3 > { 4, 3, 1 },
		utility::fixedsizearray1< core::Size, 3 > { 3, 1, 4 },
		utility::fixedsizearray1< core::Size, 3 > { 1, 2, 4 },
		utility::fixedsizearray1< core::Size, 3 > { 2, 4, 1 },
		utility::fixedsizearray1< core::Size, 3 > { 4, 1, 2 }
		};
	for ( core::Size permutation(1); permutation <= 12; ++permutation ) {
		if ( any_his ) {
			cststream << "AmbiguousConstraint\n";
		}
		for ( core::Size i(1), imax( is_his[res_permutations[permutation][1]] ? 2 : 1 ); i<=imax; ++i ) {
			...
			for ( core::Size j(1), jmax( is_his[res_permutations[permutation][2]] ? 2 : 1 ); j<=jmax; ++j ) {
				...
				for ( core::Size k(1), kmax( is_his[res_permutations[permutation][3]] ? 2 : 1 ); k<=kmax; ++k ) {
					...
					cststream << "Dihedral " << at1 << " " << res_indices[res_permutations[permutation][1]] << " " << at2 << " " << res_indices[res_permutations[permutation][2]] << " "<< at3 << " " << res_indices[res_permutations[permutation][3]] << " VM1 " << res_indices[res_permutations[permutation][1]] << " " << circularharmonic_string;
				}
			}
		}
		if ( any_his ) {
...
core::Real const &
SquarePlanarMetal_Helper::ideal_bond_length(
	...
	runtime_assert_string_msg( data != ideal_bond_lengths.end(), "Error in protocols::cyclic_peptide::crosslinker::SquarePlanarMetal_Helper::ideal_bond_length(): Square planar coordination information is unavailable for the given ligand type when it binds to " + metal_type_string_from_enum(metal_type) + "." );

	return data->second;
}

/// @brief Check that the symmetry type is one of a few compatible types.
/// @details Allowed types are C4.
void
SquarePlanarMetal_Helper::check_compatible_symmetry_type() const {
	bool passed(false);
	if ( symm_type() == 'C' || symm_type() == 'S' ) {
		if ( symm_count() == 4 || symm_count() == 2 ) {
			passed = true;
		}
	} else if ( symm_type() == 'D' && symm_count() == 2 ) {
		passed = true;
	}
	runtime_assert_string_msg(passed, "Error in protocols::cyclic_peptide::crosslinker::SquarePlanarMetal_Helper::check_compatible_symmetry_type(): Square planar metal crosslinks are only compatible with C3 symmetry.");
}
