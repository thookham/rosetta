...
FlexPepDockingAbInitio::apply( core::pose::Pose & pose )
{
	...
	core::Real trans_mag = 1; // Angstrom // TODO: runtime param?
	core::Real rot_mag = 10; // Degrees // TODO: runtime param?
	core::Real rb_acceptance, torsions_acceptance; // MC acceptance rates
	// MC temperature params
	core::Real init_MC_temp = 2;
	core::Real final_MC_temp = 0.6;
	core::Real gamma = pow( (final_MC_temp / init_MC_temp) , 1.0 / (outer_cycles - 1.0) );
...
FlexPepDockingAbInitio::convertPSERtoASP(core::pose::Pose& pose, std::set<int>& pSer_positions)
{
	...
	ResidueTypeSetCOP rsd_set( pose.residue_type_set_for_pose( FULL_ATOM_t ) );
	for ( int resid = 1; resid <= (int)pose.size(); resid++ ) {
...
FlexPepDockingAbInitio::restorePSER(core::pose::Pose& pose, std::set<int> const& pSer_positions)
{
	using namespace core::chemical;
	using namespace core::conformation;
	ResidueTypeSetCOP centroid_set( pose.residue_type_set_for_pose( CENTROID_t ) );
