...
SplitAndMixPoseMover::apply_without_merge( core::pose::Pose const & pose )
{
	TR << "Using secondary 'apply_without_merge' method " << std::endl;
	count_segments( pose );
	utility::vector1< core::pose::PoseOP > pose_list = split_pose( pose );
	utility::vector1< core::pose::PoseOP > final_list;
	if ( pose_list.size() > 0 ) {
		...
		for ( core::Size i=1; i<=order_.size(); ++i ) {
			final_list.push_back( pose_list[ order_[i] ] );
		}
	} else {
		throw CREATE_EXCEPTION(utility::excn::Exception, "Selector does not match with any residue of the provided pose");
	}
...
SplitAndMixPoseMover::split_pose( core::pose::Pose const & pose )
{
	...
	core::pose::PoseOP empty_pose( new core::pose::Pose );
	...
	for ( auto const & range : *ranges_ ) {
		if ( range.start() != 0 ) {
			...
			TR << "Adding an empty pose" << std::endl;
			pose_list.push_back( empty_pose );
		}
	}
	if ( TR.visible() ) {
		for ( core::Size i=1; i<= order_.size(); ++i ) {
