...
/// @file protocols/legacy_sewing/sampling/LegacyEnumerateAssemblyMover.cc
///
...
// Unit Headers
#include <protocols/legacy_sewing/sampling/LegacyEnumerateAssemblyMover.hh>
...
/////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////  LegacyEnumerateAssemblyMover function   //////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////////////////
...
AssemblyOP
LegacyEnumerateAssemblyMover::generate_assembly(){
	TR << "LegacyEnumerateAssemblyMover::generate_assembly()" << std::endl;
	using namespace basic::options;

	/* pseudo code for LegacyEnumerateAssemblyMover::follow_edge_from_node
	...
	if ( option[basic::options::OptionKeys::legacy_sewing::score_file_name].user() &&
		...
		utility_exit_with_message("You must give a model file and score file to an LegacyEnumerateAssemblyMover either through options or tags");
	}
	/////// <end> get model_file, edge_file

	core::Size max_ss_num = option[basic::options::OptionKeys::legacy_sewing::max_ss_num].def(5);
...
std::string LegacyEnumerateAssemblyMover::get_name() const {
	return mover_name();
}

std::string LegacyEnumerateAssemblyMover::mover_name() {
	return "LegacyEnumerateAssemblyMover";
}

void LegacyEnumerateAssemblyMover::provide_xml_schema( utility::tag::XMLSchemaDefinition & xsd )
{
	using namespace utility::tag;
	//Extra attributes
	AttributeList extra_ats;
	extra_ats
		+ XMLSchemaAttribute( "min_assembly_score", xsct_real, "Minimum assembly score cutoff (-2.0 recommended)" );

	XMLSchemaComplexTypeGeneratorOP ct_gen = LegacyAssemblyMover::define_assembly_mover_ct_gen( xsd );
	ct_gen->element_name( mover_name() );
	ct_gen->description( "Enumerates all possible assemblies for the model/edge file and keeps those with assembly scores better than a cutoff value" );
...
std::string LegacyEnumerateAssemblyMoverCreator::keyname() const {
	return LegacyEnumerateAssemblyMover::mover_name();
}
...
{
	LegacyEnumerateAssemblyMover::provide_xml_schema( xsd );
}
