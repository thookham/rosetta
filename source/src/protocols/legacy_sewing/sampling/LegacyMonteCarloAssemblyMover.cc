...
/// @file LegacyMonteCarloAssemblyMover.cc
///
...
LegacyMonteCarloAssemblyMover::LegacyMonteCarloAssemblyMover():
	cycles_(10000),
...
AssemblyOP
LegacyMonteCarloAssemblyMover::generate_assembly(){
	// TR << "LegacyMonteCarloAssemblyMover::generate_assembly()" << std::endl;
...
bool
LegacyMonteCarloAssemblyMover::add_edge(
	...
	if ( TR.Debug.visible() ) { TR << "LegacyMonteCarloAssemblyMover::add_edge" << std::endl;}
	...
	if ( assembly_list.size() == 1 ) {
		...
		Alignment size 40
		ERROR:: Exit from: src/protocols/legacy_sewing/conformation/Assembly.cc line: 834
		*/

		// as of 2016/1/18, turning off may_add_already_added_model during LegacyRepeatAssemblyMover resulted in
		/*[ERROR] Exception caught by JobDistributor for job RepeatAssembly__0001
		[ERROR] EXCN_utility_exit has been thrown from: src/protocols/legacy_sewing/conformation/Assembly.cc line: 834
...
void
LegacyMonteCarloAssemblyMover::delete_edge(
	AssemblyOP & assembly,
	utility::vector1<AssemblyOP> & assembly_list
) const {
	if ( TR.Debug.visible() ) { TR << "LegacyMonteCarloAssemblyMover::delete_edge" << std::endl;}
...
bool
LegacyMonteCarloAssemblyMover::switch_edge(
	AssemblyOP const & pre_op_assembly,
	utility::vector1<AssemblyOP> const & pre_op_assembly_list,
	AssemblyOP & assembly,
	utility::vector1<AssemblyOP> & assembly_list
...
void
LegacyMonteCarloAssemblyMover::boltzman(
	utility::vector1<AssemblyOP> const & pre_op_assembly_list,
	utility::vector1<AssemblyOP> & assembly_list,
	AssemblyOP const & pre_op_assembly,
	AssemblyOP & working_assembly,
	...
	if ( numeric::random::rg().uniform() < probability ) {
		...
		if ( check_completeness_of_assembly ) {
			...
			if ( remove_non_contiguous_assembly_ ) { // remove incomplete (cut-off) assembly (mostly comes from inherent error in pdb file), in the future, removing incomplete model at model extration step will be pursued
				...
				for ( core::Size ii=1; ii < to_be_checked_pose.size(); ii++ ) {
...
std::string LegacyMonteCarloAssemblyMover::get_name() const {
	return mover_name();
}

std::string LegacyMonteCarloAssemblyMover::mover_name() {
	return "LegacyMonteCarloAssemblyMover";
}

void LegacyMonteCarloAssemblyMover::provide_xml_schema( utility::tag::XMLSchemaDefinition & xsd )
{
	using namespace utility::tag;
	XMLSchemaComplexTypeGeneratorOP ct_gen = LegacyMonteCarloAssemblyMover::define_mc_assembly_mover_ct_gen( xsd );
	ct_gen->element_name( mover_name() );
	ct_gen->description( "Builds an assembly using a Monte Carlo simulated annealing protocol with the given temperature schedule and probabilities of adding/deleting/switching segments each cycle" );
	ct_gen->write_complex_type_to_schema( xsd );
}

utility::tag::XMLSchemaComplexTypeGeneratorOP
LegacyMonteCarloAssemblyMover::define_mc_assembly_mover_ct_gen( utility::tag::XMLSchemaDefinition & xsd ){
	...
	extra_ats
		+ XMLSchemaAttribute( "cycles", xsct_non_negative_integer, "Number of Monte Carlo cycles to perform" )
		+ XMLSchemaAttribute( "max_temperature", xsct_real, "Initial kT value for simulated annealing (1.5 to 2 recommended)" )
		+ XMLSchemaAttribute( "min_temperature", xsct_real, "Final kT value for simulated annealing (0.1 recommended)" )
		+ XMLSchemaAttribute( "min_assembly_score", xsct_real, "Assemblies with scores better than this value will be kept; -2.0 recommended" )
		+ XMLSchemaAttribute( "use_best_assembly_score", xsct_rosetta_bool, "Recover lowest-scoring assembly?" )
		+ XMLSchemaAttribute( "add_probability", xsct_real, "Probability of adding a segment in a given cycle" )
		+ XMLSchemaAttribute( "delete_probability", xsct_real, "Probability of deleting a segment in a given cycle" )
		+ XMLSchemaAttribute( "switch_probability", xsct_real, "Probability of performing a switch in a given cycle" )
		+ XMLSchemaAttribute( "remove_non_contiguous_assembly", xsct_rosetta_bool, "Discard assemblies with missing residues" );
...
std::string LegacyMonteCarloAssemblyMoverCreator::keyname() const {
	return LegacyMonteCarloAssemblyMover::mover_name();
}
...
{
	LegacyMonteCarloAssemblyMover::provide_xml_schema( xsd );
}
