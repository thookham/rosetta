...
{
	return utility::pointer::make_shared< SS_Info2 >( *this );
}
...
SS_Info2::identify_ss( String const & secstruct )
{
	...
	for ( core::Size i=1; i<= secstruct.length(); ++i ) {
		...
		if ( ss !="E" && flag_E == true ) {
			flag_E = false;
			if ( ( i - beginE ) >= 2 ) {
				strands_.push_back( utility::pointer::make_shared< Strand >( beginE, i-1 ) );
			} else {
				strands_.push_back( utility::pointer::make_shared< Strand >( beginE, beginE ) );
			}
		}
		...
		if ( ss !="H" && flag_H == true ) {
			flag_H = false;
			if ( ( i-beginH ) >= 2 ) {
				helices_.push_back( utility::pointer::make_shared< Helix >( beginH, i-1 ) );
			} else {
				helices_.push_back( utility::pointer::make_shared< Helix >( beginH, beginH ) );
			}
		}
		...
		if ( ss !="L" && flag_L == true ) {
			flag_L = false;
			if ( ( i-beginL ) >= 2 ) {
				loops_.push_back( utility::pointer::make_shared< Loop >( beginL, i-1 ) );
			} else {
				loops_.push_back( utility::pointer::make_shared< Loop >( beginL, beginL ) );
			}
		}
	...
	if ( flag_E == true ) {
		if ( ( secstruct.length() - beginE + 1 ) >= 2 ) {
			strands_.push_back( utility::pointer::make_shared< Strand >( beginE, secstruct.length() ) );
		} else {
			strands_.push_back( utility::pointer::make_shared< Strand >( beginE, beginE ) );
		}
	}
	if ( flag_H == true ) {
		if ( ( secstruct.length() - beginH + 1 ) >= 2 ) {
			helices_.push_back( utility::pointer::make_shared< Helix >( beginH, secstruct.length() ) );
		} else {
			helices_.push_back( utility::pointer::make_shared< Helix >( beginH, beginH ) );
		}
	}
	if ( flag_L == true ) {
		if ( ( secstruct.length() - beginL + 1 ) >= 2 ) {
			loops_.push_back( utility::pointer::make_shared< Loop >( beginL, secstruct.length() ) );
		} else {
			loops_.push_back( utility::pointer::make_shared< Loop >( beginL, beginL ) );
		}
	}
