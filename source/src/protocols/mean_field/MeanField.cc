...
MeanField::MeanField( core::Size option,
...
MeanField::converge ()
{
	...
	while ( ! has_converged() && counter <= core::Size( 1000 ) ) //doesn't allow it to try to converge ad infinitum. may have to raise max rounds later.
			{
		...
		for ( core::Size pos = 1 ; pos <= rot_matrix_->size() ; ++pos ) {
			for ( core::Size rot = 1 ; rot <= (*rot_matrix_)[ pos ].size() ; ++rot ) {
...
void
MeanField::build_rot_matrix_for_pose( core::Size pose_ind )
{
...
void
MeanField::delete_pose( core::Size pose_ind )
{
...
MeanField::calc_energies_matrix ()
{
	...
	for ( core::Size node = 1; node <= rot_matrix_->size(); ++node ) {
		for ( core::Size state = 1 ; state <= (*rot_matrix_)[ node ].size(); ++state ) {
	...
	for ( core::Size node = 1; node <= rot_matrix_->size(); ++node ) {
		...
		for (  pig_->reset_edge_list_iterator_for_node( node ) ; !pig_->edge_list_iterator_at_end() ; pig_->increment_edge_list_iterator() ) {
			...
			core::Size node2 = pedge.get_other_ind( node );
			...
			for ( core::Size state = 1; state <= (*rot_matrix_)[ node ].size(); ++state ) {
				...
				for ( core::Size state2 = 1 ; state2 <= (*rot_matrix_)[ node2 ].size(); ++state2 ) {
...
MeanField::convert_energies_to_alt_rm ()
{
	...
	for ( core::Size pos = 1 ; pos <= energies_matrix_.size() ; ++pos ) {
		for ( core::Size rot = 1 ; rot <= energies_matrix_[ pos ].size() ; ++rot ) {
