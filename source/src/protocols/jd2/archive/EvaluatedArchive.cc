...
void EvaluatedArchive::set_weight( std::string const& column, core::Real weight ) {
	...
	if ( weight > 0.01 ) {
		...
		invalidate_score_variations(); // NOLINT(clang-analyzer-optin.cplusplus.VirtualCall) - When set from the constructor, this doesn't necessarily invoke the derived class's version
	}
}
