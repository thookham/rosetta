...
public:
	...
	/// @brief this function is intended for saving mid-protocol poses; for example the final
	/// centroid structure before the fullatom stage begins in a combined centroid / fullatom
	/// protocol.
	virtual
	void other_pose(
		JobOP job,
	...
	/// @brief optionally pass a starting (reference) pose to a JobOutputter for later comparison
	...
	/// @brief this function is not used for output, but it belongs here since it needs to check
	/// the same output locations as the class normally writes to.  This class checks wherever
	/// output goes to see if the job's expected output already exists (on disk or whatever).
	/// This is the most basic form of checkpointing.
	virtual
	bool job_has_completed( JobCOP job ) = 0;

	/// @brief this is the master function for determining the unique output identifier for a
...
protected:
	...
	/// @brief this function generates the affixed, numbered name for the job as
	/// prefix + input + suffix + number (affixes from options system).  This
	/// function is deliberately not virtual, this should be a common mechanism;
	/// your JobOutputter can not call it if it would like.
	std::string affixed_numbered_name( JobCOP job );

private:
	/// @brief operator= defined for the sake of the remove-headers-in-headers initiative.
	/// As of this writing, there is no reason to actually call this function, so it is
	/// declared but UNIMPLEMENTED to force compiler errors if you try to do this (instead
	/// of allowing the compiler to autogenerate the code).  If you have a valid need for
	/// this function, feel free to implement it properly and make it a public function.
