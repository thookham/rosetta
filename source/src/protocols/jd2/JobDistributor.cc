...
bool JobDistributor::using_parser() const {
	return parser_->protocol_option_set( basic::options::option );
}
...
)
{
	...
	if ( ! using_parser()
		...
		try {
			...
			bool guarantee_new_mover = option[OptionKeys::run::reinitialize_mover_for_each_job];

			if (
					first_job ||
					! option[OptionKeys::jd2::parse_script_once_only]() ||
					guarantee_new_mover
					) {
				tr.Trace << "Generating mover from XML" << std::endl;
				parser_->generate_mover( mover_copy, new_input,
	...
	try {
		...
		if ( tr.visible() ) { //Skip this if we're not going to write them out anyways.
			...
			mover_copy->provide_citation_info( citations );
