...
void PeakFileFormat::set_format_from_peak( CrossPeak const& cp ) {
	info1_ = utility::pointer::make_shared< CrossPeakInfo >( cp.info( 1 ) );
	info2_ = utility::pointer::make_shared< CrossPeakInfo >( cp.info( 2 ) );
	col2proton_.clear();
	col2islabel_.clear();
...
void PeakFileFormat::read_header( std::istream& is, std::string& next_line ) {
	...
	for ( core::Size i = 1; i<=dim; i++ ) {
		...
		if ( atom_names[ i ] == "h"
			...
			if ( !info2_ ) {
				info2_ = utility::pointer::make_shared< CrossPeakInfo >( uppercased( atom_names[ i ] ), "", max_noe_dist, tolerances[ i ], 0.0 );
			} else {
			...
			if ( tolerances[ i ]==0.0 ) tolerances[ i ]=default_tolerance_H;
			if ( !info1_ ) {
				info1_ = utility::pointer::make_shared< CrossPeakInfo >( atom_names[ i ], "", max_noe_dist, tolerances[ i ], 0.0 );
			} else {
			...
			if ( !info2_ ) {
				info2_ = utility::pointer::make_shared< CrossPeakInfo >( "", uppercased( atom_names[ i ] ), max_noe_dist, 0.0, tolerances[ i ] );
			} else {
			...
			if ( tolerances[ i ]==0.0 ) tolerances[ i ]= option[ noesy_weights::tolerances ][ TOL_LABEL ];
			if ( !info1_ ) {
				info1_ = utility::pointer::make_shared< CrossPeakInfo >( "", atom_names[ i ], max_noe_dist, 0.0, tolerances[ i ] );
			} else {
...
CrossPeakOP PeakFileFormat::read_peak( std::istream& is, std::string& next_line ) const {
	...
	if ( ncol == 2 ) {
		cp = utility::pointer::make_shared< CrossPeak >();
	} else if ( ncol == 3 ) {
		cp = utility::pointer::make_shared< CrossPeak3D >();
	} else {
		cp = utility::pointer::make_shared< CrossPeak4D >();
	}
