...
CrossPeak::create_fa_and_cen_constraint(
	...
	if ( ct_ambiguous > 1 ) {
		...
		for ( auto it = first_valid; it != end(); ++it ) {
			if ( (*it)->normalized_peak_volume() <= min_vc ) continue; //not enough contribution
		...
		if ( !fa_only ) {
			...
			if ( map_to_CEN ) {
				...
				mapped_inv_weight = inv_weight * pow( 2.0, core::Real(max_maps) );
			} else {
		...
		if ( !fa_only ) {
			...
			if ( map_to_CEN ) {
				...
				mapped_inv_weight = inv_weight*pow( 2.0, core::Real(number_of_maps) );
			} else {
...
bool CrossPeak::eliminated( bool recompute, bool do_not_compute ) const {
	...
	for ( auto it = begin(); it != end() && !min_peak_volume_reached ; ++it ) {
