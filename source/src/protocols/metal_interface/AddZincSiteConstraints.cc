...
AddZincSiteConstraints::add_constraints( pose::Pose & pose ) {
	...
	for ( core::Size i(2); i <= msr_.size(); ++i ) { // zinc is 1
		...
		if ( resn == "CYS" ) { // CYS
			...
			distance_constraints_.push_back( utility::pointer::make_shared< AtomPairConstraint >( atom_ids[i], zinc_id, S_dist_func ) );
			...
			angle_constraints_.push_back( utility::pointer::make_shared< AngleConstraint >( pre_atom_ids[i], atom_ids[i], zinc_id, S_angle_func ) );
			...
			distance_constraints_.push_back( utility::pointer::make_shared< AtomPairConstraint >( atom_ids[i], zinc_id, N_dist_func ) );
			pose.add_constraint( distance_constraints_[ distance_constraints_.size() ] );

			TR << "Adding His angle_constraint " << pre_atom_ids[i] << " " << atom_ids[i] << " " << zinc_id << std::endl;
			angle_constraints_.push_back( utility::pointer::make_shared< AngleConstraint >( pre_atom_ids[i], atom_ids[i], zinc_id, N_angle_func ) );
			pose.add_constraint( angle_constraints_[ angle_constraints_.size() ] );
		} else if ( resn == "ASP" || resn == "GLU" ) { // ASP/GLU
			TR << "Adding Asp/Glu atom_pair_constraint " << atom_ids[i] << " " << zinc_id << std::endl;
			distance_constraints_.push_back( utility::pointer::make_shared< AtomPairConstraint >( atom_ids[i], zinc_id, O_dist_func ) );
			pose.add_constraint( distance_constraints_[ distance_constraints_.size() ] );

			TR << "Adding Asp/Glu angle_constraint " << pre_atom_ids[i] << " " << atom_ids[i] << " " << zinc_id << std::endl;
			angle_constraints_.push_back( utility::pointer::make_shared< AngleConstraint >( pre_atom_ids[i], atom_ids[i], zinc_id, O_angle_func ) );
			pose.add_constraint( angle_constraints_[ angle_constraints_.size() ] );
		}
	}
	...
	if ( msr_.size() == 5 ) {
		AngleConstraintCOP tetr1_constraint( utility::pointer::make_shared< AngleConstraint >( msr_[2]->get_ligand_atom_id(), zinc_id, msr_[3]->get_ligand_atom_id(), tetr_func ) );
		AngleConstraintCOP tetr2_constraint( utility::pointer::make_shared< AngleConstraint >( msr_[2]->get_ligand_atom_id(), zinc_id, msr_[4]->get_ligand_atom_id(), tetr_func ) );
		AngleConstraintCOP tetr3_constraint( utility::pointer::make_shared< AngleConstraint >( msr_[2]->get_ligand_atom_id(), zinc_id, msr_[5]->get_ligand_atom_id(), tetr_func ) );
		AngleConstraintCOP tetr4_constraint( utility::pointer::make_shared< AngleConstraint >( msr_[3]->get_ligand_atom_id(), zinc_id, msr_[4]->get_ligand_atom_id(), tetr_func ) );
		AngleConstraintCOP tetr5_constraint( utility::pointer::make_shared< AngleConstraint >( msr_[3]->get_ligand_atom_id(), zinc_id, msr_[5]->get_ligand_atom_id(), tetr_func ) );
		AngleConstraintCOP tetr6_constraint( utility::pointer::make_shared< AngleConstraint >( msr_[4]->get_ligand_atom_id(), zinc_id, msr_[5]->get_ligand_atom_id(), tetr_func ) );
		...
		AngleConstraintCOP tetr1_constraint( utility::pointer::make_shared< AngleConstraint >( msr_[2]->get_ligand_atom_id(), zinc_id, msr_[3]->get_ligand_atom_id(), tetr_func ) );
		AngleConstraintCOP tetr2_constraint( utility::pointer::make_shared< AngleConstraint >( msr_[2]->get_ligand_atom_id(), zinc_id, msr_[4]->get_ligand_atom_id(), tetr_func ) );
		AngleConstraintCOP tetr3_constraint( utility::pointer::make_shared< AngleConstraint >( msr_[3]->get_ligand_atom_id(), zinc_id, msr_[4]->get_ligand_atom_id(), tetr_func ) );
		tetrahedral_constraints_.push_back( tetr1_constraint );
		tetrahedral_constraints_.push_back( tetr2_constraint );
		tetrahedral_constraints_.push_back( tetr3_constraint );
		pose.add_constraint( tetr1_constraint );
	...
	for ( core::Size i(2); i <= msr_.size(); ++i ) { // zinc is 1
		...
		if ( msr_[i]->get_ligand_atom_name() == " ND1" ) {
			...
			dihedral_func = utility::pointer::make_shared< core::scoring::func::CircularHarmonicFunc >( ND1_dihedral, ND1_dev );
			dihedral_constraints_.push_back( utility::pointer::make_shared< DihedralConstraint >( id1, id2, id3, id4, dihedral_func ) );
			...
			dihedral_func = utility::pointer::make_shared< core::scoring::func::CircularHarmonicFunc >( NE2_dihedral, NE2_dev );
			dihedral_constraints_.push_back( utility::pointer::make_shared< DihedralConstraint >( id1, id2, id3, id4, dihedral_func ) );
			pose.add_constraint( dihedral_constraints_[ dihedral_constraints_.size() ] );
			...
			dihedral_func = utility::pointer::make_shared< core::scoring::func::MinMultiHarmonicFunc >( SG_dihedrals, SG_devs );
			dihedral_constraints_.push_back( utility::pointer::make_shared< DihedralConstraint >( id1, id2, id3, id4, dihedral_func ) );
			pose.add_constraint( dihedral_constraints_[ dihedral_constraints_.size() ] );
		}
