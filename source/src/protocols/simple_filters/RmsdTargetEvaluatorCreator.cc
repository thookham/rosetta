...
static basic::Tracer tr( "protocols.evalution.RmsdTargetEvaluatorCreator" );
	...
	if ( option[ OptionKeys::evaluation::rmsd_target ].user() ) {
		...
		*/
		...
		for ( core::Size ct = 1; ct <= rmsd_target.size(); ct ++ ) {
			...
			eval.add_evaluation( utility::pointer::make_shared< simple_filters::SelectRmsdEvaluator >( rmsd_pose, tag ) );
			if ( option[ OptionKeys::evaluation::gdtmm ]() ) eval.add_evaluation( utility::pointer::make_shared< simple_filters::SelectGdtEvaluator >( rmsd_pose, tag ) );
			if ( option[ OptionKeys::evaluation::score_with_rmsd ]() ) {
				...
				eval.add_evaluation( utility::pointer::make_shared< simple_filters::TruncatedScoreEvaluator >( tag, vector ) );
			}
			if ( option[ OptionKeys::evaluation::symmetric_rmsd ]() ) {
				eval.add_evaluation( utility::pointer::make_shared< simple_filters::SymmetricRmsdEvaluator >( rmsd_pose, tag ) );
			}
		}
	}
