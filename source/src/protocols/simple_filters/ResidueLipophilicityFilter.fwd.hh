// -*- mode:c++;tab-width:2;indent-tabs-mode:t;show-trailing-whitespace:t;rm-trailing-spaces:t -*-
// vi: set ts=2 noet:
//
// (c) Copyright Rosetta Commons Member Institutions.
// (c) This file is part of the Rosetta software suite and is made available under license.
// (c) The Rosetta software is developed by the contributing members of the Rosetta Commons.
// (c) For more information, see http://www.rosettacommons.org. Questions about this can be
// (c) addressed to University of Washington UW TechTransfer, email: license@u.washington.edu.

/// @file   protocols/simple_filters/ResidueLipophilicityFilter.fwd.hh
/// @brief returns the value calculated by the energy term mp_res_lipo.
/// useful for understanding and debugging this energy term. use the output_file tag
/// to print a full table of parameters from within the energy term
/// @author Jonathan Weinstein (jonathan.weinstein@weizmann.ac.il)

#ifndef INCLUDED_protocols_simple_filters_ResidueLipophilicityFilter_fwd_hh
#define INCLUDED_protocols_simple_filters_ResidueLipophilicityFilter_fwd_hh


namespace protocols {
namespace simple_filters {

class ResidueLipophilicityFilter;

}
}

#endif
