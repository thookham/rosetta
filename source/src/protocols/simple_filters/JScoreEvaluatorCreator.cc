...
static basic::Tracer tr( "protocols.evalution.JScoreEvaluatorCreator" );
...
void JScoreEvaluatorCreator::add_evaluators( evaluation::MetaPoseEvaluator & eval ) const {
	...
	if ( option[ OptionKeys::evaluation::jscore_evaluator ].user() ) {
		...
		for ( core::Size ii = 1; ii <= tags.size() - 1; ii += 2 ) {
			...
			eval.add_evaluation( utility::pointer::make_shared< JScoreEvaluator >( scorefxn_name, rsd_set_name ) );
		}
	}
