...
void AdditionalOutputWrapper::generate_pose(core::pose::Pose & pose)
{
	...
	if ( !mover && mover_tag_ ) {
		mover = MoverFactory::get_instance()->newMover(mover_tag_, data);
	}
...
void AdditionalOutputWrapper::parse_my_tag(
	...
	try {
		...
		throw CREATE_EXCEPTION(utility::excn::Exception, "Exception in AdditionalOutputWrapper with name \"" + my_name + "\": " + e.msg());
	}
}
