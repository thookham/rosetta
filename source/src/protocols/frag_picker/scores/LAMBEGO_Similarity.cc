...
/// @file   protocols/frag_picker/scores/LAMBEGO_Similarity.cc
/// @brief
/// @author James Thompson

#include <protocols/frag_picker/scores/LAMBEGO_Similarity.hh>
...
void LAMBEGO_Similarity::do_caching( VallChunkOP chunk ) {
	...
	for ( core::Size i = 1; i <= query_len_; ++i ) {
		for ( core::Size j = 1; j <= chunk->size(); ++j ) {
			...
			char const ss   ( chunk->at(j)->ss()    );
			char const torsion_bin( torsion2big_bin_( phi, psi, omega, ss ) );
...
FragmentScoringMethodOP MakeLAMBEGO_Similarity::make(
	core::Size priority,
	...
	protocols::frag_picker::LAMBEGO_IO reader;
	...
	return (
		utility::pointer::make_shared< LAMBEGO_Similarity >(
		priority,
