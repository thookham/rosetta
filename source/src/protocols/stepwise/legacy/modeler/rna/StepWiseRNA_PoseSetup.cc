...
static basic::Tracer TR( "protocols.stepwise.legacy.modeler.rna.StepWiseRNA_PoseSetup" );
...
StepWiseRNA_PoseSetup::setup_native_pose( core::pose::Pose & pose ){
	...
	if ( native_pose_copy.sequence() == core::pose::rna::remove_bracketed( working_parameters_->full_sequence() ) ) {
		...
		for ( core::Size seq_num = native_pose_copy.size(); seq_num >= 1 ; seq_num-- ) {
	...
	if ( working_native_alignment.size() != 0 ) { //User specified the native alignment res.
		...
		for ( core::Size n = 1; n <= working_native_alignment.size(); n++ ) {
			core::Size const seq_num = working_native_alignment[n];
	...
	for ( core::Size n = 1; n <= working_native_virtual_res_list_.size(); n++ ) {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void
StepWiseRNA_PoseSetup::Import_pose( core::Size const & i, core::pose::Pose & import_pose ) const {
	...
	utility::vector1< core::Size > const & input_res = working_parameters_->input_res_vectors()[i];
...
StepWiseRNA_PoseSetup::read_input_pose_and_copy_dofs( pose::Pose & pose )
{
	...
	for ( core::Size i = 1; i <= input_tags_.size(); i++ ) {
		...
		utility::vector1< core::Size > const & input_res = working_parameters_->input_res_vectors()[i]; //This is from input_pose numbering to full_pose numbering
		...
		for ( core::Size seq_num = 1; seq_num <= start_pose.size(); seq_num++  ) {
			for ( core::Size k = 1; k <= variant_type_list.size(); ++k ) {
		...
		for ( core::Size n = 1; n <= input_res.size(); n++ )  res_map[ full_to_sub[ input_res[n] ] ] = n;
		...
		utility::vector1< core::Size > const & working_cutpoint_closed_list = apply_full_to_sub_mapping( cutpoint_closed_list, stepwise::modeler::working_parameters::StepWiseWorkingParametersCOP( working_parameters_ ) );

		for ( core::Size n = 1; n <= start_pose_with_variant.size(); n++  ) {
...
StepWiseRNA_PoseSetup::correctly_copy_HO2prime_positions( pose::Pose & working_pose, utility::vector1< pose::Pose > const & start_pose_list ){
	...
	for ( core::Size n = 1; n <= input_res_vectors[1].size(); n++ ) {
		core::Size const seq_num = input_res_vectors[1][n];
	...
	for ( core::Size const full_seq_num : common_res_list ) {
		core::Size const working_seq_num = full_to_sub[full_seq_num];

		core::Size const input_ONE_seq_num = full_to_input_res_map_ONE.find( full_seq_num )->second;
		...
		core::Size const input_TWO_seq_num = full_to_input_res_map_TWO.find( full_seq_num )->second;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
core::Real
StepWiseRNA_PoseSetup::get_nearest_dist_to_O2prime( core::Size const O2prime_seq_num,
	pose::Pose const & input_pose,
	utility::vector1< core::Size > const & input_res_list,
	utility::vector1< core::Size > const & common_res_list ){
	...
	for ( core::Size input_pose_seq_num = 1; input_pose_seq_num <= input_res_list.size(); input_pose_seq_num++ ) {
		core::Size const full_seq_num = input_res_list[input_pose_seq_num];
		...
		for ( core::Size at = 1; at <= input_pose_rsd.natoms(); at++ ) {
...
StepWiseRNA_PoseSetup::apply_cutpoint_variants( pose::Pose & pose ){
	...
	for ( core::Size const cutpoint_closed : cutpoint_closed_list ) {
		...
		if ( full_to_sub.find( cutpoint_closed )     != full_to_sub.end() &&
			...
			core::Size const cutpos = full_to_sub[ cutpoint_closed];
...
StepWiseRNA_PoseSetup::apply_virtual_phosphate_variants( pose::Pose & pose ) const{
	...
	for ( core::Size n = 1; n <= num_chains; n++ ) {
		...
		for ( core::Size i = 1; i <= cutpoint_closed_list.size(); i++ ) {
			core::Size const cutpoint_closed = cutpoint_closed_list[i];
...
StepWiseRNA_PoseSetup::add_terminal_res_repulsion( core::pose::Pose & pose ) const
{
	...
	for ( core::Size const k : working_terminal_res ) {
		...
		for ( core::Size m = 1; m <= nres; m++ ) {
			...
			cst_set->add_constraint( utility::pointer::make_shared< AtomPairConstraint >( atom_id1, atom_id2, repulsion_func ) );
		}
	}
...
StepWiseRNA_PoseSetup::additional_setup_for_floating_base( pose::Pose & pose ) const {
	...
	core::Size const moving_res_ =  working_parameters_->working_moving_res(); // Might not correspond to user input.
	core::Size const reference_res_ =  working_parameters_->working_reference_res(); //the last static_residues that this attach to the moving residues
...
StepWiseRNA_PoseSetup::add_protonated_H1_adenosine_variants( pose::Pose & pose ) const {
	...
	core::Size const working_moving_res =  working_parameters_->working_moving_res(); // corresponds to user input.
...
StepWiseRNA_PoseSetup::update_fold_tree_at_virtual_sugars( pose::Pose & pose ){
	...
	std::map< core::Size, core::Size > const & reference_res_for_each_virtual_sugar = sugar::get_reference_res_for_each_virtual_sugar_without_fold_tree( pose,  working_parameters_->working_moving_suite() );
	...
	for ( auto const & sugar_and_ref : reference_res_for_each_virtual_sugar ) {
		core::Size const virtual_sugar_res = sugar_and_ref.first;
		core::Size const reference_res = sugar_and_ref.second;
...
StepWiseRNA_PoseSetup::apply_bulge_variants( pose::Pose & pose ) const {
	...
	utility::vector1< core::Size > const & working_terminal_res = working_parameters_->working_terminal_res();
	utility::vector1< core::Size > terminal_res = apply_sub_to_full_mapping( working_terminal_res, working_parameters_ );

	for ( core::Size const seq_num : bulge_res_ ) {
...
StepWiseRNA_PoseSetup::apply_virtual_res_variant( pose::Pose & pose ) const {
	...
	core::Size const working_bulge_moving_res = ( is_prepend ) ? working_moving_res + 1 : working_moving_res - 1;
...
StepWiseRNA_PoseSetup::add_aa_virt_rsd_as_root( core::pose::Pose & pose ) {  //Fang's electron density code
	...
	core::Size const nres = pose.size();
	core::Size const working_moving_res( working_parameters_->working_moving_res() );
...
StepWiseRNA_PoseSetup::setup_full_model_info( pose::Pose & pose ) const {
	...
	core::Size const clean_length = core::pose::rna::remove_bracketed( full_sequence ).size();
	utility::vector1< core::Size > working_res;
	for ( core::Size i = 1; i <= clean_length; ++i ) {
		if ( is_working_res[ i ] ) {
	...
	utility::vector1< core::Size > fixed_domain; // convert from old convention to new one stored in full_model info.
