...
StepWiseRNA_Clusterer::create_large_cluster_centers_member_list(){
	...
	while ( input_->has_another_pose() ) {
		...
		core::chemical::ResidueTypeSetCOP rsd_set( rsd_set_ );
		silent_struct->fill_pose( *pose_op, *rsd_set ); //umm is the pose still connected to the silent_struct? Meaning that if the pose change, does the silent struct get changed as well? Apr 23, 2010 Parin.
...
StepWiseRNA_Clusterer::create_silent_file_and_tag_list(){
	...
	while ( input_->has_another_pose() ) {
		...
		if ( perform_VDW_rep_screen_ || perform_filters_ ) {
			...
			PoseOP pose_op( new Pose );
			core::chemical::ResidueTypeSetCOP rsd_set( rsd_set_ );
			silent_struct->fill_pose( *pose_op, *rsd_set ); //umm is the pose still connected to the silent_struct?
		...
		if ( keep_pose_in_memory_ ) {
			if ( ( keep_pose_in_memory_hydrid_ == false ) || ( pose_output_list_.size() < max_memory_pose_num_ )  ) {
				...
				core::chemical::ResidueTypeSetCOP rsd_set( rsd_set_ );
				silent_struct->fill_pose( *localized_pose_op, *rsd_set );
...
StepWiseRNA_Clusterer::setup_fail_triangle_inequailty_list( pose::Pose & current_pose, std::string const & tag, utility::vector1< bool > & fail_triangle_inequality_list ){
	...
	getPoseExtraScore( current_pose, "score", current_score ); //Is this slow?
	...
	for ( core::Size n = 1; n <= large_cluster_pose_list_.size(); n++ ) { //lowest score cluster center at the beginning of the list
		...
		getPoseExtraScore( cluster_center_pose, "score", cluster_center_score ); //Is this slow?
...
StepWiseRNA_Clusterer::recalculate_rmsd_and_output_silent_file( std::string const & silent_file,
	...
	for ( core::Size n = 1 ; n <= silent_struct_output_list_.size(); n++ ) {
		...
		if ( protocols::stepwise::modeler::rna::check_for_messed_up_structure( pose, tag ) == true ) continue;
...
StepWiseRNA_Clusterer::get_best_neighboring_shift_RMSD_and_output_silent_file( std::string const & silent_file ){
	...
	for ( core::Size n = 1 ; n <= silent_struct_output_list_.size(); n++ ) {
		...
		bool has_total_score = getPoseExtraScore( current_pose, "score", start_score );
		if ( has_total_score == false ) utility_exit_with_message( "current_pose ( " + tag + " ) missing total score!" );


		Real start_shift_score( 0.0 );
		bool has_shift_score = getPoseExtraScore( current_pose, "shift_score", start_shift_score );
		...
		for ( core::Size other_pose_ID = 1 ; other_pose_ID  <= silent_struct_output_list_.size(); other_pose_ID ++ ) {
			...
			if ( protocols::stepwise::modeler::rna::check_for_messed_up_structure( other_pose, other_tag ) == true ) continue;
			...
			bool has_shift_score = getPoseExtraScore( other_pose, "shift_score", other_shift_score );
...
//////////////////////////////////////////////
void
StepWiseRNA_Clusterer::set_working_parameters( protocols::stepwise::modeler::working_parameters::StepWiseWorkingParametersCOP & working_parameters ){
...
//////////////////////////////////////////////
void
SlicedPoseWorkingParameters::setup( protocols::stepwise::modeler::working_parameters::StepWiseWorkingParametersCOP & working_parameters ){
