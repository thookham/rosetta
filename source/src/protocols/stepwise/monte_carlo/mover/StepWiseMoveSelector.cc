...
StepWiseMoveSelector::output_moves() const {
	...
	for ( core::Size n = 1; n <= swa_moves_.size(); n++ ) {
...
StepWiseMoveSelector::fill_moves_for_other_poses( pose::Pose const & pose ) {
	...
	core::Size const move_idx_original = swa_moves_.size();
	...
	utility::vector1< core::Size > first_res_in_other_poses;
	...
	for ( core::Size n = 1; n <= first_res_in_other_poses.size(); n++ ) {
		core::Size const other_pose_idx = const_full_model_info( pose_to_switch ).get_idx_for_other_pose_with_residue( first_res_in_other_poses[ n ] );
		switch_focus_to_other_pose( pose_to_switch, other_pose_idx );
		fill_moves_for_pose( pose_to_switch );
	}

	core::Size const move_idx_final = swa_moves_.size();
...
StepWiseMoveSelector::save_moves( utility::vector1< StepWiseMove > const & moves,
	...
	for ( core::Size n = 1; n <= moves.size(); n++ ) {
	...
	for ( core::Size n = 1; n <= moves.size(); n++ ) relative_weight_sum += relative_weights[ n ];
...
/////////////////////////////////////////////////////////////////////////////////////////////////
Real
StepWiseMoveSelector::sum_probabilities( core::Size const start_idx /* = 1 */,
	core::Size const final_idx_input /* = 0 */ ) const
{
	core::Size const final_idx = ( final_idx_input > 0 ) ? final_idx_input : proposal_probabilities_.size();
	Real weight( 0.0 );
	for ( core::Size n = start_idx; n <= final_idx; n++ ) {
...
/////////////////////////////////////////////////////////////////////////////////////////////////
void
StepWiseMoveSelector::normalize_probabilities( core::Size const start_idx /* = 1 */,
	core::Size const final_idx_input /* = 0 */,
...
StepWiseMoveSelector::select_random_move( pose::Pose const &  ) const {
	...
	Real weight( 0.0 );
	for ( core::Size n = 1; n <= proposal_probabilities_.size(); n++ ) {
		weight += proposal_probabilities_[ n ];
...
StepWiseMoveSelector::get_resample_terminal_move_elements( pose::Pose const & pose,
	...
	//for ( auto const & core::Size n = 1; n <= swa_moves_terminal.size(); n++ ) {
...
StepWiseMoveSelector::get_resample_internal_move_elements( pose::Pose const & pose,
	...
	//for ( core::Size n = 1; n <= swa_moves_internal.size(); n++ ) {
...
StepWiseMoveSelector::get_resample_internal_local_move_elements( pose::Pose const & pose,
	...
	swa_moves.reserve( swa_moves.size() + swa_moves_internal.size() );
	std::move( std::begin( swa_moves_internal ), std::end( swa_moves_internal ), std::back_inserter( swa_moves ) );
	//for ( core::Size n = 1; n <= swa_moves_internal.size(); n++ ) swa_moves.push_back( swa_moves_internal[ n ] );
}
...
StepWiseMoveSelector::get_intramolecular_add_move_elements( pose::Pose const & pose,
	...
	core::Size nres_full = full_model_info.full_sequence().size();
	...
	for ( core::Size n = 1; n <= cutpoint_open_in_full_model.size(); n++ ) is_cutpoint_in_full_pose[ cutpoint_open_in_full_model[n] ] = true;
	...
	for ( core::Size i = 1; i <= nres; i++ ) {
		...
		if ( ( options_->skip_bulge_frequency() > 0.0 ) &&
			...
			core::Size const other_pose_idx_intervening = full_model_info.get_idx_for_other_pose_with_residue( i_full + 1 );
	...
	for ( core::Size n = 1; n <= swa_moves_skip_bulge.size(); n++ ) swa_moves.push_back( swa_moves_skip_bulge[ n ] );
}
...
StepWiseMoveSelector::get_intramolecular_delete_move_elements( pose::Pose const & pose,
	...
	for ( core::Size n = 1; n <= swa_moves_delete.size(); n++ ) swa_moves.push_back( swa_moves_delete[ n ] );
...
StepWiseMoveSelector::get_intramolecular_split_move_elements( pose::Pose const & pose,
	...
	utility::vector1< core::Size > const domain_map = core::pose::full_model_info::get_input_domain_from_full_model_info_const( pose );
	FullModelInfo const & full_model_info = const_full_model_info( pose );
	...
	for ( core::Size n = 1; n <= pose.fold_tree().num_jump(); n++ ) {
		...
		core::Size const downstream_res = pose.fold_tree().downstream_jump_residue( n );
		core::Size const upstream_res = pose.fold_tree().upstream_jump_residue( n );
		...
		if ( partition_res1.size() == 1 ) {
			core::Size const anchor_res = get_anchor_res( partition_res1[1], pose );
		...
		if ( partition_res2.size() == 1 ) {
			core::Size const anchor_res = get_anchor_res( partition_res2[1], pose );
...
StepWiseMoveSelector::get_docking_split_move_elements( pose::Pose const & pose,
	...
	utility::vector1< bool > partition_definition;
	FullModelInfo const & full_model_info = const_full_model_info( pose );
	utility::vector1< core::Size > const dock_domain_map = full_model_info.dock_domain_map();
	utility::vector1< core::Size > const & res_list = get_res_list_from_full_model_info_const( pose );

	// look at jumps.
	for ( core::Size n = 1; n <= pose.fold_tree().num_jump(); n++ ) {
		partition_definition = get_partition_definition_by_jump( pose, n );
		core::Size const moving_res = pose.fold_tree().downstream_jump_residue( n );
		core::Size const reference_res = pose.fold_tree().upstream_jump_residue( n );
		core::Size const moving_res_full =  res_list[ moving_res ];
		core::Size const reference_res_full = res_list[ reference_res ];
		...
		utility::vector1< core::Size > const moving_partition_res = get_partition_res( partition_definition,  partition_definition[ moving_res ] );
		utility::vector1< core::Size > const reference_partition_res = get_partition_res( partition_definition,  partition_definition[ reference_res ] );
...
StepWiseMoveSelector::partitions_split_a_submotif(
	...
	utility::vector1< core::Size > const & pose_res_list( get_res_list_from_full_model_info_const( pose ) );
	...
	for ( itr = submotif_info_list.begin(); itr != submotif_info_list.end(); ++itr ) {
		...
		for ( core::Size n = 1; n <= (*itr)->res_list().size(); n++ ) {
			core::Size const submotif_res_in_pose_numbering( pose_res_list.index( (*itr)->res_list( n ) ) );
...
{
	utility::vector1< core::Size > const partition_in_full_model_numbering = sub_to_full( partition, pose );
...
StepWiseMoveSelector::check_for_intramolecular_submotif_jump(
	...
	utility::vector1< core::Size > const & check_res = utility::tools::make_vector1( moving_res, attached_res );
...
StepWiseMoveSelector::remove_from_consideration_first_multi_residue_move_element( utility::vector1< StepWiseMove > & swa_moves,
	...
	core::Size number_of_multi_residue_move_elements( 0 ), first_multi_residue_move_element( 0 );
	for ( core::Size n = 1; n <= swa_moves.size(); n++ ) {
...
StepWiseMoveSelector::get_from_scratch_add_move_elements( pose::Pose const & pose,
	...
	for ( core::Size n = 1; n < nres_full; n++ ) {
...
StepWiseMoveSelector::figure_out_from_scratch_delete_move_elements( pose::Pose const & pose,
	...
	utility::vector1< core::Size > const domain_map = core::pose::full_model_info::get_input_domain_from_full_model_info_const( pose );
	utility::vector1< core::Size > const partition_definition = utility::tools::make_vector1(true,false);
	...
	utility::vector1< PoseOP > const & other_pose_list = const_full_model_info( pose ).other_pose_list();
	for ( core::Size k = 1; k <= other_pose_list.size(); k++ ) {
...
{
	utility::vector1< core::Size > const & res_list = get_res_list_from_full_model_info_const( pose );
	std::set< core::Size > dock_domains = get_dock_domains( res_list, pose );
...
StepWiseMoveSelector::get_dock_domains(
	utility::vector1< core::Size > const & move_element,
	pose::Pose const & pose ) const
{
	std::set< core::Size > dock_domains;
	utility::vector1< core::Size > const & dock_domain_map = const_full_model_info( pose ).dock_domain_map();
	for ( core::Size i = 1; i <= move_element.size(); i++ ) {
...
StepWiseMoveSelector::get_docking_add_move_elements( pose::Pose const & pose,
	...
	utility::vector1< core::Size > const & res_list = get_res_list_from_full_model_info_const( pose );
	utility::vector1< core::Size > const & dock_domain_map = const_full_model_info( pose ).dock_domain_map();
	//utility::vector1< core::Size > const & working_res = const_full_model_info( pose ).working_res();

	if ( res_list.size() == 0 ) return;

	std::map< std::pair< core::Size, core::Size >, std::pair< core::Size, core::Size > > preferred_jump_pair = get_preferred_jump_pair_for_docking_domains( full_model_info );
	...
	for ( core::Size i = 1; i <= nres; i++ ) {
		core::Size const i_full = res_list[ i ] ;
		core::Size const dock_domain_i = dock_domain_map[ i_full ];
		if ( !is_addable_res( i_full, pose ) ) continue;
		for ( core::Size j_full = 1; j_full <= full_model_info.size(); j_full++ ) {
			if ( res_list.has_value( j_full ) ) continue; // must be docked to different pose.

			core::Size const dock_domain_j = dock_domain_map[ j_full ];
			...
			if ( preferred_jump_pair.find( std::make_pair( dock_domain_i, dock_domain_j ) ) !=
				...
				std::pair< core::Size, core::Size > preferred_jump_for_dock_domain = preferred_jump_pair[ std::make_pair( dock_domain_i, dock_domain_j ) ];
...
{
	std::map< core::Size, bool > addable;
	for ( core::Size k = 1; k <= add_moves.size(); k++ ) {
		StepWiseMove const & add_move = add_moves[ k ];
		core::Size const add_move_res = add_move.moving_res();
		addable[ add_move_res ] = true;
	}
	bool all_addable( true );
	for ( core::Size k = 1; k <= move_element.size(); k++ ) {
...
StepWiseMoveSelector::get_submotif_add_moves( pose::Pose const & pose,
	...
	for ( core::Size n = 1; n <= submotif_from_scratch_moves.size(); n++ ) {
	...
	for ( core::Size n = 1; n <= submotif_add_moves.size(); n++ ) swa_moves.push_back( submotif_add_moves[ n ] );
}
	...
	pose::Pose const & pose ) const
{
	...
	for ( core::Size n = 1; n <= submotif_add_moves.size(); n++ ) {
		runtime_assert( submotif_add_moves[ n ].move_type() == ADD_SUBMOTIF );
		// what domains might be docked in submotif?
		std::set< core::Size > const dock_domains = get_dock_domains( submotif_add_moves[ n ].move_element(), pose );
...
StepWiseMoveSelector::get_unique_chains( pose::Pose const & pose ){
	utility::vector1< core::Size > const chains = figure_out_chain_numbers_from_full_model_info_const( pose );
	utility::vector1< core::Size > unique_chains;
	for ( core::Size n = 1; n <= chains.size(); n++ ) {
...
bool
StepWiseMoveSelector::share_chains( utility::vector1< core::Size > const & current_unique_chains,
	pose::Pose const & pose,
	core::Size const j_full ) {

	FullModelInfo const & full_model_info = const_full_model_info( pose );
	utility::vector1< core::Size > const chains_full = get_chains_full( pose );

	core::Size const chain_j = chains_full[ j_full ];
	utility::vector1< core::Size > other_pose_chains = utility::tools::make_vector1( chain_j );

	core::Size const other_pose_idx = full_model_info.get_idx_for_other_pose_with_residue( j_full );
	if ( other_pose_idx > 0 ) other_pose_chains = get_unique_chains( *full_model_info.other_pose_list()[ other_pose_idx ] );

	for ( core::Size q = 1; q <= current_unique_chains.size(); q++ ) {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool
StepWiseMoveSelector::is_addable_res( core::Size const n, pose::Pose const & pose ) const {
...
{
	utility::vector1< std::pair< core::Size, core::Size > > design_loops;
	utility::vector1< core::Size > const & cutpoint_open = parent_full_model_parameters->get_res_list( CUTPOINT_OPEN );
	std::string const & full_sequence( parent_full_model_parameters->full_sequence() );
	bool in_loop( false );
	core::Size start_loop( 1 );
	core::Size const nres( full_sequence.size() );
	for ( core::Size n = 1; n <= nres + 1; n++ ) {
		bool const is_designable_residue = ( n <= nres && full_sequence[ n - 1 ] == 'n' );
		if ( in_loop && ( !is_designable_residue || cutpoint_open.has_value( n - 1 ) ) ) {
			in_loop = false;
			core::Size const end_loop = n - 1;
	...
	utility::vector1< StepWiseMove > & vary_loop_length_moves ) const
{
	...
	utility::vector1< core::Size > const & slice_res_list = const_full_model_info( pose ).full_model_parameters()->slice_res_list();
	utility::vector1< core::Size > const & cutpoint_open_in_full_model = const_full_model_info( pose ).cutpoint_open_in_full_model();
	core::Size const nres = full_model_parameters->size();

	// loops in parent full model numbering:
	utility::vector1< std::pair< core::Size, core::Size > > design_loops = get_design_loops( parent_full_model_parameters );
	for ( core::Size n = 1; n <= design_loops.size(); n++ ) {
		std::pair< core::Size, core::Size > const & design_loop = design_loops[ n ];
		core::Size const max_gap_size = design_loop.second - design_loop.first + 1;
		...
		core::Size takeoff_res( 0 ), landing_res( 0 );
		if ( slice_res_list.has_value( design_loop.first - 1 ) ) {
			...
			for ( core::Size q = design_loop.first; q <= design_loop.second + 1; q++ ) {
		...
		if ( slice_res_list.has_value( design_loop.second + 1 ) ) {
			...
			for ( core::Size q = design_loop.second; q >= design_loop.first - 1; q-- ) {
				if ( slice_res_list.has_value( q ) ) {
		...
		core::Size const gap_size = landing_res - takeoff_res - 1;
...
bool
StepWiseMoveSelector::partition_splits_an_input_domain( utility::vector1< core::Size > const & partition_definition,
	...
	utility::vector1< utility::vector1< core::Size > > all_domain_res;
	for ( core::Size i = 1; i <= domain_map.size(); i++ ) {
		core::Size const & domain = domain_map[ i ];
		for ( core::Size n = all_domain_res.size(); n < domain; n++ ) all_domain_res.push_back( blank_vector );
		if ( domain > 0 ) all_domain_res[ domain ].push_back( i );
	}
	for ( core::Size n = 1; n <= all_domain_res.size(); n++ ) {
		// for this domain, check if all residues are in a single partition.
		bool in_partition_0( false ), in_partition_1( false );
		for ( core::Size k = 1; k <= all_domain_res[ n ].size(); k++ ) {
...
StepWiseMoveSelector::get_terminal_move_elements( pose::Pose const & pose,
	...
	core::Size const num_move_elements( move_elements.size() );
	for ( core::Size n = 1; n <= num_move_elements; n++ ) {
...
StepWiseMoveSelector::get_partition_res( utility::vector1< bool > const & partition_definition, bool const setting ) const {
	...
	for ( core::Size n = 1; n <= partition_definition.size(); n++ ) {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
Attachments
StepWiseMoveSelector::get_attachments( pose::Pose const & pose, core::Size const & moving_res ){
...
StepWiseMoveSelector::get_attachments( pose::Pose const & pose, MoveElement const & move_element ){
	...
	for ( core::Size k = 1; k <= move_element.size(); k++ ) {
		...
		core::Size const i_full = move_element[ k ];
		core::Size const i = res_list.index( i_full );
		...
		if ( i > 1 && ! move_element.has_value( res_list[ i - 1 ] ) ) {
			...
			core::Size const jump_attachment_res = check_jump_to_previous_residue_in_chain( pose, i, move_element, const_full_model_info( pose ) );
		...
		if ( i < nres && ! move_element.has_value( res_list[ i + 1 ] ) ) {
			...
			core::Size const jump_attachment_res = check_jump_to_next_residue_in_chain( pose, i, move_element, const_full_model_info( pose ) );
			if ( jump_attachment_res > 0 ) attachments_to_next.push_back( Attachment( res_list[ jump_attachment_res ], JUMP_TO_NEXT_IN_CHAIN ) );
		}
	}

	for ( core::Size k = 1; k <= attachments_to_previous.size(); k++ ) attachments.push_back( attachments_to_previous[ k ] );
	for ( core::Size k = 1; k <= attachments_to_next.size()    ; k++ ) attachments.push_back( attachments_to_next[ k ] );
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool
StepWiseMoveSelector::already_instantiated_in_pose( pose::Pose const & pose, core::Size const & resnum_in_full_model_numbering ) const {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
utility::vector1< core::Size >
StepWiseMoveSelector::figure_out_first_res_in_pose( utility::vector1< core::Size > const & pose_domain_map ) const {
	utility::vector1< core::Size > first_res_in_pose( max( pose_domain_map ), 0 );
...
StepWiseMoveSelector::filter_pose_order( utility::vector1< StepWiseMove > & swa_moves,
	...
	utility::vector1< core::Size > const pose_domain_map = figure_out_pose_domain_map_const( pose );
	utility::vector1< core::Size > const first_res_in_pose = figure_out_first_res_in_pose( pose_domain_map );
	...
	for ( core::Size i = 1; i <= swa_moves.size(); i++ ) {

		StepWiseMove const & swa_move = swa_moves[ i ];

		if ( swa_move.move_type() == ADD ) {
			core::Size const moving_res   = swa_move.moving_res();
			core::Size const attached_res = swa_move.attached_res();

			core::Size const pose_idx1 = pose_domain_map[ moving_res ];
			core::Size const pose_idx2 = pose_domain_map[ attached_res ];
...
StepWiseMoveSelector::get_actual_moving_res( StepWiseMove const & swa_move, pose::Pose const & pose ) const {
	...
	utility::vector1< core::Size >  moving_partition_res = full_model_info.full_to_sub( swa_move.move_element() );
	...
	core::Size actual_moving_res = find_downstream_connection_res( pose, moving_partition_res );
...
StepWiseMoveSelector::reverse_delete_move( StepWiseMove const & swa_move, pose::Pose const & pose_before, pose::Pose const & pose_after ) const {
	...
	core::Size pose_res = swa_move.move_element()[ 1 ];
	core::Size const other_pose_idx = const_full_model_info( pose_before ).get_idx_for_other_pose_with_residue( pose_res );
	...
	core::Size const actual_moving_res = get_actual_moving_res( swa_move, old_pose ); // in working numbering
	core::Size const res1 = sub_to_full( actual_moving_res, old_pose );
	core::Size const res2 = sub_to_full( old_pose.fold_tree().get_parent_residue( actual_moving_res ), old_pose );

	utility::vector1< core::Size > pose_domain_map = figure_out_pose_domain_map_const( pose_after );
	core::Size const & pose_domain1 = pose_domain_map[ res1 ];
	core::Size const & pose_domain2 = pose_domain_map[ res2 ];
	if ( pose_domain1 > 0 && pose_domain2 > 0 ) {
		utility::vector1< core::Size > const first_res_in_pose = figure_out_first_res_in_pose( pose_domain_map );
		utility::vector1< core::Size > const & sample_res = const_full_model_info( pose_before ).sample_res();
...
StepWiseMoveSelector::reverse_add_move( StepWiseMove const & swa_move, pose::Pose const & pose_after ) const {
	...
	core::Size working_moving_res   = res_list.index( swa_move.moving_res() );
	core::Size working_attached_res = res_list.index( swa_move.attached_res() );
	core::Size actual_moving_res, actual_attached_res;
	if ( core::Size( pose_after.fold_tree().get_parent_residue( working_moving_res ) ) ==  working_attached_res ) {
		actual_moving_res = working_moving_res;
		actual_attached_res = working_attached_res;
	} else {
		// may have re-rooted
		if ( core::Size( pose_after.fold_tree().get_parent_residue( working_attached_res ) ) != working_moving_res )  {
		...
		runtime_assert( core::Size( pose_after.fold_tree().get_parent_residue( working_attached_res ) ) == working_moving_res );
...
StepWiseMoveSelector::reverse_add_submotif_move( StepWiseMove const & swa_move, pose::Pose const & pose_after ) const {
	core::Size const actual_moving_res   = get_actual_moving_res( swa_move, pose_after );
	core::Size const actual_attached_res = pose_after.fold_tree().get_parent_residue( actual_moving_res );
	return ordered_move_from_partition( actual_moving_res, actual_attached_res, pose_after, DELETE );
}
...
StepWiseMoveSelector::reverse_resample_move( StepWiseMove const & swa_move, pose::Pose const & pose_after ) const {
	...
	core::Size actual_moving_res = get_actual_moving_res( swa_move, pose_after );
	core::Size const actual_attached_res = pose_after.fold_tree().get_parent_residue( actual_moving_res );
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
StepWiseMove
StepWiseMoveSelector::ordered_move_from_partition( core::Size const actual_moving_res, core::Size const actual_attached_res,
	...
	core::Size moving_res, attached_res;
...
//////////////////////////////////////////////////////////////////////////////////////////////////
Attachment
StepWiseMoveSelector::figure_out_attachment( core::Size const moving_res, core::Size const attached_res,
	pose::Pose const & pose ) const {
...
StepWiseMoveSelector::just_simple_cycles( StepWiseMove const & swa_move, pose::Pose const & pose, bool const verbose /* = false */ ) const {
	...
	utility::vector1< core::Size > pose_domain_map = figure_out_pose_domain_map_const( pose );
	utility::vector1< core::Size > const & cutpoint_open_in_full_model = const_full_model_info( pose ).cutpoint_open_in_full_model();
	...
	if ( move_type == ADD ) {
		core::Size const old_domain = pose_domain_map[ swa_move.moving_res() ];
		core::Size const new_domain = pose_domain_map[ swa_move.attached_res() ];
		...
		utility::vector1< core::Size > const & res_list = get_res_list_from_full_model_info_const( pose );
		utility::vector1< core::Size > other_element;
		...
		if ( const_full_model_info( pose ).is_a_submotif( move_element ) &&
			...
			for ( core::Size k = 1; k <= move_element.size(); k++ ) pose_domain_map[ move_element[ k ] ] = 0;
			...
			for ( core::Size k = 1; k <= other_element.size(); k++ ) pose_domain_map[ other_element[ k ] ] = 0;
		} else {
			core::Size const new_domain = max( pose_domain_map ) + 1;
			for ( core::Size k = 1; k <= move_element.size(); k++ ) pose_domain_map[ move_element[ k ] ] = new_domain;
		}
	} else if ( move_type == FROM_SCRATCH ) {
		core::Size const new_domain = max( pose_domain_map ) + 1;
		for ( core::Size k = 1; k <= move_element.size(); k++ ) pose_domain_map[ move_element[ k ] ] = new_domain;
