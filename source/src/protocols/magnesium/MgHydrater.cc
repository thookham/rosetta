...
//Constructor
MgHydrater::MgHydrater( utility::vector1< core::Size > const & mg_res_list ):
...
///////////////////////////////////////////
void
MgHydrater::hydrate_magnesium( pose::Pose & pose, core::Size const i ) {
	...
	if ( excise_mini_pose_ ) {
		...
		Pose pose_full = pose;
		core::Size const mg_res_in_full( i );
		vector1< core::Size > slice_res = pdbslice( pose, mg_res_in_full );
		...
		if ( use_virtual_waters_as_placeholders_ ) { // swap out mg & waters into full pose
			...
			for ( core::Size ii = 1; ii <= pose_full.residue( mg_res_in_full ).natoms(); ii++ ) {
				pose_full.set_xyz( AtomID( ii, mg_res_in_full ), pose.xyz( AtomID( ii, mg_res_in_mini ) ) );
			}
			for ( core::Size n = nres_original + 1; n <= pose.size(); n++ ) {
...
///////////////////////////////////////////
void
MgHydrater::hydrate_magnesium_in_pose( pose::Pose & pose, core::Size const i,
	...
	if ( use_fast_frame_heuristic_ ) {
		...
		for ( core::Size n = 1; n <= nbr_atom_ids.size(); n++ ) {
		...
		if ( ligands.size() == 1 ) {
			...
			for ( core::Size n = 1; n <= ligands.size(); n++ ) {
				...
				for ( core::Size m = 1; m <= ligands.size(); m++ ) {
					if ( m == n ) continue;
	...
	for ( core::Size pass = 1; pass <= 2; pass++ ) {
...
MgHydrater::update_full_model_info_with_new_waters( pose::Pose & pose,
	...
	utility::vector1< core::Size > res_list = full_model_info.res_list(); // will be updated
	runtime_assert( pose.size() >= res_list.size() );
	core::Size const num_new_waters( pose.size() - res_list.size() );
	if ( expect_no_new_waters ) runtime_assert( num_new_waters == 0 );
	if ( num_new_waters == 0 ) return;
	for ( core::Size n = pose.size() - num_new_waters + 1; n <= pose.size(); n++ ) {
	...
	std::map< core::Size, std::string > const & non_standard_residue_map = full_model_parameters.non_standard_residue_map();

	utility::vector1< core::Size > water_res;
	for ( core::Size n = 1; n <= full_model_parameters.size(); n++ ) {
	...
	for ( core::Size n = 1; n <= num_new_waters; n++ )  res_list.push_back( water_res[ n ] );
...
MgHydrater::hydrate_magnesium_with_orbital_frame( pose::Pose & pose,
	...
	core::Size const nres_orig = pose.size();
	...
	for ( core::Size n = 1; n <= 6; n++ ) {
		...
		for ( core::Size k = 1; k <= nbr_atom_ids.size(); k++ ) {
		...
		if ( do_instantiate_water ) {
			core::Size instantiated_water_res = instantiate_water_at_octahedral_vertex( pose, i, n, MG_HOH_DISTANCE, replace_residue );
	...
	for ( core::Size k = 1; k <= num_waters; k++ ) {
...
////////////////////////////////////////////////////////////////
void
fix_water_jump( pose::Pose & pose, core::Size const & parent_res, core::Size const & water_res )
{
	using namespace core::kinematics;
	FoldTree f( pose.fold_tree() );
	core::Size const jump_number = f.get_jump_that_builds_residue( water_res );
...
MgHydrater::fix_fold_tree_in_excised_pose_for_mg_bound_waters(
	pose::Pose & pose, core::Size const mg_res,
	pose::Pose const & pose_full,
	utility::vector1< core::Size > const & slice_res ) const
{
	using namespace core::kinematics;
	FoldTree f( pose.fold_tree() );
	core::Size const mg_res_in_full = slice_res[ mg_res ];
	...
	for ( core::Size const water_res_in_full : bound_waters_in_full_pose ) {
	...
	for ( core::Size const water_res_in_mini : bound_waters_in_mini_pose ) {
		core::Size const water_res_in_full = slice_res[ water_res_in_mini ];
...
{
	vector1< core::Size > const all_mg_res = get_mg_res( pose );

	// first check -- all Mg(2+) might actually be OK.
	bool all_magnesiums_have_six_waters( true );
	for ( core::Size n = 1; n <= all_mg_res.size(); n++ ) {
	...
	std::map< core::Size, vector1< core::Size > > mg_water_map = define_mg_water_map( pose );

	// get mg-water pairs, properly assigned.
	for ( core::Size const mg_res : all_mg_res ) {
		// now go through each mg(2+), and fix up fold tree for waters bound to it already.
		vector1< core::Size > const & water_ligands = mg_water_map[ mg_res ];
		for ( core::Size const water_res : water_ligands ) {
		...
		for ( core::Size n = water_ligands.size() + 1; n <= 6; n++ ) {
