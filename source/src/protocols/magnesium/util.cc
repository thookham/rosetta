...
hydrate_magnesiums( pose::Pose & pose,
	...
	utility::vector1< core::Size > const mg_res = get_mg_res( pose );
...
utility::vector1< std::pair< core::Size, core::id::AtomID > >
filter_acceptor_ligands( pose::Pose const & pose, utility::vector1< std::pair< core::Size, core::id::AtomID > > const & ligands ) {
	using namespace core::id;
	utility::vector1< std::pair< core::Size, core::id::AtomID > > acceptor_ligands;
	for ( core::Size n = 1; n <= ligands.size(); n++ ) {
...
filter_water_ligands( pose::Pose const & pose, utility::vector1< std::pair< core::Size, core::id::AtomID > > const & ligands ) {
	using namespace core::id;
	utility::vector1< std::pair< core::Size, core::id::AtomID > > water_ligands;
	for ( core::Size n = 1; n <= ligands.size(); n++ ) {
		AtomID const & ligand = ligands[ n ].second;
...
//
std::map< core::Size, vector1< core::Size > >
define_mg_water_map( pose::Pose const & pose )
{
	std::map< core::Size, vector1< core::Size > > mg_water_map; // each mg mapped to a list of up to 6 waters
	std::map< core::Size, core::Size > water_to_magnesium; // each water assigned to one magnesium
	utility::vector1< std::pair< core::Size, core::Size > > mg_water_pairs = get_mg_water_pairs( pose, false /*exclude_virtual_water*/ );

	TR << "NUM WATER PAIRS " << mg_water_pairs.size() << std::endl;

	for ( core::Size n = 1; n <= mg_water_pairs.size(); n++ ) {
		core::Size const mg_res    = mg_water_pairs[ n ].first;
		core::Size const water_res = mg_water_pairs[ n ].second;
	...
	for ( std::map< core::Size, core::Size >::const_iterator it = water_to_magnesium.begin(); it != water_to_magnesium.end(); it++ ) {
		if ( it->second == 0 ) {
...
///////////////////////////////////////////////////////////////////////////////
utility::vector1< std::pair< core::Size, core::id::AtomID > >
get_mg_ligands( pose::Pose const & pose, utility::vector1< core::Size > const mg_res,
	...
	bool const just_waters = false )
{
	...
	utility::vector1< std::pair< Distance, std::pair< core::Size, core::id::AtomID > > > distance_ligand_pairs;
	for ( core::Size n = 1; n <= mg_res.size(); n++ ) {
		...
		for ( core::Size j = 1; j <= pose.size(); j++ ) {
			...
			for ( core::Size jj = 1; jj <= rsd_j.nheavyatoms(); jj++ ) {
	...
	utility::vector1< std::pair< core::Size, core::id::AtomID > > ligands;
	for ( core::Size n = 1; n <= distance_ligand_pairs.size(); n++ ) {
...
///////////////////////////////////////////////////////////////////////////////
utility::vector1< core::id::AtomID >
get_mg_ligands( pose::Pose const & pose, core::Size const i,
	bool const filter_for_acceptors /* true */,
	bool const exclude_virtual_waters /* true */ )
{
	vector1< std::pair< core::Size, core::id::AtomID > > mg_and_ligands = get_mg_ligands( pose, utility::tools::make_vector1( i ),
		filter_for_acceptors, exclude_virtual_waters );
	utility::vector1< core::id::AtomID > ligands;
	for ( core::Size n = 1; n <= mg_and_ligands.size(); n++ ) ligands.push_back( mg_and_ligands[ n ].second );
	return ligands;
}
...
get_mg_water_pairs( pose::Pose const & pose,
	...
	vector1< std::pair< core::Size, id::AtomID > > const ligands = get_mg_ligands( pose, mg_res, false /*filter_for_acceptors*/,
		exclude_virtual_waters, true /*just_waters*/ );
	utility::vector1< std::pair< core::Size, core::Size > > mg_water_pairs;
...
find_bound_waters_that_are_daughters_in_fold_tree( pose::Pose const & pose, core::Size const mg_res )
{
	vector1< core::Size > water_res;
...
///////////////////////////////////////////////////////////////////////////////
core::Size
get_bound_water_res( pose::Pose const & pose, core::Size const mg_res, core::Size const n /* = 1..6 */) {
	runtime_assert( n >= 1);
	runtime_assert( n <= 6);
	vector1< core::Size > water_res = find_bound_waters_that_are_daughters_in_fold_tree( pose, mg_res );
	if ( water_res.size() != 6 ) {
...
update_jump_atoms_for_mg_bound_water( core::pose::Pose & pose, core::Size const n )
{
	...
	core::Size const njump( f.get_jump_that_builds_residue( n ) );
...
get_res_with_name( pose::Pose const & pose, std::string const & name ) {
	utility::vector1< core::Size > mg_res;
	for ( core::Size n = 1; n <= pose.size(); n++ ) {
...
remove_waters_except_mg_bound( pose::Pose & pose,
	...
	for ( core::Size i = 1; i <= mg_water_pairs.size(); i++ ) {
		core::Size const n = mg_water_pairs[ i ].second;
...
///////////////////////////////////////////
void
remove_mg_bound_waters( pose::Pose & pose, utility::vector1< core::Size > const & mg_res, bool const leave_other_waters /* = false */ ) {

	using namespace core::pose;
	using namespace core::pose::full_model_info;

	vector1< std::pair< core::Size, core::Size > > mg_water_pairs = get_mg_water_pairs( pose, mg_res );
	vector1< core::Size > mg_bound_waters;
	for ( core::Size i = 1; i <= mg_water_pairs.size(); i++ ) {
		core::Size const n = mg_water_pairs[ i ].second;
	...
	if ( full_model_info_defined( pose ) ) {
		...
		vector1< core::Size > remove_water_res_in_full_model_numbering;
		for ( core::Size n = 1; n <= pose.size(); n++ ) {
		...
		for ( core::Size n = 1; n <= remove_water_res_in_full_model_numbering.size(); n++ ) {
			core::Size const water_res = const_full_model_info( pose ).res_list().index( remove_water_res_in_full_model_numbering[ n ] );
...
get_hoh_xyz( pose::Pose const & pose, core::Size const pdb_mg_res, core::Size & nlig ) {
	...
	core::Size pose_mg_res = pdb_to_pose( pose, pdb_mg_res );
	vector1< AtomID > mg_ligands = get_mg_ligands( pose, pose_mg_res );
	nlig = mg_ligands.size();
	for ( core::Size i = 1; i <= mg_ligands.size(); i++ ) {
...
get_hydration_stats( pose::Pose const & pose,
	...
	vector1< core::Size> pdb_mg_res_list( pdb_mg_res_list_in );
	if ( pdb_mg_res_list.size() == 0 ) pdb_mg_res_list = pose_to_pdb( pose, get_mg_res( pose ) );
	for ( core::Size n = 1; n <= pdb_mg_res_list.size(); n++ ) {
		core::Size pdb_mg_res = pdb_mg_res_list[ n ];
		//  runtime_assert( );
		core::Size nlig_ref( 0 ), nlig_model( 0 );
		...
		core::Size const num_ref_hoh(   ref_hoh_xyz.size() );
		core::Size const num_model_hoh( model_hoh_xyz.size() );
		core::Real rmsd( 0.0 );
		for ( core::Size i = 1; i <= ref_hoh_xyz.size(); i++ ) {
			Distance best_distance( 10.0 ); // placeholder
			for ( core::Size j = 1; j <= model_hoh_xyz.size(); j++ ) {
