...
public:
	...
	/// @brief Get the custom Ramachandran table (by enum) for the perturb_alpha_backbone_by_rama perturber.
	///
	inline core::scoring::Rama_Table_Type custom_rama_table() const { return custom_rama_table_; }

	/// @brief Set the custom Ramachandran table (by enum) for the perturb_alpha_backbone_by_rama perturber.
	///
	inline void set_custom_rama_table ( core::scoring::Rama_Table_Type const type ) {
		runtime_assert_string_msg( type > 0 && type < core::scoring::unknown_ramatable_type,
			"Error in protocols::generalized_kinematic_closure::perturber::GeneralizedKicPerturber::set_custom_rama_table(): the custom Ramachandran table type is unknown.");
		custom_rama_table_ = type;
		return;
	}
...
private:
	...
	/// @brief Should poses generated by the perturber be used for ghost display in BOINC?
	/// @details Outside of the BOINC build, this does nothing.
	bool attach_boinc_ghost_observer_;

	/// @brief A parameter specifically for the perturb_alpha_backbone_by_rama perturber.  An optional
	/// custom Ramachandran map for sampling.  Defaults to unknown_ramatable_type (unused -- use default
	/// Ramachandran map for the amino acid residue in question.
