...
void
protocols::pose_metric_calculators::SaltBridgeCalculator::save( Archive & arc ) const {
...
template< class Archive >
void
protocols::pose_metric_calculators::SaltBridgeCalculator::load( Archive & arc ) {
	arc( cereal::base_class< core::pose::metrics::StructureDependentCalculator >( this ) );
	arc( distance_cutoff_ ); // core::Real
	arc( salt_bridge_total_ ); // core::Size
}

SAVE_AND_LOAD_SERIALIZABLE( protocols::pose_metric_calculators::SaltBridgeCalculator );
CEREAL_REGISTER_TYPE( protocols::pose_metric_calculators::SaltBridgeCalculator )
