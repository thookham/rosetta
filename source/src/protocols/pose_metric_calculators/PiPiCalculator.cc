...
void PiPiCalculator::recompute(core::pose::Pose const & pose){
	...
	for ( core::Size res_num1=1; res_num1 <= pose.size(); ++res_num1 ) {
		...
		if ( acceptor.name3() == "TYR" || acceptor.name3() =="HIS" || acceptor.name3() =="PHE" || acceptor.name3() == "TRP" ) {
			...
			for ( core::Size res_num2=res_num1; res_num2 <= pose.size(); ++res_num2 ) {
...
void
protocols::pose_metric_calculators::PiPiCalculator::save( Archive & arc ) const {
...
template< class Archive >
void
protocols::pose_metric_calculators::PiPiCalculator::load( Archive & arc ) {
	arc( cereal::base_class< core::pose::metrics::StructureDependentCalculator >( this ) );
	arc( distance_cutoff_ ); // core::Real
	arc( pi_pi_total_ ); // core::Size
}

SAVE_AND_LOAD_SERIALIZABLE( protocols::pose_metric_calculators::PiPiCalculator );
CEREAL_REGISTER_TYPE( protocols::pose_metric_calculators::PiPiCalculator )
