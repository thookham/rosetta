...
///
/// core::pose::metrics::PoseMetricCalculatorOP unsat_calculator = new protocols::pose_metric_calculators::SemiExplicitWaterUnsatisfiedPolarsCalculator("num_hbonds");
...
#include <core/scoring/hbonds/HBondDatabase.hh> // AUTO IWYU For HBondDatabase
...
SemiExplicitWaterUnsatisfiedPolarsCalculator::assert_calculators()
{
	if ( !CalculatorFactory::Instance().check_calculator_exists( name_of_hbond_calc_ ) ) {
		...
		if ( !CalculatorFactory::Instance().check_calculator_exists( name_of_hbond_calc_ ) ) {
			CalculatorFactory::Instance().register_calculator( name_of_hbond_calc_, utility::pointer::make_shared< NumberHBondsCalculator >() );
		}
	}
}
...
)
{
	...
	//protocols::minimization_packing::MinMoverOP min_mover = new protocols::minimization_packing::MinMover( mm, scorefxn, "lbfgs_armijo_nonmonotone", 0.01, true );
...
void
protocols::pose_metric_calculators::SemiExplicitWaterUnsatisfiedPolarsCalculator::save( Archive & arc ) const {
...
template< class Archive >
void
protocols::pose_metric_calculators::SemiExplicitWaterUnsatisfiedPolarsCalculator::load( Archive & arc ) {
	arc( cereal::base_class< core::pose::metrics::EnergyDependentCalculator >( this ) );
...
SAVE_AND_LOAD_SERIALIZABLE( protocols::pose_metric_calculators::SemiExplicitWaterUnsatisfiedPolarsCalculator );
CEREAL_REGISTER_TYPE( protocols::pose_metric_calculators::SemiExplicitWaterUnsatisfiedPolarsCalculator )
