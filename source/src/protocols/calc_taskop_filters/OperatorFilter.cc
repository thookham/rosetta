...
/// @file protocols/calc_taskop_filters/OperatorFilter.cc
/// @brief
...
Operator::reset_baseline( core::pose::Pose const & pose, bool const attempt_read_from_checkpoint ){
	for ( protocols::filters::FilterOP filter : filters() ) {
		if ( filter->get_type() == "Sigmoid" ) {
			SigmoidOP sigmoid_filter( utility::pointer::dynamic_pointer_cast< protocols::calc_taskop_filters::Sigmoid > ( filter ) );
		...
		if ( filter->get_type() == "MultipleSigmoids" ) {
			MultipleSigmoidsOP multisigmoid_filter( utility::pointer::dynamic_pointer_cast< protocols::calc_taskop_filters::MultipleSigmoids > ( filter ) );
			...
			OperatorOP operator_filter( utility::pointer::dynamic_pointer_cast< protocols::calc_taskop_filters::Operator > ( filter ) );
			...
			for ( auto & cs_it : *comp_filt_op ) {
				...
				if ( f->get_type() == "Sigmoid" ) {
					SigmoidOP sigmoid_filter( utility::pointer::dynamic_pointer_cast< protocols::calc_taskop_filters::Sigmoid > ( f ) );
					runtime_assert( sigmoid_filter != nullptr );
					sigmoid_filter->reset_baseline( pose, attempt_read_from_checkpoint );
					TR<<"Resetting Sigmoid filter's baseline"<<std::endl;
				} else if ( f->get_type() == "Operator" ) { //recursive call
					OperatorOP operator_filter( utility::pointer::dynamic_pointer_cast< protocols::calc_taskop_filters::Operator > ( f ) );
					runtime_assert( operator_filter != nullptr );
					operator_filter->reset_baseline( pose, attempt_read_from_checkpoint );
					TR<<"Resetting Operator filter's baseline"<<std::endl;
				}
