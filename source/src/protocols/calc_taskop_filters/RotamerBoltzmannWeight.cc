...
RotamerBoltzmannWeight::first_pass_ala_scan( core::pose::Pose const & pose ) const
{
	...
	if ( !skip_ala_scan() ) {
		protocols::simple_ddg::DdgFilter ddg_filter( 100/*ddg_threshold*/, scorefxn(), jump, 1 /*repeats*/ );
...
RotamerBoltzmannWeight::compute( core::pose::Pose const & const_pose ) const{
	...
	protocols::pose_metric_calculators::RotamerBoltzCalculator rotboltz_calc( this->scorefxn(), this->temperature(), this->repacking_radius() );
...
RotamerBoltzmannWeight::compute_Boltzmann_weight( core::pose::Pose const & const_pose, core::Size const resi ) const
{
	...
	min_mover = utility::pointer::make_shared< protocols::minimization_packing::MinMover >( mm, scorefxn_, "lbfgs_armijo_nonmonotone", 0.01, true, false, false ); // NK 110621
...
RotamerBoltzmannWeight::fresh_instance() const{
	return utility::pointer::make_shared< RotamerBoltzmannWeight >();
}
...
RotamerBoltzmannWeight::clone() const{
	return utility::pointer::make_shared< RotamerBoltzmannWeight >( *this );
}
...
RotamerBoltzmannWeight::compute_modified_ddG( core::pose::Pose const & pose, std::ostream & out ) const
{
	...
	protocols::simple_ddg::DdgFilter ddg_filter( 100/*ddg_threshold*/, scorefxn(), rb_jump(), repack() ? 3 : 1 /*repeats*/ ); //120423
	ddg_filter.repack( repack() );
