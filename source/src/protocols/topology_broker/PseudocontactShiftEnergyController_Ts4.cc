...
static basic::Tracer tr_control( "protocols.topology_broker.PseudocontactShiftEnergyController_Ts4" );
...
void
PseudocontactShiftEnergyController_Ts4::set_defaults( ){

}

bool PseudocontactShiftEnergyController_Ts4::read_tag( std::string tag, std::istream& is ) {
	using namespace protocols::scoring::methods::pcsTs4;

	if ( tag == "TS4_GRID_EDGE_SIZE" ) {
		if ( (is >>
				grid_edge_stage1_ >>
				grid_edge_stage2_ >>
				grid_edge_stage3_ >>
	...
	if ( tag == "TS4_GRID_CONE_ANGLE_CUTOFF" ) {
		if ( (is >>
	...
	if ( tag == "TS4_VECTOR_ATOM1_RESIDUE_NUM" ) {
		if ( (is >>
	...
	if ( tag == "TS4_VECTOR_ATOM2_RESIDUE_NUM" ) {
		if ( (is >>
	...
	if ( tag == "TS4_MINIMIZE_BEST_TENSOR" ) {
		core::Size a, b, c, d;
		if ( (is >> a >> b >>c >> d).fail() ) {
...
void PseudocontactShiftEnergyController_Ts4::add_mover(
...
)
{
	...
	if ( stageID == abinitio::STAGE_1 ) {
		...
		PCS_Energy_parameters_manager_Ts4::get_instance()->set_grid_param(grid_edge_stage1_,
			grid_step_stage1_,
			grid_small_cutoff_stage1_,
			grid_large_cutoff_stage1_,
			grid_cone_angle_cutoff_stage1_,
	...
	if ( stageID == abinitio::STAGE_2 ) {
		...
		PCS_Energy_parameters_manager_Ts4::get_instance()->set_grid_param(grid_edge_stage2_,
			grid_step_stage2_,
			grid_small_cutoff_stage2_,
			grid_large_cutoff_stage2_,
			grid_cone_angle_cutoff_stage2_,
	...
	if ( stageID == abinitio::STAGE_3a ) {
		...
		PCS_Energy_parameters_manager_Ts4::get_instance()->set_grid_param(grid_edge_stage3_,
			grid_step_stage3_,
			grid_small_cutoff_stage3_,
			grid_large_cutoff_stage3_,
			grid_cone_angle_cutoff_stage3_,
	...
	if ( stageID == abinitio::STAGE_4 ) {
		...
		PCS_Energy_parameters_manager_Ts4::get_instance()->set_grid_param(grid_edge_stage4_,
			grid_step_stage4_,
...
void
PseudocontactShiftEnergyController_Ts4::init_after_reading(){
	using namespace protocols::scoring::methods::pcsTs4;
	control_grid_param();
	...
	PCS_Energy_parameters_manager_Ts4::get_instance()->set_vector_name_and_weight(filenames_, individual_weights_);

}

void
PseudocontactShiftEnergyController_Ts4::control_grid_param(){
