...
void DensityScoringClaimer::generate_sequence_claims( claims::DofClaims& new_claims ){
	...
	new_claims.push_back ( utility::pointer::make_shared< claims::SequenceClaim >( get_self_weak_ptr(), "X", label() ));
}
...
void DensityScoringClaimer::generate_claims( claims::DofClaims& new_claims ) {
	...
	new_claims.push_back( utility::pointer::make_shared< claims::LegacyRootClaim >( get_self_weak_ptr(), local_vrt_pos, claims::DofClaim::EXCLUSIVE) );
	new_claims.push_back( utility::pointer::make_shared< claims::JumpClaim >( get_self_weak_ptr(), local_vrt_pos, local_anchor_pos, "ORIG", "CA", claims::DofClaim::EXCLUSIVE ) );
...
TopologyClaimerOP DensityScoringClaimer::clone() const {
	return utility::pointer::make_shared< DensityScoringClaimer >( *this );
}
