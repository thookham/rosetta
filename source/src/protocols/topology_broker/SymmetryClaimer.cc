...
bool SymmetryClaimer::read_tag( std::string tag, std::istream& in ){
	...
	if ( tag == "symmetry_definition" ) {
		...
		symm_data_ = utility::pointer::make_shared< core::conformation::symmetry::SymmData >();
...
void SymmetryClaimer::generate_symmetry_claims( claims::SymmetryClaims& symm_claims ){
	symm_claims.push_back( utility::pointer::make_shared< claims::SymmetryClaim >( get_self_weak_ptr(), symm_data_,
...
void SymmetryClaimer::symmetry_duplicate( claims::DofClaims& pre_accepted,
	...
	for ( core::Size j=1; j<symm_data_->get_subunits(); ++j ) {
		...
		for ( core::Size i=1; i<= original_claims.size(); ++i ) {
			...
			pre_accepted.push_back( utility::pointer::make_shared< claims::SequenceClaim >( get_self_weak_ptr(),
	...
	for ( core::Size i=1; i<=symm_data_->get_num_virtual(); ++i ) {
		...
		pre_accepted.push_back( utility::pointer::make_shared< claims::SequenceClaim >( get_self_weak_ptr(), "X", tag ));
	}
