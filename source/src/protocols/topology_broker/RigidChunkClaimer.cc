...
void RigidChunkClaimer::new_decoy() {
	...
	current_jump_calculator_ = utility::pointer::make_shared< JumpCalculator >( current_rigid_core_, bAllowAdjacentJumps_ );
...
bool RigidChunkClaimer::allow_claim( claims::DofClaim const& foreign_claim ) {
	...
	if ( jump_ptr ) {
		...
		if ( current_jump_calculator_->irrelevant_jump( jump_ptr->global_pos1(), jump_ptr->global_pos2() ) ) {
			...
			//reclaim the claim
			current_jumps_.push_back( utility::pointer::make_shared< claims::JumpClaim >( get_self_weak_ptr(), jump_ptr->local_pos1(), jump_ptr->local_pos2(), claims::DofClaim::EXCLUSIVE ) );
			return false;
		} else {
