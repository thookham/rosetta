...
FoldandDockClaimer::clone() const {
	return utility::pointer::make_shared< FoldandDockClaimer >( *this );
}
...
void FoldandDockClaimer::initialize_dofs(
	...
	protocols::symmetry::SetupForSymmetryMoverOP setup_mover( new
	...
	if ( !core::pose::symmetry::is_symmetric( pose ) ) {
		...
		protocols::symmetry::SymDockingInitialPerturbation initial( true /*slide into contact*/ );
		initial.apply( pose );
	}
