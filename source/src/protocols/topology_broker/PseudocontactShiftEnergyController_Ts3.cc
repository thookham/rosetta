...
static basic::Tracer tr_control( "protocols.topology_broker.PseudocontactShiftEnergyController_Ts3" );
...
PseudocontactShiftEnergyController_Ts3::PseudocontactShiftEnergyController_Ts3()
{
...
void
PseudocontactShiftEnergyController_Ts3::set_defaults( ){

}

bool PseudocontactShiftEnergyController_Ts3::read_tag( std::string tag, std::istream& is ) {
	using namespace protocols::scoring::methods::pcsTs3;

	if ( tag == "TS3_GRID_EDGE_SIZE" ) {
		if ( (is >>
				grid_edge_stage1_ >>
				grid_edge_stage2_ >>
				grid_edge_stage3_ >>
	...
	if ( tag == "TS3_GRID_CONE_ANGLE_CUTOFF" ) {
		if ( (is >>
	...
	if ( tag == "TS3_VECTOR_ATOM1_RESIDUE_NUM" ) {
		if ( (is >>
	...
	if ( tag == "TS3_VECTOR_ATOM2_RESIDUE_NUM" ) {
		if ( (is >>
	...
	if ( tag == "TS3_MINIMIZE_BEST_TENSOR" ) {
		core::Size a, b, c, d;
		if ( (is >> a >> b >>c >> d).fail() ) {
...
void PseudocontactShiftEnergyController_Ts3::add_mover(
...
)
{
	...
	// PCS_Energy_parameters_manager_Ts3::get_instance()->set_vector_exclude_residues(vec_exclude);
	//}


	if ( stageID == abinitio::STAGE_1 ) {

		PCS_Energy_parameters_manager_Ts3::get_instance()->set_grid_param(grid_edge_stage1_,
			grid_step_stage1_,
			grid_small_cutoff_stage1_,
			grid_large_cutoff_stage1_,
			grid_cone_angle_cutoff_stage1_,
	...
	if ( stageID == abinitio::STAGE_2 ) {
		...
		PCS_Energy_parameters_manager_Ts3::get_instance()->set_grid_param(grid_edge_stage2_,
			grid_step_stage2_,
			grid_small_cutoff_stage2_,
			grid_large_cutoff_stage2_,
			grid_cone_angle_cutoff_stage2_,
	...
	if ( stageID == abinitio::STAGE_3a ) {
		...
		PCS_Energy_parameters_manager_Ts3::get_instance()->set_grid_param(grid_edge_stage3_,
			grid_step_stage3_,
			grid_small_cutoff_stage3_,
			grid_large_cutoff_stage3_,
			grid_cone_angle_cutoff_stage3_,
	...
	if ( stageID == abinitio::STAGE_4 ) {
		...
		PCS_Energy_parameters_manager_Ts3::get_instance()->set_grid_param(grid_edge_stage4_,
			grid_step_stage4_,
...
void
PseudocontactShiftEnergyController_Ts3::init_after_reading(){
	using namespace protocols::scoring::methods::pcsTs3;
	control_grid_param();
	...
	PCS_Energy_parameters_manager_Ts3::get_instance()->set_vector_name_and_weight(filenames_, individual_weights_);

}

void
PseudocontactShiftEnergyController_Ts3::control_grid_param(){
