// -*- mode:c++;tab-width:2;indent-tabs-mode:t;show-trailing-whitespace:t;rm-trailing-spaces:t -*-
// vi: set ts=2 noet:
//
// (c) Copyright Rosetta Commons Member Institutions.
// (c) This file is part of the Rosetta software suite and is made available under license.
// (c) The Rosetta software is developed by the contributing members of the Rosetta Commons.
// (c) For more information, see http://www.rosettacommons.org. Questions about this can be
// (c) addressed to University of Washington CoMotion, email: license@uw.edu.

/// @file StartingNodeMotifScorer.hh
///
/// @brief
/// @author Tim Jacobs

#ifndef INCLUDED_protocols_sewing_scoring_StartingNodeMotifScorer_hh
#define INCLUDED_protocols_sewing_scoring_StartingNodeMotifScorer_hh

//Unit headers
#include <protocols/sewing/scoring/StartingNodeMotifScorer.fwd.hh>
#include <protocols/sewing/scoring/MotifScorer.hh>

//Package headers
#include <protocols/sewing/data_storage/SmartAssembly.fwd.hh>

//Core headers

//Utility headers

namespace protocols {
namespace sewing  {
namespace scoring {

class StartingNodeMotifScorer : public MotifScorer {

public:

	///@brief default construct
	StartingNodeMotifScorer();

	~StartingNodeMotifScorer() override =default;

	StartingNodeMotifScorer( StartingNodeMotifScorer const & );

	core::Real
	score(
		data_storage::SmartAssemblyCOP assembly
	) override;
	static std::string type_name();

	core::Real
	get_weight() const override;

	void
	set_weight( core::Real ) override;



	core::Real
	full_motif_score(
		data_storage::SmartAssemblyCOP assembly
	);

	//core::Real
	//norm_motif_score(
	// AssemblyCOP assembly
	//);
	std::string
	get_name() const override;

	void
	set_options_from_tag(
		utility::tag::TagCOP scorer_tag,
		basic::datacache::DataMap& datamap
	) override;
	static void
	provide_xml_schema( utility::tag::XMLSchemaDefinition & );

private:
	core::Real weight_=1.0;

};


} //scoring namespace
} //sewing namespace
} //protocols namespace

#endif
