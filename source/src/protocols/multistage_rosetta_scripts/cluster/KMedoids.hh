...
/// @file   protocols/multistage_rosetta_scripts/cluster/KMedoids.hh
...
k_medoids_with_edge_precalculation (
	...
	utility::vector1< bool > medoid_positions;
	k_medoids_with_edge_precalculation( points, num_medoids, medoid_positions );
	return medoid_positions;
}

///@brief This is a simple utility for debugging. Makes sure we have the right number of medoids assigned.
inline bool
vector_has_n_trues(
	utility::vector1< bool > const & vec,
	int n
) {
	for ( bool b : vec ) {
		if ( b ) --n;
