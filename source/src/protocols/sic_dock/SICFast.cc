...
inline core::Real dist_score( core::Real const & sqdist, core::Real const & start, core::Real const & stop ) {
	if ( sqdist > stop*stop ) {
		return 0.0;
...
get_bounds_intersection(
	...
	core::Real & xmx, core::Real & xmn,
	core::Real & ymx, core::Real & ymn
){
	// get bounds for plane hashess
	core::Real xmx1=-9e9,xmn1=9e9,ymx1=-9e9,ymn1=9e9;
		...
		for ( int j = 1; j <= ysize; ++j ) {
			for ( int k = -1; k <= 1; ++k ) {
				...
				for ( int l = -1; l <= 1; ++l ) {
					...
					core::Real const xa1 = ha(i,j).x();
					core::Real const ya1 = ha(i,j).y();
					core::Real const xb1 = hb(i+k,j+l).x();
					core::Real const yb1 = hb(i+k,j+l).y();
					core::Real const d21 = (xa1-xb1)*(xa1-xb1)+(ya1-yb1)*(ya1-yb1);
					if ( d21<clashdis2 ) {
						core::Real const dz = hb(i+k,j+l).z() - ha(i,j).z() - sqrt(clashdis2-d21);
						if ( dz<m ) m=dz;
					}
				}
			}
...
struct CorrectionVisitor {
	...
	void visit(
		...
		numeric::xyzVector<core::Real> const & c,
		core::Real /*vr*/,
		core::Real /*cr*/
	){
		core::Real const dxy2 = dof.cross(v-c).length_squared();
		if ( dxy2 < clash_dis_sq ) {
			core::Real const dz = dof.dot(v) - dof.dot(c) - sqrt(clash_dis_sq-dxy2);
...
refine_mindis_with_xyzHash(
	...
	Vec const & ori,
	core::Real const & clash_dis_sq,
	core::Real const & mindis_approx
){
	core::Real mindis = mindis_approx;
