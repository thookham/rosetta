...
app_list_template = '''\
// THIS FILE IS AUTOMATICALLY GENERATED BY source/src/ui/update_ui_project.py script
#pragma once

namespace ui {{
namespace util {{

auto const rosetta_app_list = {{ {apps} }};
...
def generate_app_list(rosetta_source_path):
    scons_app_file = rosetta_source_path + '/src/apps.src.settings'

    apps, G = [], {}
    with open(scons_app_file) as f:
        code = compile(f.read(), scons_app_file, 'exec')
        exec(code, G)

        for dir_ in G['sources']:
            for f in G['sources'][dir_]:
                _, _, app = f.rpartition('/')
                if app: apps.append( '"' + app + '"' )

    apps.sort()

    update_source_file('util/apps.gen.h', app_list_template.format(apps = ', '.join(apps)) )
...
def main(args):
    ...
    generate_app_list(rosetta_source_path)
