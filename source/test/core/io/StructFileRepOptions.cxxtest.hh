...
/// @file   test/core/io/StructFileRepOptions.cxxtest.hh
/// @brief  Test suite for StructFileRepOptions's methods
...
class StructFileRepOptionsTests : public CxxTest::TestSuite {
public:
	typedef utility::keys::VariantKey< utility::options::OptionKey > VariantOptionKey;

	/// @brief Make sure that StructFileRepOptions::list_options_read lists every one of
	/// the options that the StructFileRepOptions object reads in its init_from_options
	...
	void test_options_and_options_list_in_sync()
	{
		...
		core::io::StructFileRepOptions::list_options_read( sfro_options );
		// cursory check of some of the options known to be read by the SFRO
		TS_ASSERT_EQUALS( std::count( sfro_options.begin(), sfro_options.end(), VariantOptionKey( basic::options::OptionKeys::in::Ntermini )), 1 );
		TS_ASSERT_EQUALS( std::count( sfro_options.begin(), sfro_options.end(), VariantOptionKey( basic::options::OptionKeys::in::remember_unrecognized_res )), 1 );
		TS_ASSERT_EQUALS( std::count( sfro_options.begin(), sfro_options.end(), VariantOptionKey( basic::options::OptionKeys::in::file::treat_residues_in_these_chains_as_separate_chemical_entities )), 1 );
		...
		try {
			...
			core::io::StructFileRepOptions sfro( *sfro_option_collection );
		} catch ( ... ) {
