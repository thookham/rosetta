...
/// @file  core/pack/rotamers/SingleNCAARotamerLibraryTests_no_voronoi.cxxtest.hh
/// @brief  Unit tests for the non-canonical rotamer libraries, with the Voronoi detection disabled.
...
static basic::Tracer TR("SingleNCAARotamerLibraryTests_Dun02_no_voronoi");
#define TEST_DELTA 1.0e-6

class SingleNCAARotamerLibraryTests_Dun02_no_voronoi : public CxxTest::TestSuite {
	//Define Variables
...
public:
	...
	void setUp(){
		...
		core_init_with_additional_options("-extra_res_fa core/pack/rotamers/LEU_NCAA_DUN02.params -dun10 false -fa_dun_canonicals_use_voronoi false -fa_dun_noncanonicals_use_voronoi false");
	}
	...
	void test_equivalent_rotlib_values_to_canonical() {
		...
		pose::make_pose_from_sequence(pose_ncaa, "GX[LEU_NCAA_DUN02]G", "fa_standard");
		...
		for ( Real phi = -180; phi <= 180; phi += 1 ) {
			for ( Real psi = -180; psi <= 180; psi += 1 ) {
				...
				TS_ASSERT_DELTA( nleu_samples.size(), cleu_samples.size(), TEST_DELTA );
	...
	void test_equivalent_scoring_to_canonical() {
		...
		core::pose::make_pose_from_sequence(pose_ncaa, "GX[LEU_NCAA_DUN02]G", "fa_standard");
