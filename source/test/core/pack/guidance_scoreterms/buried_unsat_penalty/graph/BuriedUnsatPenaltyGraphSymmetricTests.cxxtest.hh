...
/// @file  core/scoring/buried_unsat_penalty/graph/BuriedUnsatPenaltyGraphSymmetricTests.cxxtest.hh
...
//Convenience headers from Protocols
#include <protocols/symmetry/SetupForSymmetryMover.hh>

static basic::Tracer TR("BuriedUnsatPenaltyGraphSymmetricTests");


class BuriedUnsatPenaltyGraphSymmetricTests : public CxxTest::TestSuite {
	//Define Variables
...
public:
	...
	void test_graph_setup() {
		...
		core::pack::guidance_scoreterms::buried_unsat_penalty::graph::BuriedUnsatPenaltyGraph graph( pose.total_residue() / 3, options, hboptions );
		...
		{ //Tests for residue ASN54
			...
			TS_ASSERT_EQUALS( curnode.num_edges(), 3 );
			...
			for ( utility::graph::EdgeListConstIterator it( curnode.const_edge_list_begin() ); it != curnode.const_edge_list_end(); ++it ) {
				...
				if ( count == 2 ) {
					TS_ASSERT_EQUALS( don_node_index, 54 );
					TS_ASSERT_EQUALS( acc_node_index, 50 );
					TS_ASSERT_EQUALS( curhbond.acceptor_symmetry_copy_index(), 1 );
					TS_ASSERT_EQUALS( curhbond.donor_symmetry_copy_index(), 1 );
					...
					TS_ASSERT_EQUALS( don_node_index, 58 );
					TS_ASSERT_EQUALS( acc_node_index, 54 );
					TS_ASSERT_EQUALS( curhbond.acceptor_symmetry_copy_index(), 1 );
					TS_ASSERT_EQUALS( curhbond.donor_symmetry_copy_index(), 1 );
					...
					TS_ASSERT_EQUALS( don_node_index, 54 );
					TS_ASSERT_EQUALS( acc_node_index, 53 );
					TS_ASSERT_EQUALS( curhbond.acceptor_symmetry_copy_index(), 3 );
					TS_ASSERT_EQUALS( curhbond.donor_symmetry_copy_index(), 1 );
	...
	void test_smallpose_graph_setup() {
		...
		*/
		...
		{ //Tests for residue ASN2
			TR << "\nTESTING NODE 2's CONNECTIONS:\n";
			using namespace core::pack::guidance_scoreterms::buried_unsat_penalty::graph;
			BuriedUnsatPenaltyNode const & curnode( *(dynamic_cast<BuriedUnsatPenaltyNode*>(graph.get_node(2))) );
			TS_ASSERT_EQUALS( curnode.num_edges(), 0 );
			TS_ASSERT_EQUALS( curnode.stored_data_->donor_acceptor_groups_equivalent_res_symmetry_copy_hbonds_accepted_.size(), 4 );
			TS_ASSERT_EQUALS( curnode.stored_data_->donor_acceptor_groups_equivalent_res_symmetry_copy_hbonds_donated_.size(), 4 );
			TS_ASSERT_EQUALS( curnode.stored_data_->donor_acceptor_groups_equivalent_res_symmetry_copy_hbonds_accepted_[1], 0 );
			TS_ASSERT_EQUALS( curnode.stored_data_->donor_acceptor_groups_equivalent_res_symmetry_copy_hbonds_accepted_[2], 1 );
			TS_ASSERT_EQUALS( curnode.stored_data_->donor_acceptor_groups_equivalent_res_symmetry_copy_hbonds_accepted_[3], 0 );
			TS_ASSERT_EQUALS( curnode.stored_data_->donor_acceptor_groups_equivalent_res_symmetry_copy_hbonds_accepted_[4], 0 );
			TS_ASSERT_EQUALS( curnode.stored_data_->donor_acceptor_groups_equivalent_res_symmetry_copy_hbonds_donated_[1], 0 );
			TS_ASSERT_EQUALS( curnode.stored_data_->donor_acceptor_groups_equivalent_res_symmetry_copy_hbonds_donated_[2], 0 );
			TS_ASSERT_EQUALS( curnode.stored_data_->donor_acceptor_groups_equivalent_res_symmetry_copy_hbonds_donated_[3], 0 );
			TS_ASSERT_EQUALS( curnode.stored_data_->donor_acceptor_groups_equivalent_res_symmetry_copy_hbonds_donated_[4], 1 );
		}
		TR.flush();
	}
