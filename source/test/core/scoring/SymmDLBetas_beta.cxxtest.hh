...
/// @file test/core/scoring/SymmDLBetas.cxxtest.hh
/// @brief Unit tests for beta-aa scoring with the --symmetric_gly_tables option.
...
static basic::Tracer TR("core.scoring.SymmDLBetasTests_beta.cxxtest");

class SymmDLBetasTests_beta : public CxxTest::TestSuite {

public:

	void setUp() {
		core_init_with_additional_options( "-beta -symmetric_gly_tables true" );
	}
	...
	void repeat_structure_test( core::scoring::ScoreFunctionOP sfxn ) {
		...
		core::pose::make_pose_from_sequence(*pose, "X[B3A]X[B2A]X[B3P]X[B3G]", "fa_standard", false);
		auto pose2 = mirror_pose(pose);
	...
	/// @author Andy Watkins (watkina6@gene.com)
	void dont_test_symm_DLBetas_cart_bonded() {
	...
	/// @author Andy Watkins (watkina6@gene.com)
	void test_symm_DLBetas_fa_intra_rep() {
		//Set up the scorefunction
		core::scoring::ScoreFunctionOP scorefxn( new core::scoring::ScoreFunction );
	...
	/// @author Andy Watkins (watkina6@gene.com)
	void test_symm_DLBetas_fa_elec() {
		//Set up the scorefunction
		core::scoring::ScoreFunctionOP scorefxn( new core::scoring::ScoreFunction );
	...
	/// @author Andy Watkins (watkina6@gene.com)
	void test_symm_DLBetas_rama_prepro() {
		//Set up the scorefunction
		core::scoring::ScoreFunctionOP scorefxn( new core::scoring::ScoreFunction );
	...
	/// @author Andy Watkins (watkina6@gene.com)
	void test_symm_DLBetas_p_aa_pp() {
		//Set up the scorefunction
		core::scoring::ScoreFunctionOP scorefxn( new core::scoring::ScoreFunction );
	...
	/// @author Andy Watkins (watkina6@gene.com)
	void test_symm_DLBetas_default_scorefxn() {
		//Set up the scorefunction
	...
	/// @brief Tests symmetric scoring and minimization of a chiral beta pose with the full default scorefunction, whatever it currently is.
	/// @details Skip because of pathology, file as issue, debug later.
	/// @author Andy Watkins (watkina6@gene.com)
	void dont_test_symm_min_DLBetas_default_scorefxn() {
		//Set up the scorefunction
		core::scoring::ScoreFunctionOP scorefxn( core::scoring::get_score_function() );
