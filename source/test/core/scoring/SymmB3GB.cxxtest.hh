...
static basic::Tracer TR("core.scoring.SymmB3GBTests.cxxtest");

class SymmB3GBTests : public CxxTest::TestSuite {

public:

	void setUp() {
		//core_init();
		core_init_with_additional_options( "-symmetric_gly_tables true -corrections:beta_nov16" );
	}
	...
	/// @brief Tests symmetric scoring of beta-3-glycine with the full beta scorefunction.
	/// @author Andy Watkins (watkina6@gene.com)
	void test_symm_B3G_beta_scorefxn() {
		//Set up the scorefunction
		core::scoring::ScoreFunctionOP scorefxn( core::scoring::ScoreFunctionFactory::create_score_function( "beta_nov16.wts" ) );
		TR << "Testing full beta score function." << std::endl;
		repeat_structure_test(scorefxn);
		return;
	}

	/// @brief Tests symmetric scoring of beta-3-glycine with minimization and one scoreterm.
	/// @details Skip because of pathology, file as issue, debug later.
	/// @author Andy Watkins (watkina6@gene.com)
	void dont_test_symm_min_B3G_scoreterm() {
		//Set up the scorefunction
		core::scoring::ScoreFunctionOP scorefxn( new core::scoring::ScoreFunction );
		scorefxn->set_weight( core::scoring::rama_prepro, 1.0 );
		TR << "Testing rama_prepro score term, with minimization." << std::endl;
	...
	/// @details Skip because of pathology, file as issue, debug later.
	/// @author Andy Watkins (watkina6@gene.com)
	void dont_test_symm_min_B3G_beta_scorefxn() {
		//Set up the scorefunction
		core::scoring::ScoreFunctionOP scorefxn( core::scoring::ScoreFunctionFactory::create_score_function( "beta_nov16.wts" ) );
		TR << "Testing full beta score function, with minimization." << std::endl;
		repeat_structure_min_test(scorefxn);
		return;
	}
