...
class AACompositionEnergyTests_10pctaro : public CxxTest::TestSuite {
...
public:
	...
	void test_energy_eval_ten_percent_aromatic() {
		...
		if ( TR.visible() ) {
			TR << "Starting AACompositionEnergyTests_10pctaro::test_energy_eval_ten_percent_aromatic()." << std::endl;
		...
		if ( TR.visible() ) {
			TR << "Test AACompositionEnergyTests_10pctaro::test_energy_eval_ten_percent_aromatic() complete." << std::endl;
			TR.flush();
		}
		return;
	}
