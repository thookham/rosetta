...
class AACompositionEnergyTests_20pctpro : public CxxTest::TestSuite {
...
public:
	...
	void test_energy_eval_twenty_percent_pro() {
		...
		if ( TR.visible() ) {
			TR << "Starting AACompositionEnergyTests_20pctpro::test_energy_eval_twenty_percent_pro()." << std::endl;
		...
		if ( TR.visible() ) {
			TR << "Test AACompositionEnergyTests_20pctpro::test_energy_eval_twenty_percent_pro() complete." << std::endl;
			TR.flush();
		}
		return;
	}
