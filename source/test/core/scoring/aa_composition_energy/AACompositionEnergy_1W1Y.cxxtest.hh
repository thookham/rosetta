...
class AACompositionEnergyTests_1W1Y : public CxxTest::TestSuite {
...
public:
	...
	void test_energy_eval_one_trp_one_tyr() {
		...
		if ( TR.visible() ) {
			TR << "Starting AACompositionEnergyTests_1W1Y::test_energy_eval_one_trp_one_tyr()." << std::endl;
		...
		if ( TR.visible() ) {
			TR << "Test AACompositionEnergyTests_1W1Y::test_energy_eval_one_trp_one_tyr() complete." << std::endl;
			TR.flush();
		}
		return;
	}
