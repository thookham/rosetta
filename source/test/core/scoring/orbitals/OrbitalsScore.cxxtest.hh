...
public:
	core::chemical::ResidueTypeSetCOP orbitals_residue_type_set_;
	...
	void setUp() {
		...
		orbitals_residue_type_set_ = utility::pointer::make_shared< core::chemical::ResidueTypeSet >( "ORBITALS_FA_STANDARD", basic::database::full_name("chemical/residue_type_sets/fa_standard/") );
	...
	void test_orbital_scoring_function_values()
	{
		...
		TS_ASSERT_DELTA( -6.5336, start_score, 0.0001 );
	...
	void test_orbital_start_score_start_func_match_w_total_flexibility()
	{
		...
		adv.validate_start_func_matches_start_score( -6.5336, false, 1e-3 );
	}
	...
	void test_orbital_deriv_check_w_total_flexibility_all()
	{
		...
		kinematics::MoveMap movemap( create_movemap_to_allow_all_torsions() );
		AtomDerivValidator adv( pose, sfxn, movemap );
		adv.simple_deriv_check( false, /*1e-3*/0.025 ); // let's be more permissive for now
	}
	...
	void test_orbital_deriv_check_w_total_flexibility_pci_pi_pi()
	{
		...
		kinematics::MoveMap movemap( create_movemap_to_allow_all_torsions() );
		AtomDerivValidator adv( pose, sfxn, movemap );
		adv.simple_deriv_check( false, /*1e-3*/ 0.025 );
	}
