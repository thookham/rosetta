...
public:
	...
	void test_constraint_set_remapping()
	{
		...
		pose.add_constraint( utility::pointer::make_shared< AtomPairConstraint >( AtomID(1, 12), AtomID(3, 57), some_func ) );
		pose.add_constraint( utility::pointer::make_shared< AtomPairConstraint >( AtomID(6, 20), AtomID(1, 17), some_other_func ) );
		pose.add_constraint( utility::pointer::make_shared< AtomPairConstraint >( AtomID(7, 15), AtomID(3, 16), some_other_func ) );
		pose.add_constraint( utility::pointer::make_shared< AngleConstraint >( AtomID(6, 19), AtomID(1, 3), AtomID(10, 3 ), some_func ) );
		pose.add_constraint( utility::pointer::make_shared< AngleConstraint >( AtomID(4, 69), AtomID(1, 57), AtomID(3, 30 ), some_func ) );
		pose.add_constraint( utility::pointer::make_shared< DihedralConstraint >( AtomID(4, 65), AtomID(1, 65), AtomID(3, 65 ), AtomID(2, 65 ), some_func ) );
		pose.add_constraint( utility::pointer::make_shared< DihedralConstraint >( AtomID(8, 44), AtomID(1, 44), AtomID(2, 48 ), AtomID(1, 48 ), some_func ) );
		pose.add_constraint( utility::pointer::make_shared< ResidueTypeConstraint >( pose, 33, 1.0 ) );

		utility::vector1< ConstraintCOP > multi_csts;
		utility::vector1< ConstraintCOP > ambig_csts;

		multi_csts.push_back( utility::pointer::make_shared< AngleConstraint >( AtomID(6, 15), AtomID(1, 15), AtomID(3, 3 ), some_func ) );
		multi_csts.push_back( utility::pointer::make_shared< AtomPairConstraint >( AtomID(2, 15), AtomID(2, 3),  some_func ) );
		multi_csts.push_back( utility::pointer::make_shared< DihedralConstraint >( AtomID(2, 15), AtomID(7, 15), AtomID(1, 65), AtomID(3, 65 ),  some_func ) );

		ambig_csts.push_back( utility::pointer::make_shared< AngleConstraint >( AtomID(6, 33), AtomID(1, 33), AtomID(3, 30 ), some_func ) );
		ambig_csts.push_back( utility::pointer::make_shared< AtomPairConstraint >( AtomID(2, 35), AtomID(2, 13),  some_func ) );
		ambig_csts.push_back( utility::pointer::make_shared< DihedralConstraint >( AtomID(2, 35), AtomID(7, 35), AtomID(1, 45), AtomID(3, 65 ),  some_func ) );

		ConstraintCOP posemulticst = pose.add_constraint( utility::pointer::make_shared< MultiConstraint >( multi_csts ) );
		ConstraintCOP poseambigcst = pose.add_constraint( utility::pointer::make_shared< AmbiguousConstraint >( ambig_csts ) );

		//first test whether constraint == operators work
		utility::vector1< ConstraintCOP > testeq_csts;
		testeq_csts.push_back( utility::pointer::make_shared< AngleConstraint >(AtomID(6, 15), AtomID(1, 15), AtomID(3, 3 ), some_func ) );
		testeq_csts.push_back( utility::pointer::make_shared< AtomPairConstraint >( AtomID(2, 15), AtomID(2, 3),  some_func ) );
		testeq_csts.push_back( utility::pointer::make_shared< DihedralConstraint >( AtomID(2, 15), AtomID(7, 15), AtomID(1, 65), AtomID(3, 65 ),  some_func ) );
		...
		testeq_ambig_csts.push_back( utility::pointer::make_shared< AngleConstraint >( AtomID(6, 33), AtomID(1, 33), AtomID(3, 30 ), some_func ) );
		testeq_ambig_csts.push_back( utility::pointer::make_shared< AtomPairConstraint >( AtomID(2, 35), AtomID(2, 13),  some_func ) );
		testeq_ambig_csts.push_back( utility::pointer::make_shared< DihedralConstraint >( AtomID(2, 35), AtomID(7, 35), AtomID(1, 45), AtomID(3, 65 ),  some_func ) );
		...
		ConstraintCOP bstubcst =  pose.add_constraint( utility::pointer::make_shared< BackboneStubConstraint >( pose, 15, fixed_pt, pose.residue(19), -80.0, 0.2) );
		(*scorefxn)( pose );
