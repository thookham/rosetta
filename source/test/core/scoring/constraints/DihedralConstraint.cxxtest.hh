...
public:
	...
	void test_dihe_constraints()
	{
		...
		//protocols::minimization_packing::MinMover min_mover( mm, scorefxn, "dfpmin_armijo_nonmonotone_atol", 0.001, true /*use_nblist*/ );
		...
		for ( core::Size i = 1; i <= constraints_orig.size(); ++i ) {
			...
			TS_ASSERT_EQUALS( constraints_orig[i], constraints_copy[i] );
			TS_ASSERT_EQUALS( constraints_orig[i].get(), constraints_copy[i].get() );
