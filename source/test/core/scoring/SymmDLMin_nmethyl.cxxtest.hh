...
/// @file test/core/scoring/SymmDLMin_nmethyl.cxxtest.hh
/// @brief Unit tests for minimiziation of mirror-image poses with the --symmetric_gly_tables option and N-methylation.
...
static basic::Tracer TR("core.scoring.SymmDLMin_nmethyl_Tests.cxxtest");

class SymmDLMin_nmethyl_Tests : public CxxTest::TestSuite {
...
public:
	...
	void mirror_pose_test( core::scoring::ScoreFunctionOP sfxn, bool const cartesian ) {
		...
		protocols::simple_moves::MutateResidue mutres( 5, "ALA:N_Methylation" );
		protocols::simple_moves::MutateResidue mutres2( 4, "TRP:N_Methylation" );
		...
		mutres2.apply(pose);
		pose.set_chi( 1, 4, -56.9 );
		pose.set_chi( 2, 4, -28.1 );
		//}
		//}
		...
		//pose.dump_pdb("L_PRE.pdb");
		//pose2.dump_pdb("R_PRE.pdb");
		...
		//DELETE THE FOLLOWING TWO LINES.
		//pose.dump_pdb("L_POST.pdb");
		//pose2.dump_pdb("R_POST.pdb");
		...
		for ( core::Size ir=1, irmax=pose.size(); ir<=irmax; ++ir ) {
			...
			TS_ASSERT( within_thresh( pose.phi(ir), -1.0*pose2.phi(ir), 0.1 ) );
			TS_ASSERT( within_thresh( pose.psi(ir), -1.0*pose2.psi(ir), 0.1 ) );
			TS_ASSERT( within_thresh( pose.omega(ir), -1.0*pose2.omega(ir), 0.1 ) );
			for ( core::Size ichi=1, ichimax=pose.residue(ir).nchi(); ichi<=ichimax; ++ichi ) {
				...
				TS_ASSERT( within_thresh( pose.chi(ichi, ir), -1.0*pose2.chi(ichi, ir), 0.1 ) );
			}
		}
		TR.flush();
	...
	/// @brief Construct a few L-amino acid poses, and confirm that mirror-image conformations
	/// score identically with a given scorefunction.
	/// @details This version ensures that there's a GLY-peptoid sequence.
	void mirror_pose_test2( core::scoring::ScoreFunctionOP sfxn, bool const cartesian ) {
		core::pose::Pose pose = pdb1rpb_pose();

		//Mutate residue 4 to ALA_nme.  Since residue 3 is a gly, this makes a nice gly-ALA_nme.
