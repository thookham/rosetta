...
/// @file core/select/jump_selector/AndJumpSelector.cxxtest.hh
/// @brief  test suite for core::select::jump_selector::AndJumpSelector
...
class AndJumpSelectorTests : public CxxTest::TestSuite {
...
public:
	...
	void test_and_jump_selector_odd_1mod5_lots_of_jumps() {
		...
		JumpSelectorOP and_js( new AndJumpSelector( odd_js, one_mod_5_js ) );
	...
	/// @brief Test AndJumpSelector::parse_my_tag
	void test_AndJumpSelector_parse_my_tag() {
	...
	/// @brief Test that an excpetion is thrown if the AndJumpSelector is ever initialized
	/// from parse_my_tag where no JumpSelectors have been provided.
	void test_AndJumpSelector_parse_my_tag_no_provided_selectors() {
	...
	void test_AndJumpSelector_parse_my_tag_selectors_not_in_datamap() {
		std::string tag_string = "<And name=and_js selectors=odd,one_mod_five/>";
		std::stringstream ss( tag_string );
		utility::tag::TagOP tag( new utility::tag::Tag() );
		tag->read( ss );
	...
	void test_AndJumpSelector_parse_subtag_v1() {
		std::string tag_string = "<And name=and_js>\n\t<JumpIndex jump=\"2\" />\n\t<JumpIndex jump=\"2\" />\n</And>";
		std::stringstream ss( tag_string );
		utility::tag::TagOP tag( new utility::tag::Tag() );
		tag->read( ss );
	...
	void test_AndJumpSelector_parse_subtag_v2() {
		std::string tag_string = "<And name=and_js>\n\t<JumpIndex jump=\"2\" />\n\t<JumpIndex jump=\"3\" />\n</And>";
		std::stringstream ss( tag_string );
		utility::tag::TagOP tag( new utility::tag::Tag() );
		tag->read( ss );
	...
	void test_AndJumpSelector_fail_parse_subtag() {
		std::string tag_string = "<And name=and_js>\n\t<JumpIndex jump=\"2\"/>\n\t<Bogus />\n</And>";
		std::stringstream ss( tag_string );
		utility::tag::TagOP tag( new utility::tag::Tag() );
		tag->read( ss );
