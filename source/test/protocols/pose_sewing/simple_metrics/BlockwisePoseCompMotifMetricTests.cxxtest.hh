...
/// @file  protocols/pose_sewing/simple_metrics/BlockwisePoseCompMotifMetricTests.cxxtest.hh
/// @brief  tests the BlockwisePoseCompMotifMetric
...
static basic::Tracer TR("BlockwisePoseCompMotifMetricTests");


class BlockwisePoseCompMotifMetricTests : public CxxTest::TestSuite {
	//Define Variables
...
public:
	...
	void test_BWPCMM() {
		...
		protocols::pose_sewing::simple_metrics::BlockwisePoseCompMotifMetricOP metric( new protocols::pose_sewing::simple_metrics::BlockwisePoseCompMotifMetric );
		metric->set_selector(in_selector);
		metric->set_distance_mode(true);
		for ( auto current_pair : metric->calculate(test_3_bundle) ) {
			TS_ASSERT(current_pair.second < -8);
		}
