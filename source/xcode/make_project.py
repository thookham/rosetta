...
def project_callback(project, project_path, project_files):
    ...
    build_file_begin_ln = xcode_util.find_line(
        "/* Begin PBXBuildFile section */", lines, 0
    )
    build_file_end_ln = xcode_util.find_line(
        "/* End PBXBuildFile section */", lines, build_file_begin_ln
    )

    file_ref_begin_ln = xcode_util.find_line(
        "/* Begin PBXFileReference section */", lines, build_file_end_ln
    )
    file_ref_end_ln = xcode_util.find_line(
        "/* End PBXFileReference section */", lines, file_ref_begin_ln
    )

    group_begin_ln = xcode_util.find_line(
        "/* Begin PBXGroup section */", lines, file_ref_end_ln
    )
    group_end_ln = xcode_util.find_line(
        "/* End PBXGroup section */", lines, group_begin_ln
    )

    proj_sources_ln = xcode_util.find_line(
        "\t\t" + sources_key + " /* Sources */ = {", lines, group_end_ln
    )
    source_files_begin_ln = xcode_util.find_line(
        "\t\t\tfiles = (", lines, proj_sources_ln
    )
    ...
    xcode_util.remove_groups_and_file_refs(
        group_key, project, group_lines, file_ref_lines
    )
    xcode_util.remove_build_files_and_sources(source_files_lines, build_file_lines)

    # generate new information

    root = xcode_util.make_groups_and_file_refs(
        group_key, project, project, project_files
    )
    xcode_util.add_new_lines(
        "../src/",
        root,
        group_lines,
        file_ref_lines,
...
def project_main(path_to_mini, argv, project_callback, known_projects):
    ...
    for project in projects:
        ...
        project_path, project_files = build_util.list_project_files(
            path_to_mini, project
        )
        try:
            ...
            sys.exit(
                "Deleted Current project as new libraries were added.  Please re-run this script. "
            )
...
if __name__ == "__main__":
    ...
    os.system(
        "cd {source_dir}; ./update_ResidueType_enum_files.sh".format(
