...
def parseCommandline():
    ...
    try:
        options, patterns = getopt.getopt(sys.argv[1:], 'o:r:',
                                          ['version', 'output=', 'runner=',
                                           'gui=', 'error-printer',
                                           'abort-on-fail', 'have-std',
                                           'no-std', 'have-eh', 'no-eh',
                                           'template=', 'include=', 'root',
                                           'part', 'no-static-init', 'factor',
                                           'longlong='])
...
def setOptions(options):
    ...
    global outputFileName, templateFileName, runner, gui, haveStandardLibrary
    global factor, longlong
    global haveExceptionHandling, noExceptionHandling, abortOnFail, headers
    global root, part, noStaticInit
    for o, a in options:
        if o in ('-v', '--version'):
            printVersion()
        elif o in ('-o', '--output'):
...
def lineBelongsToSuite(suite, lineNo, line):
    ...
    This can be false when we are in a generated suite outside of
    CXXTEST_CODE() blocks. If the suite is generated, adds the line to the list
    of lines
    '''
...
std_re = re.compile(r"\b(std\s*::|CXXTEST_STD|using\s+namespace\s+std\b|^\s*"
                    r"\#\s*include\s+<[a-z0-9]+>)")
...
suite_re = re.compile(r'\bclass\s+(\w+)\s*:\s*public\s+((::)?\s*'
                      r'CxxTest\s*::\s*)?TestSuite\b')
...
test_re = re.compile(r'^([^/]|/[^/])*\bvoid\s+([Tt]est\w+)\s*'
                     r'\(\s*(void)?\s*\)')
...
def fixBlockLine(suite, lineNo, line):
    '''Change all [E]TS_ macros used in a line to _[E]TS_ macros with the
    correct file/line
    '''
...
create_re = re.compile(r'\bstatic\s+\w+\s*\*\s*createSuite\s*'
                       r'\(\s*(void)?\s*\)')
...
destroy_re = re.compile(r'\bstatic\s+void\s+destroySuite\s*'
                        r'\(\s*\w+\s*\*\s*\w*\s*\)')
...
def addSuiteCreateDestroy(suite, which, line):
    ...
    if which in suite:
        abort('%s:%s: %sSuite() already declared' %
              (suite['file'], str(line), which))
    suite[which] = line
...
def writeMain(output):
    ...
    if gui:
        ...
        output.write(' return CxxTest::GuiTuiRunner<CxxTest::%s, '
                     'CxxTest::%s>(argc, argv).run();\n' % (gui, runner))
        output.write('}\n')
    elif runner:
        # ----------------------------------------------
        # mini add on: We need command line arguments!!!
        output.write('#include <cxxtest/ProcessCommandLine.h>\n\n')

        output.write('int real_command_line_argc; char ** '
                     'real_command_line_argv;\n')
        output.write('int command_line_argc; char ** command_line_argv;\n\n')

        output.write('namespace CxxTest { RunType run_type = _NormalRun_; }'
                     '\n\n')

        # output.write('int main() {\n')
        output.write('int main(int argc, char *argv[]) {\n')
        output.write(' command_line_argc=argc; command_line_argv=argv;\n')
        output.write(' real_command_line_argc=argc; '
                     'real_command_line_argv=argv;\n\n')

        output.write(' CxxTest::ProcessCommandLine();\n')
        output.write(' if(CxxTest::run_type == _ListAllTests_) return 0;\n')
        ...
        output.write('  }\n')
        output.write('  std::cerr << std::endl;\n')
...
def writeTestDescription(output, suite, test):
    ...
    output.write('public:\n')
    if not noStaticInit:
        str = ' %s() : CxxTest::RealTestDescription(%s, %s, %s, "%s") {}\n'
        output.write(str % (test['class'], suite['tlist'], suite['dobject'],
                            test['line'], test['name']))
...
def dynamicRun(suite, test):
    ...
    return 'if (' + suite['object'] + ') ' + suite['object'] + '->' + \
           test['name'] + '();'
...
def writeDynamicDescription(output, suite):
    ...
    output.write('CxxTest::DynamicSuiteDescription<%s> %s' %
                 (suite['name'], suite['dobject']))
    if not noStaticInit:
        output.write('(%s, %s, "%s", %s, %s, %s, %s)' %
                     (suite['cfile'], suite['line'], suite['name'],
                      suite['tlist'], suite['object'], suite['create'],
                      suite['destroy']))
    output.write(';\n\n')
    ...
    for suite in suites:
        ...
        if isDynamic(suite):
            ...
            output.write('  %s.initialize(%s, %s, "%s", %s, %s, %s, %s);\n' %
                         (suite['dobject'], suite['cfile'], suite['line'],
                          suite['name'], suite['tlist'], suite['object'],
                          suite['create'], suite['destroy']))
        else:
            output.write('  %s.initialize(%s, %s, "%s", %s, %s);\n' %
                         (suite['dobject'], suite['cfile'], suite['line'],
                          suite['name'], suite['object'], suite['tlist']))

        for test in suite['tests']:
            output.write('  %s.initialize(%s, %s, %s, "%s");\n' %
                         (test['object'], suite['tlist'], suite['dobject'],
                          test['line'], test['name']))
