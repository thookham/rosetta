/*****************************************************************************
 * DAlphaSurf.c  : Computes the surface area of a union of balls
 *             using the Alpha Shape theory introduced by Herbert
 *             Edelsbrunner.
 *             Also computes the derivatives of the surface
 *             areas with respect to either the coordinates of the centers of
 *	       the balls, or the internal distances, if needed.
 *
 ...
 *
 * Calculations in the program DAlphaSurf are performed first in floating point; if the
...
char input_file[FSIZE], output_file[FSIZE], alpha_file[FSIZE];
char delaunay_file[FSIZE];
...
int list_redundant[MAX_ATOM];
int list_redalpha[MAX_ATOM];

/* Variables for surface calculation:
	coef:		: we compute a weighted sum of the individual surface 
			  coef is the array containing the weights
	surface		: contribution of each atom to the total surface area of the molecule
	deriv_surf	: derivatives of surface
								*/
double coef[MAX_ATOM];
double surface[MAX_ATOM];
double deriv_surf[3*MAX_ATOM];
// double deriv_surf_dist[MAX_PAIR];
...
with:     \n\
	...
	-s <switch> ........ switch = 0: computes surface area  only \n\
				      1: computes surface area and derivatives wrt coordinates  \n\
				      2: computes surface area and derivatives wrt internal distances \n\
                                      3: compute Delaunay only                  \n\
                                      4: compute Delaunay + Dual Complex only   \n\
	-o <res_file> ...... path name of output result file (default DATA.surf) \n\
...
void print_deriv(double surf);
void print_surface(double surf);
void write_delaunay();
void write_alphacx();
...
extern void F77Name2(surface_only)(double coef[MAX_ATOM], double *wsurf, double *surf, 
	double surface [MAX_ATOM]);
extern void F77Name2(surface_deriv_coord)(double coef[MAX_ATOM], double *wsurf, double *surf, 
	double surface [MAX_ATOM], double deriv_surf [3*MAX_ATOM]);
// extern void F77Name2(surface_deriv_dist)(double coef[MAX_ATOM], double *wsurf, double *surf, 
// 	double surface [MAX_ATOM], double deriv_surf [3*MAX_ATOM], double deriv_surf_dist [MAX_PAIR]);
...
int command_line (char *List[], int ListLength)
{
	register int i;
	int switchval;
	int OutputFileName = NO;
	int InputFileName = NO;
	...
	if (OutputFileName == 0) sprintf(output_file,"%s.surf",input_file);
...
/* and get surface area and possibly derivatives of surface area, if needed  */
...
//	printf("compute surface ...\n");
	starttime=clock();
	if(switchval == 0) {
		F77Name2(surface_only)(coef, &wsurf, &surf, surface); /* computes surface */
	}
	else {
		// if(switchval == 1) {
			F77Name2(surface_deriv_coord)(coef, &wsurf, &surf, surface, deriv_surf); /* computes surface */
		// }
		// else {
		// 	F77Name2(surface_deriv_dist)(coef, &wsurf, &surf, surface, 
		// 	deriv_surf, deriv_surf_dist); /* computes surface */
		// }
	}
	endtime=clock();
        timediff = ((double)(endtime-starttime))/CLOCKS_PER_SEC;
...
void print_surface(double surf)
{
	...
	fprintf(out,"#   i        X          Y          Z        R         Surf\n");
	for(iatom = 0; iatom < npoint; iatom++) 
	{
		fprintf(out," %5d %10.4f %10.4f %10.4f %10.4f %10.4f\n",iatom+1,F77Name2(xyz_vertex).coord[3*iatom+12],
		F77Name2(xyz_vertex).coord[3*iatom+13],F77Name2(xyz_vertex).coord[3*iatom+14],F77Name2(xyz_vertex).radius[iatom+4],surface[iatom+4]);
	}
	fprintf(out,"\n");
	fprintf(out,"#                                         Sum      %10.4f\n",surf);

	fclose(out);
}
/***************************************************************************************/
/* Print surface area and derivatives                                                       */
...
void print_deriv(double surf)
{
        ...
        fprintf(out,"#   i        X          Y          Z        R         Surf");
        fprintf(out,"      dSurf/dx   dSurf/dy   dSurf/dz      \n");
        for(iatom = 0; iatom < npoint; iatom++)
        {
                fprintf(out," %5d %10.4f %10.4f %10.4f %10.4f %10.4f",iatom+1,F77Name2(xyz_vertex).coord[3*iatom+12],
                F77Name2(xyz_vertex).coord[3*iatom+13],F77Name2(xyz_vertex).coord[3*iatom+14],F77Name2(xyz_vertex).radius[iatom+4],surface[iatom+4]);
                fprintf(out," %10.4f %10.4f %10.4f\n",
                deriv_surf[3*iatom+12],deriv_surf[3*iatom+13],deriv_surf[3*iatom+14]);
        }
        fprintf(out,"\n");
        fprintf(out,"#                                         Sum      %10.4f\n",surf);
...
/* The program computes a weighted sum of the contribution of each point to the 
   total surface area of the molecule of interest. Here we set all weights to 1, 
...
/* Performs calculation: compute regular triangulation, filter to get alpha shape with 
   alpha = 0 then compute surface area. if switchval is set to 1, the procedure also 
   computes the derivatives of the surface area with respect to the atomic 
   coordinates                 */
...
/* Write results in file. Only surface info are written out (no derivatives info)
*/
		if(switchval !=0 ) {
			(void) print_deriv(surf);
		}
		else {
			(void) print_surface(surf);
		}
	}
