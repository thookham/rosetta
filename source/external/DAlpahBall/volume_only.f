c	Volume_only.f		Version 1 9/1/2008	Patrice Koehl
c
c	This file contains a suite of routines for computed the (weighted) surface
c	area and volume of a union of balls. 
c
...
c
	subroutine volume_only(coef,WSurf,WVol,Surf,Vol,ballwsurf,
     1          ballwvol)
c
...
c
     ...
     3                  sh_abc,sh_acb,sh_bca,sh_abd,sh_adb,sh_bda,
     4                  sh_acd,sh_adc,sh_cda,sh_bcd,sh_bdc,sh_cdb,
     5                  pacb,pabd,padc,pbcd,
     6                  eps_abc,eps_abd,eps_acd,eps_bcd,
...
c
     ...
     3                  sh_abd,sh_adb,sh_bda,sh_abc,sh_acb,sh_bca,
     4                  sh_adc,sh_acd,sh_cda,sh_bdc,sh_bcd,sh_cdb,
     5                  padb,pabc,pacd,pbdc,
     6                  eps_abd,eps_abc,eps_acd,eps_bcd,
     7			surfa,surfb,surfd,surfc,vola,volb,vold,volc)
c
		endif
c
...
c
		if(iedge.eq.1) then
			call twosphere_vol(c,d,rc,rc2,rd,rd2,rcd,rcd2,
     1			surfc,surfd,volc,vold)
		elseif(iedge.eq.2) then
			call twosphere_vol(b,d,rb,rb2,rd,rd2,rbd,rbd2,
     1			surfb,surfd,volb,vold)
		elseif(iedge.eq.3) then
			call twosphere_vol(b,c,rb,rb2,rc,rc2,rbc,rbc2,
     1			surfb,surfc,volb,volc)
		elseif(iedge.eq.4) then
			call twosphere_vol(a,d,ra,ra2,rd,rd2,rad,rad2,
     1			surfa,surfd,vola,vold)
		elseif(iedge.eq.5) then
			call twosphere_vol(a,c,ra,ra2,rc,rc2,rac,rac2,
     1			surfa,surfc,vola,volc)
		elseif(iedge.eq.6) then
			call twosphere_vol(a,b,ra,ra2,rb,rb2,rab,rab2,
